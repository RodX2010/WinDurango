...
#pragma region String and identifier comparisons
    ...
    //
    // Indentifiers require a locale-less (ordinal), and often case-insensitive, comparison (filenames, registry keys,
    // XML node names, etc). DO NOT use locale-sensitive (lexical) comparisons for resource identifiers (e.g.wcs*()
    ...
    namespace details
    {
        [[nodiscard]] inline int CompareStringOrdinal(std::wstring_view left, std::wstring_view right,
                                                      bool caseInsensitive) WI_NOEXCEPT
        {
            ...
            return ::CompareStringOrdinal(left.data(), static_cast<int>(left.size()), right.data(),
                                          static_cast<int>(right.size()), caseInsensitive);
        }
    } // namespace details
    /// @endcond

    [[nodiscard]] inline wistd::weak_ordering compare_string_ordinal(std::wstring_view left, std::wstring_view right,
                                                                     bool caseInsensitive) WI_NOEXCEPT
    {
...
#pragma region FILETIME helpers
    ...
    namespace filetime
    {
        ...
        __WI_CONSTEXPR_BIT_CAST FILETIME from_int64(unsigned long long i64) WI_NOEXCEPT
        {
            ...
            static_assert(__alignof(unsigned long long) >= __alignof(FILETIME),
                          "alignment not compatible with type pun");
        ...
        /// Returns the current unbiased interrupt-time count, in units of 100 nanoseconds. The unbiased interrupt-time
        /// count does not include time the system spends in sleep or hibernation.
        ///
        /// This API avoids prematurely shortcircuiting timing loops due to system sleep/hibernation.
        ///
        /// This is equivalent to GetTickCount64() except it returns units of 100 nanoseconds instead of milliseconds,
        /// and it doesn't include time the system spends in sleep or hibernation. For example
        ///
        ...
        ///
        /// @note This is identical to QueryUnbiasedInterruptTime() but returns the value as a return value (rather than
        /// an out
        ///       parameter).
        ...
        /// Returns the current unbiased interrupt-time count, in units of milliseconds. The unbiased interrupt-time
        /// count does not include time the system spends in sleep or hibernation.
...
#pragma region RECT helpers
    template <typename rect_type> constexpr auto rect_width(rect_type const &rect)
    {
        return rect.right - rect.left;
    }

    template <typename rect_type> constexpr auto rect_height(rect_type const &rect)
    {
        return rect.bottom - rect.top;
    }

    template <typename rect_type> constexpr auto rect_is_empty(rect_type const &rect)
    {
...
#pragma endregion
    ...
    HRESULT AdaptFixedSizeToAllocatedResult(string_type &result,
                                            wistd::function<HRESULT(PWSTR, size_t, size_t *)> callback) WI_NOEXCEPT
    {
        details::string_maker<string_type> maker;

        wchar_t value[stackBufferLength]{};
        size_t valueLengthNeededWithNull{}; // callback returns the number of characters needed including the null
                                            // terminator.
    ...
    template <typename string_type, size_t stackBufferLength = 256>
    HRESULT SearchPathW(_In_opt_ PCWSTR path, _In_ PCWSTR fileName, _In_opt_ PCWSTR extension,
        ...
        return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
            ...
            [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
                ...
                *valueLengthNeededWithNul =
                    ::SearchPathW(path, fileName, extension, static_cast<DWORD>(valueLength), value, nullptr);
                ...
                // If the result is copied to the buffer, SearchPathW returns the length of copied string, WITHOUT the
                // NUL. If the buffer is too small to hold the result, SearchPathW returns the length of the required
                // buffer WITH the nul.
    ...
    HRESULT QueryFullProcessImageNameW(HANDLE processHandle, _In_ DWORD flags, string_type &result) WI_NOEXCEPT
    {
        return wil::AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
            ...
            [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
                ...
                *valueLengthNeededWithNul =
                    success ? (static_cast<size_t>(lengthToUse) + 1) : (static_cast<size_t>(lengthToUse) * 2);
                return S_OK;
            });
    }
    ...
    HRESULT ExpandEnvAndSearchPath(_In_ PCWSTR input, string_type &result) WI_NOEXCEPT
    {
        ...
        // ERROR_FILE_NOT_FOUND is an expected return value for SearchPathW
        const HRESULT searchResult =
            (wil::SearchPathW<string_type, stackBufferLength>(nullptr, expandedName.get(), nullptr, result));
        RETURN_HR_IF_EXPECTED(searchResult, searchResult == HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND));
        RETURN_IF_FAILED(searchResult);
    ...
    {
        auto adapter = [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
                           _Out_ size_t *valueLengthNeededWithNul) -> HRESULT {
            DWORD copiedCount{};
    ...
    string_type QueryFullProcessImageNameW(HANDLE processHandle = GetCurrentProcess(), DWORD flags = 0)
    {
        string_type result{};
        THROW_IF_FAILED(
            (wil::QueryFullProcessImageNameW<string_type, stackBufferLength>(processHandle, flags, result)));
        return result;
    }
...
// GetModuleHandleExW was added to the app partition in version 22000 of the SDK
    ...
    [[nodiscard]] inline auto get_module_reference_for_thread() noexcept
    {
        ...
        return wil::scope_exit([thisModule] { FreeLibraryAndExitThread(thisModule, 0); });
    }
#endif
...
#pragma warning(pop)
    ...
    */
    template <typename T>
    HRESULT init_once_nothrow(_Inout_ INIT_ONCE &initOnce, T func,
                              _Out_opt_ bool *callerCompleted = nullptr) WI_NOEXCEPT
    {
        BOOL pending = FALSE;
    ...
    */
    ...
    namespace details
    {
        template <typename RangeT> struct deduce_char_type_from_string_range
        {
            template <typename T = RangeT> static auto deduce(T &range)
            {
                using std::begin;
        ...
        template <typename T> struct iterable_span
        {
            T *pointer;
            size_t size;
    ...
    enum class ArgvToCommandLineFlags : uint8_t
    {
        ...
        //! By default, arguments are only surrounded by quotes when necessary (i.e. the argument contains a space).
        //! When this flag is specified, all arguments are surrounded with quotes, regardless of whether or not they
        //! contain space(s). This is an optimization as it means that we are not required to search for spaces in each
        //! argument and we don't need to potentially go back and insert a quotation character in the middle of the
        //! string after we've already written part of the argument to the result. That said, wrapping all arguments
        //! with quotes can have some adverse effects with some applications, most notably cmd.exe, which do their own
        //! command line processing.
        ForceQuotes = 0x01 << 0,

        //! `CommandLineToArgvW` has an "optimization" that assumes that the first argument is the path to an
        //! executable. Because valid NTFS paths cannot contain quotation characters, `CommandLineToArgvW` disables its
        //! quote escaping logic for the first argument. By default, `ArgvToCommandLine` aims to ensure that an argv
        //! array can "round trip" to a string and back, meaning it tries to replicate this logic in reverse. This flag
        //! disables this logic and escapes backslashes and quotes the same for all arguments, including the first one.
        //! This is useful if the output string is only an intermediate command line string (e.g. if the executable path
        //! is prepended later).
    ...
    //! Converts an argv array to a command line string that is guaranteed to "round trip" with CommandLineToArgvW. That
    //! is, a call to wil::ArgvToCommandLine followed by a call to ArgvToCommandLineW will produce the same array that
    //! was passed to wil::ArgvToCommandLine. Note that the reverse is not true. I.e. calling ArgvToCommandLineW
    //! followed by wil::ArgvToCommandLine will not produce the original string due to the optionality of of quotes in
    //! the command line string. This functionality is useful in a number of scenarios, most notably:
    //!     1.  When implementing a "driver" application. That is, an application that consumes some command line
    //!     arguments,
    //!         translates others into new arguments, and preserves the rest, "forwarding" the resulting command line to
    //!         a separate application.
    //!     2.  When reading command line arguments from some data storage, e.g. from a JSON array, which then need to
    //!     get
    ...
    //! information on how to control the behavior of this function as well as scenarios when you may want to use each
    //! one.
    template <typename RangeT, typename CharT = details::deduce_char_type_from_string_range_t<RangeT>>
    inline std::basic_string<CharT> ArgvToCommandLine(RangeT &&range,
                                                      ArgvToCommandLineFlags flags = ArgvToCommandLineFlags::None)
    {
            ...
            while (pos < str.size())
            {
                ...
                if (str[pos] == '"')
                {
                    ...
                    else
                    {
                        ...
                        // path begins with a quote, even if the next character is not a space. This assert won't catch
                        // all of such issues as we may detect a space, and therefore the need to surroud the argument
                        // with quotes, later in the string; this is best effort. Such arguments wouldn't be valid and
                        // are not representable anyway
                        WI_ASSERT((pos > 0) && !WI_IsFlagSet(flags, ArgvToCommandLineFlags::ForceQuotes) &&
                                  !terminateWithQuotes);
                ...
                else if (str[pos] == '\\')
                {
                    ...
                    // backslashes will be functionally equivalent to escaping each one. This copies all of the
                    // backslashes once. We _might_ do it again later
                    result.append(str, pos, nextPos - pos);
