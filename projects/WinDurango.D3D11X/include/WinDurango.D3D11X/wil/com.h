...
namespace wil
{
    ...
    namespace details
    {
        ...
        struct is_com_convertible : wistd::bool_constant<__is_convertible_to(TFrom, TTo) &&
                                                         (__is_abstract(TFrom) || wistd::is_same<TFrom, TTo>::value)>
        {
        };
        ...
        {
          public:
            template <typename T> inline static HRESULT query(_In_ T *ptr, REFIID riid, _COM_Outptr_ void **result)
            {
          ...
          private:
            template <typename T, typename TResult>
            inline static HRESULT query_dispatch(_In_ T *ptr, wistd::true_type,
                                                 _COM_Outptr_ TResult **result) // convertible
            {
                *result = ptr;
                (*result)->AddRef();
                return S_OK;
            ...
            template <typename T, typename TResult>
            inline static HRESULT query_dispatch(_In_ T *ptr, wistd::false_type,
                                                 _COM_Outptr_ TResult **result) // not convertible
            {
        ...
        template <typename T> struct query_policy_helper
        {
        ...
        class weak_query_policy
        {
          public:
            ...
            {
                static_assert(!wistd::is_same<IWeakReference, TResult>::value,
                              "Cannot resolve a weak reference to IWeakReference");
        ...
        template <> struct query_policy_helper<IWeakReference>
        {
          ...
          public:
            inline static HRESULT query(_In_ IAgileReference *ptr, REFIID riid, _COM_Outptr_ void **result)
            {
                ...
                __analysis_assume(SUCCEEDED(hr) ||
                                  (*result == nullptr)); // IAgileReference::Resolve not annotated correctly
                return hr;
            }

            template <typename TResult> static HRESULT query(_In_ IAgileReference *ptr, _COM_Outptr_ TResult **result)
            {
                static_assert(!wistd::is_same<IAgileReference, TResult>::value,
                              "Cannot resolve a agile reference to IAgileReference");
                return query(ptr, __uuidof(TResult), reinterpret_cast<void **>(result));
            }
        };

        template <> struct query_policy_helper<IAgileReference>
        {
    ...
    //!                         For com_ptr, this will always be the interface being represented.  For com_weak_ref,
    //!                         this will always be IWeakReference.  For com_agile_ref, this will always be
    //!                         IAgileReference.
    //! @tparam err_policy      Represents the error policy for the class (error codes, exceptions, or fail fast; see
    //! @ref page_errors)
    template <typename T, typename err_policy = err_exception_policy> class com_ptr_t
    {
      ...
      public:
        ...
        //! Move assign from a like `com_ptr_t` (releases current pointer, avoids AddRef/Release by moving the
        //! parameter).
        ...
        //! Move assignment from a compatible `com_ptr_t` (releases current pointer, avoids AddRef/Release by moving
        //! from the parameter).
        template <class U, typename err, class = wistd::enable_if_t<__is_convertible_to(U *, pointer)>>
        ...
        template <typename err> void swap(com_ptr_t<T, err> &other) WI_NOEXCEPT
        {
            auto ptr = m_ptr;
            m_ptr = other.m_ptr;
            other.m_ptr = ptr;
        ...
        //! Takes ownership of a compatible raw interface pointer (releases pointer, copies but DOES NOT AddRef the
        //! parameter).
        void attach(pointer other) WI_NOEXCEPT
        {
            ...
            if (ptr)
            {
                ...
                WI_ASSERT_MSG(((other != ptr) || (ref > 0)),
                              "Bug: Attaching the same already assigned, destructed pointer");
            }
        }

        //! Relinquishes ownership and returns the internal interface pointer (DOES NOT release the detached pointer,
        //! sets class pointer to null).
        ...
        //! Returns the address of the internal pointer (releases ownership of the pointer BEFORE returning the
        //! address). The pointer is explicitly released to prevent accidental leaks of the pointer.  Coding standards
        //! generally indicate that there is little valid `_Inout_` use of `IInterface**`, making this safe to do under
        //! typical use.
        //! @see addressof
        //! ~~~~
        ...
        //! Returns the address of the internal pointer casted to void** (releases ownership of the pointer BEFORE
        ...
        //! Returns the address of the internal pointer casted to IUnknown** (releases ownership of the pointer BEFORE
        //! returning the address).
        //! @see put
        ...
        //! address). The pointer is explicitly released to prevent accidental leaks of the pointer.  Coding standards
        //! generally indicate that there is little valid `_Inout_` use of `IInterface**`, making this safe to do under
        //! typical use.  Since this behavior is not always immediately apparent, prefer to scope variables as close to
        //! use as possible (generally avoiding use of the same com_ptr variable in successive calls to receive an
        //! output interface).
        //! @see addressof
        ...
        //! Returns the address of the internal pointer (does not release the pointer; should not be used for `_Out_`
        //! parameters)
        ...
        //! Query and return a smart pointer matching the interface specified by 'U':  `auto foo =
        //! m_ptr.query<IFoo>();`. See @ref page_query for more information.
        //!
        //! This method is the primary method that should be used to query a com_ptr in exception-based or fail-fast
        //! based code. Error-code returning code should use @ref query_to so that the returned HRESULT can be examined.
        //! In the following examples, `m_ptr` is an exception-based or fail-fast based com_ptr, com_weak_ref, or
        //! com_agile_ref:
        //! ~~~~
        ...
        //!         `com_ptr_t` type will be @ref com_ptr or @ref com_ptr_failfast (matching the error handling form of
        //!         the pointer being queried (exception based or fail-fast).
        template <class U> WI_NODISCARD inline com_ptr_t<U, err_policy> query() const
        {
            static_assert(wistd::is_same<void, result>::value,
                          "query requires exceptions or fail fast; use try_query or query_to");
        ...
        //! See @ref page_query for more information.
        //!
        //! For fail-fast and exception-based behavior this routine should primarily be used to write to out parameters
        //! and @ref query should be used to perform most queries.  For error-code based code, this routine is the
        //! primary method that should be used to query a com_ptr.
        //!
        ...
        //! }
        //! ~~~~
        //! @tparam U           Represents the interface being queried (type of the output parameter).  This interface
        //! does not need
        //!                     to be specified directly.  Rely upon template type deduction to pick up the type from
        //!                     the output parameter.
        //! @param ptrResult    The output pointer that will receive the newly queried interface.  This pointer will be
        //! assigned null
        //!                     on failure.
        //! @return             For the nothrow (error code-based) classes (@ref com_ptr_nothrow, @ref
        //! com_weak_ref_nothrow,
        //!                     @ref com_agile_ref_nothrow) this method returns an `HRESULT` indicating whether the
        //!                     query was successful.  Exception-based and fail-fast based classes do not return a value
        //!                     (void).
        template <class U> result query_to(_COM_Outptr_ U **ptrResult) const
        {
            // Prefast cannot see through the error policy + query_policy mapping and as a result fires 6388 and 28196
            // for this function. Suppression is also not working. Wrapping this entire function in #pragma
            // warning(disable: 6388 28196) does not stop all of the prefast errors from being emitted.
        ...
        //! See @ref page_query for more information.
        //!
        //! This method is built to implement an API boundary that exposes a returned pointer to a caller through the
        //! REFIID and void** pointer pattern (like QueryInterface).  This pattern should not be used outside of that
        //! pattern (through IID_PPV_ARGS) as it is less efficient than the typed version of @ref query_to which can
        //! elide the QueryInterface in favor of AddRef when the types are convertible.
        //! ~~~~
        //! // class member being queried:
        //! wil::com_ptr_nothrow<IUnknown> m_ptr;
        //!
        ...
        {
            // Prefast cannot see through the error policy + query_policy mapping and as a result and as a result fires
            // 6388 and 28196 for this function. Suppression is also not working. Wrapping this entire function in
            // #pragma warning(disable: 6388 28196) does not stop the prefast errors from being emitted.
#if defined(_PREFAST_)
            *ptrResult = nullptr;
            return err_policy::HResult(E_NOINTERFACE);
#else
        ...
        //! m_ptr.try_query<IFoo>();` (null result when interface is unsupported). See @ref page_query for more
        //! information.
        //!
        //! This method can be used to query a com_ptr for an interface when it's known that support for that interface
        //! is optional (failing the query should not produce an error).  The caller must examine the returned pointer
        ...
        //! ~~~~
        //! @tparam U   Represents the interface being queried
        //! @return     A `com_ptr_t` pointer to the given interface `U`.  The returned pointer will be null if the
        //! interface is
        //!             not supported.  The returned `com_ptr_t` will have the same error handling policy (exceptions,
        //!             failfast or error codes) as the pointer being queried.
        template <class U> WI_NODISCARD inline com_ptr_t<U, err_policy> try_query() const
        {
            return com_ptr_t<U, err_policy>(m_ptr, details::tag_try_com_query());
        }

        //! Attempts to query for the interface matching the given output parameter; returns a bool indicating if the
        //! query was successful (non-null). See @ref page_query for more information.
        //!
        //! This method can be used to perform a query against a non-null interface when it's known that support for
        //! that interface is optional (failing the query should not produce an error).  The caller must examine the
        //! returned bool before using the returned pointer.
        //! ~~~~
        //! wil::com_ptr_nothrow<IFoo> foo;
        ...
        //! }
        //! ~~~~
        //! @param ptrResult    The pointer to query for.  The interface to query is deduced from the type of this out
        //! parameter; do
        //!                     not specify the type directly to the template.
        //! @return             A `bool` indicating `true` of the query was successful (the returned parameter is
        //! non-null).
        template <class U> _Success_return_ bool try_query_to(_COM_Outptr_ U **ptrResult) const
        {
        ...
        //!
        //! This method is built to implement an API boundary that exposes a returned pointer to a caller through the
        //! REFIID and void** pointer pattern (like QueryInterface).  The key distinction is that this routine does not
        //! produce an error if the request isn't fulfilled, so it's appropriate for `_COM_Outptr_result_maybenull_`
        //! cases.  This pattern should not be used outside of that pattern (through IID_PPV_ARGS) as it is less
        //! efficient than the typed version of @ref try_query_to which can elide the QueryInterface in favor of AddRef
        //! when the types are convertible.  The caller must examine the returned bool before using the returned
        //! pointer.
        //! ~~~~
        //! // class member being queried:
        //! wil::com_ptr_nothrow<IUnknown> m_ptr;
        ...
        //! Query and return a smart pointer matching the interface specified by 'U':  `auto foo = m_ptr.copy<IFoo>();`
        //! (succeeds and returns a null ptr if the queried pointer is null). See @ref page_query for more information.
        //!
        //! This method is identical to @ref query with the exception that it can be used when the pointer is null. When
        //! used against a null pointer, the returned pointer will always be null and an error will not be produced.
        //! Like query it will produce an error for a non-null pointer that does not support the requested interface.
        //! @tparam U Represents the interface being queried
        //! @return A `com_ptr_t` pointer to the given interface `U`.  The pointer will be null ONLY if the pointer
        //! being queried is
        //!         null.  The returned `com_ptr_t` type will be @ref com_ptr or @ref com_ptr_failfast (matching the
        //!         error handling form of the pointer being queried (exception based or fail-fast).
        template <class U> WI_NODISCARD inline com_ptr_t<U, err_policy> copy() const
        {
            static_assert(wistd::is_same<void, result>::value,
                          "copy requires exceptions or fail fast; use the try_copy or copy_to method");
            return com_ptr_t<U, err_policy>(m_ptr, details::tag_com_copy());
        }

        //! Query for the interface of the given out parameter `U`:  `ptr.copy_to(&foo);` (succeeds and returns null ptr
        //! if the queried pointer is null). See @ref page_query for more information.
        //!
        //! This method is identical to @ref query_to with the exception that it can be used when the pointer is null.
        //! When used against a null pointer, the returned pointer will always be null and an error will not be
        //! produced.  Like query_to it will produce an error for a non-null pointer that does not support the requested
        //! interface.
        //! @tparam U           Represents the interface being queried (type of the output parameter).  This interface
        //! does not need
        //!                     to be specified directly.  Rely upon template type deduction to pick up the type from
        ...
        //! com_weak_ref_nothrow,
        //!                     @ref com_agile_ref_nothrow) this method returns an `HRESULT` indicating whether the
        //!                     query was successful.  Copying a null value is considered success. Exception-based and
        //!                     fail-fast based classes do not return a value (void).
        template <class U> result copy_to(_COM_Outptr_result_maybenull_ U **ptrResult) const
        {
            if (m_ptr)
            {
                // Prefast cannot see through the error policy + query_policy mapping and as a result and as a result
                // fires 6388 and 28196 for this function. Suppression is also not working. Wrapping this entire
                // function in #pragma warning(disable: 6388 28196) does not stop the prefast errors from being emitted.
#if defined(_PREFAST_)
                *ptrResult = nullptr;
                return err_policy::HResult(E_NOINTERFACE);
#else
        ...
        //! Query for the requested interface using the iid, ppv pattern:  `ptr.copy_to(riid, ptr);`. (succeeds and
        //! returns null ptr if the queried pointer is null). See @ref page_query for more information.
        //!
        //! Identical to the corresponding @ref query_to method with the exception that it can be used when the pointer
        //! is null.  When used against a null pointer, the returned pointer will always be null and an error will not
        //! be produced.  Like query_to it will produce an error for a non-null pointer that does not support the
        //! requested interface.
        //! @param riid         The interface to query for.
        //! @param ptrResult    The output pointer that will receive the newly queried interface.  This pointer will be
        //! assigned null
        ...
        //! Attempt a query and return a smart pointer matching the interface specified by 'U':  `auto foo =
        //! m_ptr.try_query<IFoo>();` (null result when interface is unsupported or queried pointer is null). See @ref
        //! page_query for more information.
        //!
        //! Identical to the corresponding @ref try_query method with the exception that it can be used when the pointer
        //! is null. When used against a null pointer, the returned pointer will always be null and an error will not be
        //! produced.
        //! @tparam U   Represents the interface being queried
        //! @return     A `com_ptr_t` pointer to the given interface `U`.  The returned pointer will be null if the
        //! interface was
        //!             not supported or the pointer being queried is null.  The returned `com_ptr_t` will have the same
        //!             error handling policy (exceptions, failfast or error codes) as the pointer being queried.
        template <class U> WI_NODISCARD inline com_ptr_t<U, err_policy> try_copy() const
        {
            return com_ptr_t<U, err_policy>(m_ptr, details::tag_try_com_copy());
        }

        //! Attempts to query for the interface matching the given output parameter; returns a bool indicating if the
        //! query was successful (returns `false` if the pointer is null). See @ref page_query for more information.
        //!
        //! Identical to the corresponding @ref try_query_to method with the exception that it can be used when the
        //! pointer is null. When used against a null pointer, the returned pointer will be null and the return value
        //! will be `false`.
        //! @param ptrResult    The pointer to query for.  The interface to query is deduced from the type of this out
        //! parameter; do
        //!                     not specify the type directly to the template.
        //! @return             A `bool` indicating `true` of the query was successful (the returned parameter is
        //! non-null).
        template <class U> _Success_return_ bool try_copy_to(_COM_Outptr_result_maybenull_ U **ptrResult) const
        {
            if (m_ptr)
            {
                return SUCCEEDED(query_policy::query(m_ptr, ptrResult));
        ...
        //! Attempts a query for the requested interface using the iid, ppv pattern:  `ptr.try_query_to(riid, ptr);`
        //! (returns `false` if the pointer is null) See @ref page_query for more information.
        //!
        //! Identical to the corresponding @ref try_query_to method with the exception that it can be used when the
        //! pointer is null. When used against a null pointer, the returned pointer will be null and the return value
        //! will be `false`.
      ...
      public:
        // Internal Helpers
        /// @cond
        template <class U> inline com_ptr_t(_In_ U *ptr, details::tag_com_query) : m_ptr(nullptr)
        {
            err_policy::HResult(details::query_policy_t<U>::query(ptr, &m_ptr));
        }

        template <class U> inline com_ptr_t(_In_ U *ptr, details::tag_try_com_query) WI_NOEXCEPT : m_ptr(nullptr)
        {
            details::query_policy_t<U>::query(ptr, &m_ptr);
        }

        template <class U> inline com_ptr_t(_In_opt_ U *ptr, details::tag_com_copy) : m_ptr(nullptr)
        {
            if (ptr)
            {
                err_policy::HResult(details::query_policy_t<U>::query(ptr, &m_ptr));
        ...
        template <class U> inline com_ptr_t(_In_opt_ U *ptr, details::tag_try_com_copy) WI_NOEXCEPT : m_ptr(nullptr)
        {
            if (ptr)
            {
                details::query_policy_t<U>::query(ptr, &m_ptr);
    ...
    template <typename T> using com_ptr = com_ptr_t<T, err_exception_policy>;
#endif

    //! COM pointer, errors return error codes (see @ref com_ptr_t for details)
    template <typename T> using com_ptr_nothrow = com_ptr_t<T, err_returncode_policy>;

    //! COM pointer, errors fail-fast (see @ref com_ptr_t for details)
    template <typename T> using com_ptr_failfast = com_ptr_t<T, err_failfast_policy>;
    ...
    template <typename T, typename Err> inline void swap(com_ptr_t<T, Err> &left, com_ptr_t<T, Err> &right) WI_NOEXCEPT
    {
        left.swap(right);
    }
    ...
    {
        static_assert(__is_convertible_to(TLeft *, TRight *) || __is_convertible_to(TRight *, TLeft *),
    ...
    inline bool operator==(const com_ptr_t<TLeft, ErrLeft> &left,
                           const Microsoft::WRL::ComPtr<TRight> &right) WI_NOEXCEPT
    {
        static_assert(__is_convertible_to(TLeft *, TRight *) || __is_convertible_to(TRight *, TLeft *),
                      "comparison operator requires left and right pointers to be compatible");
        return (left.get() == right.Get());
    ...
    inline bool operator==(const Microsoft::WRL::ComPtr<TLeft> &left,
                           const com_ptr_t<TRight, ErrRight> &right) WI_NOEXCEPT
    {
        static_assert(__is_convertible_to(TLeft *, TRight *) || __is_convertible_to(TRight *, TLeft *),
                      "comparison operator requires left and right pointers to be compatible");
        return (left.Get() == right.get());
    ...
    //! An overloaded function that retrieves the raw com pointer from a raw pointer, wil::com_ptr_t<T>, WRL ComPtr<T>,
    //! or Platform::Object^.  This function is primarily useful by library or helper code.  It allows code to be
    //! written to accept a forwarding reference template that can be used as an input com pointer.  That input com
    //! pointer is allowed to be any of:
    ...
    //! ~~~~
    template <typename T> T *com_raw_ptr(T *ptr)
    {
        return ptr;
    }

    /// @cond
    template <typename T, typename err> T *com_raw_ptr(const wil::com_ptr_t<T, err> &ptr)
    {
        return ptr.get();
    }

    template <typename T> T *com_raw_ptr(const Microsoft::WRL::ComPtr<T> &ptr)
    {
        return ptr.Get();
    }
...
inline IInspectable* com_raw_ptr(T^ ptr)
{
    ...
    //! }
    //! ~~~
    template <typename T> com_ptr<T> make_com_ptr(T *p)
    {
        return p;
    }
#endif
    ...
    //! }
    //! ~~~
    template <typename T> com_ptr_nothrow<T> make_com_ptr_nothrow(T *p)
    {
        return p;
    }
    ...
    //! }
    //! ~~~
    template <typename T> com_ptr_failfast<T> make_com_ptr_failfast(T *p)
    {
        return p;
    }
    ...
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), should
    //! not be null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr<U>` pointer to the given interface `U`.  The returned pointer is guaranteed
    //! not null.
    template <typename U, typename T> inline com_ptr<U> com_query(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! Queries for the specified interface and returns a fail-fast-based wil::com_ptr_failfast to that interface
    //! (fail-fast if unsupported). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), should
    //! not be null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr<U>` pointer to the given interface `U`.  The returned pointer is guaranteed
    //! not null.
    template <typename U, typename T> inline com_ptr_failfast<U> com_query_failfast(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename U, typename T> _Success_true_ void com_query_to(T &&ptrSource, _COM_Outptr_ U **ptrResult)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename U, typename T> HRESULT com_query_to_nothrow(T &&ptrSource, _COM_Outptr_ U **ptrResult)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! @param riid         The interface to query for
    //! @param ptrResult    Represents the output pointer to populate.  The returned pointer is guaranteed not null.
    template <typename T> _Success_true_ void com_query_to(T &&ptrSource, REFIID riid, _COM_Outptr_ void **ptrResult)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! @param riid         The interface to query for
    //! @param ptrResult    Represents the output pointer to populate.  The returned pointer will be null upon failure.
    template <typename T> HRESULT com_query_to_nothrow(T &&ptrSource, REFIID riid, _COM_Outptr_ void **ptrResult)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
...
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Attempts a query for the specified interface and returns an exception-based wil::com_ptr to that interface
    //! (returns null if unsupported). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), should
    //! not be null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr<U>` pointer to the given interface `U`.  The returned pointer is null if the
    //! requested interface was not supported.
    template <class U, typename T> inline com_ptr<U> try_com_query(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! Attempts a query for the specified interface and returns an fail-fast wil::com_ptr_failfast to that interface
    //! (returns null if unsupported). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), should
    //! not be null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr_failfast<U>` pointer to the given interface `U`.  The returned pointer is
    //! null if the
    //!                     requested interface was not supported.
    template <class U, typename T> inline com_ptr_failfast<U> try_com_query_failfast(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
        return com_ptr_failfast<U>(raw, details::tag_try_com_query());
    }

    //! Attempts a query for the specified interface and returns an error-code-based wil::com_ptr_nothrow to that
    //! interface (returns null if unsupported). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), should
    //! not be null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr_nothrow<U>` pointer to the given interface `U`.  The returned pointer is
    //! null if the
    //!                     requested interface was not supported.
    template <class U, typename T> inline com_ptr_nothrow<U> try_com_query_nothrow(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
        return com_ptr_nothrow<U>(raw, details::tag_try_com_query());
    }

    //! Attempts a query for the interface specified by the type of the output parameter (returns `false` if
    //! unsupported). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), should
    //! not be null.
    //! @param ptrResult    Represents the output pointer to populate.  If the interface is unsupported, the returned
    //! pointer will be
    //!                     null.
    //! @return             A bool value representing whether the query was successful (non-null return result).
    template <typename U, typename T> _Success_return_ bool try_com_query_to(T &&ptrSource, _COM_Outptr_ U **ptrResult)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
...
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Queries for the specified interface and returns an exception-based wil::com_ptr to that interface (exception if
    //! unsupported, preserves null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr<U>` pointer to the given interface `U`.  The returned pointer will be null
    //! only if the
    //!                     source is null.
    template <class U, typename T> inline com_ptr<U> com_copy(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! Queries for the specified interface and returns a fail-fast-based wil::com_ptr_failfast to that interface
    //! (fail-fast if unsupported, preserves null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr<U>` pointer to the given interface `U`.  The returned pointer will be null
    //! only if the
    //!                     source is null.
    template <class U, typename T> inline com_ptr_failfast<U> com_copy_failfast(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
...
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Queries for the interface specified by the type of the output parameter (throws an exception if unsupported,
    //! preserves null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @param ptrResult    Represents the output pointer to populate.  The returned pointer will be null only if the
    //! source is null.
    template <typename U, typename T>
    ...
    //! Queries for the interface specified by the type of the output parameter (fail-fast if unsupported, preserves
    //! null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @param ptrResult    Represents the output pointer to populate.  The returned pointer will be null only if the
    //! source is null.
    ...
    //! or if the
    //!                     source is null.
    //! @return             Returns an HRESULT representing whether the query succeeded (returns S_OK if the source is
    //! null).
    template <typename U, typename T>
...
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Queries for the interface specified by the given REFIID parameter (throws an exception if unsupported, preserves
    //! null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @param riid         The interface to query for
    ...
    //! Queries for the interface specified by the given REFIID parameter (returns an error if unsupported, preserves
    //! null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @param riid         The interface to query for
...
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Attempts a query for the specified interface and returns an exception-based wil::com_ptr to that interface
    //! (returns null if unsupported, preserves null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr<U>` pointer to the given interface `U`.  The returned pointer is null if the
    ...
    template <class U, typename T> inline com_ptr<U> try_com_copy(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! null if the
    //!                     requested interface was not supported.
    template <class U, typename T> inline com_ptr_failfast<U> try_com_copy_failfast(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    //! Attempts a query for the specified interface and returns an error-code-based wil::com_ptr_nothrow to that
    //! interface (returns null if unsupported, preserves null). See @ref page_query for more information.
    //! @param ptrSource    The pointer to query (may be a raw interface pointer, wil com_ptr, or WRL ComPtr), may be
    //! null
    //! @tparam U           Represents the interface being queried
    //! @return             A `wil::com_ptr_nothrow<U>` pointer to the given interface `U`.  The returned pointer is
    //! null if the
    //!                     requested interface was not supported.
    template <class U, typename T> inline com_ptr_nothrow<U> try_com_copy_nothrow(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
...
//! @}
...
#if (NTDDI_VERSION >= NTDDI_WINBLUE)
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Agile reference to a COM interface, errors throw exceptions (see @ref com_ptr_t and @ref com_agile_query for
    //! details)
    using com_agile_ref = com_ptr<IAgileReference>;
#endif
    //! Agile reference to a COM interface, errors return error codes (see @ref com_ptr_t and @ref
    //! com_agile_query_nothrow for details)
    using com_agile_ref_nothrow = com_ptr_nothrow<IAgileReference>;
    //! Agile reference to a COM interface, errors fail fast (see @ref com_ptr_t and @ref com_agile_query_failfast for
    //! details)
    ...
    template <typename T>
    com_agile_ref_failfast com_agile_query_failfast(T &&ptrSource,
                                                    AgileReferenceOptions options = AGILEREFERENCE_DEFAULT)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename T>
    HRESULT com_agile_query_nothrow(T &&ptrSource, _COM_Outptr_ IAgileReference **ptrResult,
                                    AgileReferenceOptions options = AGILEREFERENCE_DEFAULT)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename T>
    com_agile_ref_failfast com_agile_copy_failfast(T &&ptrSource,
                                                   AgileReferenceOptions options = AGILEREFERENCE_DEFAULT)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
        com_agile_ref_failfast agileRef;
    ...
    //! return an agile ref (com_agile_ref_XXX or other representation) representing the given source pointer (return
    //! error on failure, source maybe null)
    template <typename T>
    HRESULT com_agile_copy_nothrow(T &&ptrSource, _COM_Outptr_result_maybenull_ IAgileReference **ptrResult,
                                   AgileReferenceOptions options = AGILEREFERENCE_DEFAULT)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
        if (raw)
...
//! @}
    ...
    namespace details
    {
        template <typename T> HRESULT GetWeakReference(T *ptr, _COM_Outptr_ IWeakReference **weakReference)
        {
            static_assert(!wistd::is_same<IWeakReference, T>::value,
                          "Cannot get an IWeakReference to an IWeakReference");
...
#ifdef WIL_ENABLE_EXCEPTIONS
    //! Weak reference to a COM interface, errors throw exceptions (see @ref com_ptr_t and @ref com_weak_query for
    //! details)
    using com_weak_ref = com_ptr<IWeakReference>;
#endif
    //! Weak reference to a COM interface, errors return error codes (see @ref com_ptr_t and @ref com_weak_query_nothrow
    //! for details)
    using com_weak_ref_nothrow = com_ptr_nothrow<IWeakReference>;
    //! Weak reference to a COM interface, errors fail fast (see @ref com_ptr_t and @ref com_weak_query_failfast for
    //! details)
    ...
    template <typename T> com_weak_ref com_weak_query(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename T> com_weak_ref_failfast com_weak_query_failfast(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename T> HRESULT com_weak_query_nothrow(T &&ptrSource, _COM_Outptr_ IWeakReference **ptrResult)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
    ...
    template <typename T> com_weak_ref com_weak_copy(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
        com_weak_ref weakRef;
        if (raw)
    ...
    template <typename T> com_weak_ref_failfast com_weak_copy_failfast(T &&ptrSource)
    {
        auto raw = com_raw_ptr(wistd::forward<T>(ptrSource));
        com_weak_ref_failfast weakRef;
        if (raw)
...
#pragma region COM Object Helpers
    ...
    template <typename T> inline bool is_agile(T &&ptrSource)
    {
    ...
    /** constructs a COM object using the class as the identifier (that has an associated CLSID) on a specific interface
    or IUnknown. */
    ...
    wil::com_ptr_failfast<Interface> CoCreateInstanceFailFast(REFCLSID rclsid,
                                                              DWORD dwClsContext = CLSCTX_INPROC_SERVER) WI_NOEXCEPT
    {
    ...
    template <typename Interface = IUnknown>
    wil::com_ptr_nothrow<Interface> CoCreateInstanceNoThrow(REFCLSID rclsid,
                                                            DWORD dwClsContext = CLSCTX_INPROC_SERVER) WI_NOEXCEPT
    {
    ...
    /** constructs a COM object using the class as the identifier (that has an associated CLSID) on a specific interface
    or IUnknown.  Note, failures are reported as a null result, the HRESULT is lost. */
    template <typename Class, typename Interface = IUnknown>
    ...
    wil::com_ptr_failfast<Interface> CoGetClassObjectFailFast(REFCLSID rclsid,
    ...
    namespace details
    {
        ...
        auto CoCreateInstanceEx(REFCLSID clsid, CLSCTX clsCtx) noexcept
        {
            ...
            std::apply(
                [i = 0, &multiQis](auto &...a) mutable {
                    (a.attach(reinterpret_cast<typename std::remove_reference<decltype(a)>::type::pointer>(
                         multiQis[i++].pItf)),
                     ...);
                },
                resultTuple);
        ...
        template <typename error_policy, typename... Results> auto com_multi_query(IUnknown *obj)
        {
            MULTI_QI multiQis[sizeof...(Results)]{};
            const IID *iids[sizeof...(Results)]{&__uuidof(Results)...};
    ...
    template <typename... Results> auto CoCreateInstanceEx(REFCLSID clsid, CLSCTX clsCtx = CLSCTX_LOCAL_SERVER)
    {
    ...
    template <typename... Results> auto TryCoCreateInstanceEx(REFCLSID clsid, CLSCTX clsCtx = CLSCTX_LOCAL_SERVER)
    {
        auto [error, result] = details::CoCreateInstanceEx<err_exception_policy, Results...>(clsid, clsCtx);
        return result;
    }
...
#ifdef WIL_ENABLE_EXCEPTIONS
    template <typename... Results> auto com_multi_query(IUnknown *obj)
    {
    ...
    template <typename... Results> auto try_com_multi_query(IUnknown *obj)
    {
        auto [error, result] = details::com_multi_query<err_exception_policy, Results...>(obj);
        return result;
    }
...
#pragma region Stream helpers
    ...
    */
    inline HRESULT stream_read_partial_nothrow(_In_ ISequentialStream *stream,
                                               _Out_writes_bytes_to_(size, *wrote) void *data, unsigned long size,
                                               unsigned long *wrote)
    {
    ...
    */
    inline HRESULT stream_read_nothrow(_In_ ISequentialStream *stream, _Out_writes_bytes_all_(size) void *data,
                                       unsigned long size)
    {
    ...
    */
    ...
    @param size The size, in bytes, of the buffer pointed to by `data`
    */
    inline HRESULT stream_write_nothrow(_In_ ISequentialStream *stream, _In_reads_bytes_(size) const void *data,
                                        unsigned long size)
    {
        unsigned long wrote;
    ...
    */
    inline HRESULT stream_seek_nothrow(_In_ IStream *stream, long long offset, unsigned long from,
                                       _Out_opt_ unsigned long long *value = nullptr)
    {
    ...
    @param value Optionally receives the new absolute position from the stream
    */
    inline HRESULT stream_set_position_nothrow(_In_ IStream *stream, unsigned long long offset,
                                               _Out_opt_ unsigned long long *value = nullptr)
    {
    ...
    */
    inline HRESULT stream_seek_from_current_position_nothrow(_In_ IStream *stream, long long amount,
                                                             _Out_opt_ unsigned long long *value = nullptr)
    {
    ...
    */
    inline HRESULT stream_copy_bytes_nothrow(_In_ IStream *source, _In_ IStream *target, unsigned long long amount,
                                             _Out_opt_ unsigned long long *pCopied = nullptr)
    {
    ...
    @param pCopied If non-null, set to the number of bytes copied between the two.
    */
    inline HRESULT stream_copy_all_nothrow(_In_ IStream *source, _In_ IStream *target,
                                           _Out_opt_ unsigned long long *pCopied = nullptr)
    {
    ...
    */
    inline HRESULT stream_read_string_nothrow(_In_ ISequentialStream *source,
                                              _When_(options == empty_string_options::returns_empty, _Outptr_result_z_)
                                                  _When_(options == empty_string_options::returns_null,
                                                         _Outptr_result_maybenull_z_) wchar_t **value,
                                              empty_string_options options = empty_string_options::returns_empty)
    {
        ...
        else
        {
            ...
            RETURN_IF_FAILED(
                stream_read_nothrow(source, allocated.get(), static_cast<unsigned long>(cch) * sizeof(wchar_t)));
            allocated[cch] = 0;
    ...
    */
    inline HRESULT stream_write_string_nothrow(_In_ ISequentialStream *target,
                                               _In_reads_opt_(writeLength) const wchar_t *source,
                                               _In_ size_t writeLength)
    {
        ...
        {
            RETURN_IF_FAILED(
                stream_write_nothrow(target, source, static_cast<unsigned short>(writeLength) * sizeof(wchar_t)));
        }
    ...
    */
    inline unsigned long stream_read_partial(_In_ ISequentialStream *stream,
                                             _Out_writes_bytes_to_(size, return) void *data, unsigned long size)
    {
        unsigned long didRead;
    ...
    */
    ...
    @param size The size, in bytes, of the buffer pointed to by `data`
    */
    inline void stream_write(_In_ ISequentialStream *stream, _In_reads_bytes_(size) const void *data,
                             unsigned long size)
    {
    ...
    */
    inline wil::unique_cotaskmem_string stream_read_string(
        _In_ ISequentialStream *source, empty_string_options options = empty_string_options::returns_empty)
    {
    ...
    */
    inline void stream_write_string(_In_ ISequentialStream *target, _In_reads_opt_(toWriteCch) const wchar_t *source,
                                    _In_ size_t toWriteCch)
    {
...
#pragma endregion // stream helpers
    ...
    using unique_set_site_null_call =
        wil::unique_com_call<IObjectWithSite, decltype(details::SetSiteNull), details::SetSiteNull>;
    ...
    Note, this does not preserve the existing site if there is one (an uncommon case) so only use this when that is not
    required.
    ~~~
    ...
    */
    ...
    template <typename TLambda> void for_each_site(_In_opt_ IUnknown *siteInput, TLambda &&callback)
    {
    ...
    namespace details
    {
        template <typename> struct com_enumerator_next_traits;
        ...
        template <typename T> struct has_next
        {
            ...
            template <typename> static auto test(...) -> wistd::false_type;
        ...
        template <typename T> struct You_must_specify_Smart_Output_type_explicitly
        {
            ...
            // If that type is a COM pointer type (IFoo*), then we use wil::com_ptr<IFoo>. Otherwise, you must
            // explicitly specify a smart-object type to receive the enumerated objects as it is not obvious how to
            // handle disposing of an enumerated object. For example, if you have an enumerator that enumerates BSTRs,
            // you must specify wil::unique_bstr as the smart pointer type to receive the enumerated BSTRs. auto it =
            // wil::com_iterator<wil::unique_bstr>(pEnumBStr);
            static_assert(wistd::is_same_v<T, void>,
                          "Couldn't deduce a smart pointer type for the enumerator's output. You must explicitly "
                          "specify a smart-object type to receive the enumerated objects.");
        };

        template <typename Interface> struct com_enumerator_traits
        {
            ...
            using smart_result =
                wistd::conditional_t<wistd::is_pointer_v<Result> &&
                                         wistd::is_base_of_v<::IUnknown, wistd::remove_pointer_t<Result>>,
    ...
    template <typename TStoredType, typename IEnumType> struct com_iterator
    {
        using TActualStoredType =
            wistd::conditional_t<wistd::is_same_v<TStoredType, void>,
                                 typename wil::details::com_enumerator_traits<IEnumType>::smart_result, TStoredType>;
      ...
      private:
        void FetchNext()
        {
            if (m_enum)
            {
                // we cannot say m_currentValue = {} because com_ptr has 2 operator= overloads: one for T* and one for
                // nullptr_t
    ...
    template <typename IEnumType> com_iterator(IEnumType *) -> com_iterator<void, IEnumType>;

    template <typename TStoredType = void, typename IEnumXxx,
              wistd::enable_if_t<wil::details::has_next_v<IEnumXxx *>, int> = 0>
    WI_NODISCARD auto make_range(IEnumXxx *enumPtr)
    {
        using TActualStoredType =
            wistd::conditional_t<wistd::is_same_v<TStoredType, void>,
                                 typename wil::details::com_enumerator_traits<IEnumXxx>::smart_result, TStoredType>;

        struct iterator_range
        {

            static_assert(!wistd::is_same_v<TActualStoredType, void>,
                          "You must specify a type to receive the enumerated objects.");

            // the stored type must be constructible from the output type of the enumerator
            static_assert(wistd::is_constructible_v<TActualStoredType,
                                                    typename wil::details::com_enumerator_traits<IEnumXxx>::Result>,
    ...
    ~~~
    */
    template <typename err_policy> class com_timeout_t
    {
      public:
      ...
      private:
        ...
        wil::unique_call<decltype(&details::CoDisableCallCancellationNull), details::CoDisableCallCancellationNull,
                         false>
            m_ensureDisable{};
        DWORD m_threadId{};
        bool m_timedOut{};
