...
namespace wil
{
    namespace reg
    {
        ...
        // Access rights for opening registry keys. See
        ...
        namespace reg_view_details
        {
            ...
            /**
             * @brief A utility function that walks a contiguous wchar_t container looking for strings within a
             * multi-string
             * @tparam InputIt An iterator type that reference a container that holds wchar_t characters to translate
             * into individual strings
             * @tparam Fn A callback function to be called each time a string is found - given the [begin, end]
             * iterators referencing the found string
             * @param first An iterator referencing to the beginning of the target container (like a std::begin
             * iterator)
             * @param last An iterator referencing one-past-the-end of the target container (like a std::end iterator)
             * @param func A callback function to be called each time a string is found - given the [begin, end]
             * iterators referencing the found string
             */
            template <class InputIt, class Fn> void walk_multistring(const InputIt &first, const InputIt &last, Fn func)
            {
                auto current = first;
            ...
            /**
             * @brief A translation function taking iterators referencing std::wstring objects and returns a
             * corresponding std::vector<wchar_t> to be written to a MULTI_SZ registry value. The translation follows
             * the rules for how MULTI_SZ registry values should be formatted, notably how null characters should be
             * embedded within the returned vector
             * @tparam InputIt An iterator type that references a container that holds std::wstring objects to translate
             * into a wchar_t buffer
             * @param first An iterator referencing to the beginning of the target container (like a std::begin
             * iterator)
             * @param last An iterator referencing one-past-the-end of the target container (like a std::end iterator)
             * @return A std::vector<wchar_t> with the raw wchar_t buffer of bytes prepared to write to a MULTI_SZ
             * registry value
             */
            ...
            /**
             * @brief A translation function taking iterators referencing wchar_t characters and returns extracted
             * individual std::wstring objects. The translation follows the rules for how MULTI_SZ registry value can be
             * formatted, notably with embedded null characters. Note that this conversion avoids returning empty
             * std::wstring objects even though the input may contain contiguous null wchar_t values
             * @tparam InputIt An iterator type that reference a container that holds wchar_t characters to translate
             * into individual strings
             * @param first An iterator referencing to the beginning of the target container (like a std::begin
             * iterator)
             * @param last An iterator referencing one-past-the-end of the target container (like a std::end iterator)
             * @return A std::vector<std::wstring> of the extracted strings from the input container of wchar_t
             * characters
             */
            template <class InputIt>
            ...
            template <size_t C>
            void get_multistring_bytearray_from_strings_nothrow(
                const PCWSTR data[C], ::wil::unique_cotaskmem_array_ptr<BYTE> &multistring) WI_NOEXCEPT
            {
            ...
            /**
             * @brief A translation function taking iterators referencing wchar_t characters and returns extracted
             * individual wil::unique_cotaskmem_string objects. The translation follows the rules for how MULTI_SZ
             * registry value can be formatted, notably with embedded null characters. Note that this conversion avoids
             * returning empty wil::unique_cotaskmem_string objects even though the input may contain contiguous null
             * wchar_t values
             * @tparam InputIt An iterator type that reference a container that holds wchar_t characters to translate
             * into individual strings
             * @param first An iterator referencing to the beginning of the target container (like a std::begin
             * iterator)
             * @param last An iterator referencing one-past-the-end of the target container (like a std::end iterator)
             * @param cotaskmem_array The [out] wil::unique_cotaskmem_array_ptr<wil::unique_cotaskmem_string> to contain
             * the array of strings. A wil::unique_cotaskmem_array_ptr<wil::unique_cotaskmem_string> of the extracted
             * strings from the input container of wchar_t characters. An empty wil::unique_cotaskmem_array_ptr should
             * be translated as out-of memory as there should always be at least one wil::unique_cotaskmem_string
             */
            template <class InputIt>
            void get_cotaskmemstring_array_from_multistring_nothrow(
                const InputIt &first, const InputIt &last,
                ::wil::unique_cotaskmem_array_ptr<::wil::unique_cotaskmem_string> &cotaskmem_array) WI_NOEXCEPT
            {
                if (last - first < 3)
                {
                    // it doesn't have the required 2 terminating null characters - return an empty string
                ...
                size_t arraySize = 0;
                walk_multistring(first, last, [&](const InputIt &, const InputIt &) { ++arraySize; });
            ...
            {
                // supports_prepare_buffer is used to determine if the input buffer to read a registry value should be
                // prepared before the first call to the registry read API
                template <typename T> constexpr bool supports_prepare_buffer() WI_NOEXCEPT
                {
                    return false;
                }
                template <typename T> HRESULT prepare_buffer(T &) WI_NOEXCEPT
                {
                    // no-op in the default case
                    return S_OK;
                }

                // supports_resize_buffer_bytes is used to determine if the input buffer to read a registry value can be
                // resized for those cases if the error from the registry read API indicates it needs a larger buffer
                template <typename T> constexpr bool supports_resize_buffer_bytes() WI_NOEXCEPT
                {
                    return false;
                }
                template <typename T> constexpr HRESULT resize_buffer_bytes(T &, DWORD) WI_NOEXCEPT
                {
                    return E_NOTIMPL;
                }

                // supports_trim_buffer is used to determine if the input buffer to read a registry value must be
                // trimmed after the registry read API has successfully written into the supplied buffer note that
                // currently only std::wstring requires this as it cannot have embedded nulls
                template <typename T> constexpr bool supports_trim_buffer() WI_NOEXCEPT
                {
                    return false;
                }
                // if called for a type that does not support trimming, will return a zero-length
                template <typename T> constexpr size_t trim_buffer(T &) WI_NOEXCEPT
                {
                    return 0;
                }
                ...
                template <> constexpr bool supports_prepare_buffer<::std::vector<uint8_t>>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_resize_buffer_bytes<::std::vector<uint8_t>>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_prepare_buffer<::std::vector<wchar_t>>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_prepare_buffer<::std::wstring>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_resize_buffer_bytes<::std::wstring>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_trim_buffer<::std::wstring>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_prepare_buffer<BSTR>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_resize_buffer_bytes<BSTR>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                inline HRESULT resize_buffer_bytes(BSTR &string, DWORD byteSize,
                                                   bool transferringOwnership = true) WI_NOEXCEPT
                {
                    ...
                    const bool original_string_length_too_small =
                        (original_string_length + 1) < byteSize / sizeof(WCHAR);
                    ...
                    const DWORD sourceLengthToCopy =
                        original_string_length < length_to_alloc ? original_string_length : length_to_alloc;
                    if (sourceLengthToCopy > 0 && original_string_length_too_small)
                    {
                        ::memcpy_s(new_bstr, length_to_alloc * sizeof(WCHAR), original_string,
                                   sourceLengthToCopy * sizeof(WCHAR));
                    }
                ...
                template <> constexpr bool supports_prepare_buffer<::wil::unique_bstr>() WI_NOEXCEPT
                {
                    return true;
                }
                inline HRESULT prepare_buffer(const ::wil::unique_bstr &value) WI_NOEXCEPT
                {
                    ...
                    {
                        // zero out the buffer if pre-allocated
                        for (auto &string_char :
                             ::wil::make_range(value.get(), get_buffer_size_bytes(value) / sizeof(WCHAR)))
                        {
                            string_char = L'\0';
                        }
                    }
                ...
                template <> constexpr bool supports_resize_buffer_bytes<::wil::unique_bstr>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_prepare_buffer<::wil::shared_bstr>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_resize_buffer_bytes<::wil::shared_bstr>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                template <> constexpr bool supports_resize_buffer_bytes<::wil::unique_cotaskmem_string>() WI_NOEXCEPT
                {
                    return true;
                }
                inline HRESULT resize_buffer_bytes(::wil::unique_cotaskmem_string &string, DWORD byteSize) WI_NOEXCEPT
                {
                    ...
                    // ::wil::make_unique_string_nothrow adds one to the length when it allocates, so subtracting 1 from
                    // the input length
                    length = length > 0 ? length - 1 : length;
                    auto new_string =
                        ::wil::make_unique_string_nothrow<::wil::unique_cotaskmem_string>(string.get(), length);
                ...
                inline HRESULT resize_buffer_bytes(::wil::unique_cotaskmem_array_ptr<uint8_t> &arrayValue,
                                                   DWORD byteSize) WI_NOEXCEPT
                {
                ...
                template <> constexpr bool supports_resize_buffer_bytes<::wil::shared_cotaskmem_string>() WI_NOEXCEPT
                {
                    return true;
                }
                ...
                inline HRESULT resize_buffer_bytes(::wil::unique_process_heap_string &string,
                                                   DWORD byteSize) WI_NOEXCEPT
                {
                    // convert bytes to length (number of WCHAR's)
                    size_t length = byteSize / sizeof(wchar_t);
                    ...
                    length = length > 0 ? length - 1 : length;
                    auto new_string =
                        ::wil::make_unique_string_nothrow<::wil::unique_process_heap_string>(string.get(), length);
                    RETURN_IF_NULL_ALLOC(new_string.get());
                ...
                template <typename T> DWORD get_value_type() WI_NOEXCEPT
                {
                    static_assert(sizeof(T) != sizeof(T), "Unsupported type for get_value_type");
                }

                template <typename T> DWORD set_value_type() WI_NOEXCEPT
                {
                    static_assert(sizeof(T) != sizeof(T), "Unsupported type for set_value_type");
                }

                template <> constexpr DWORD get_value_type<int32_t>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_DWORD);
                }
                template <> constexpr DWORD set_value_type<int32_t>() WI_NOEXCEPT
                {
                    return REG_DWORD;
                }

                template <> constexpr DWORD get_value_type<uint32_t>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_DWORD);
                }
                template <> constexpr DWORD set_value_type<uint32_t>() WI_NOEXCEPT
                {
                    return REG_DWORD;
                }

                template <> constexpr DWORD get_value_type<long>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_DWORD);
                }
                template <> constexpr DWORD set_value_type<long>() WI_NOEXCEPT
                {
                    return REG_DWORD;
                }

                template <> constexpr DWORD get_value_type<unsigned long>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_DWORD);
                }
                template <> constexpr DWORD set_value_type<unsigned long>() WI_NOEXCEPT
                {
                    return REG_DWORD;
                }

                template <> constexpr DWORD get_value_type<int64_t>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_QWORD);
                }
                template <> constexpr DWORD set_value_type<int64_t>() WI_NOEXCEPT
                {
                    return REG_QWORD;
                }

                template <> constexpr DWORD get_value_type<uint64_t>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_QWORD);
                }
                template <> constexpr DWORD set_value_type<uint64_t>() WI_NOEXCEPT
                {
                    return REG_QWORD;
                }

                template <> constexpr DWORD get_value_type<PCWSTR>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }
                template <> constexpr DWORD set_value_type<PCWSTR>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }

#if (WIL_USE_STL && defined(WIL_ENABLE_EXCEPTIONS)) || defined(WIL_DOXYGEN)
                template <> constexpr DWORD get_value_type<::std::wstring>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }

                template <> constexpr DWORD set_value_type<const ::std::wstring>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }
#endif
...
#if defined(__WIL_OLEAUTO_H_)
                template <> constexpr DWORD get_value_type<BSTR>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }
                template <> constexpr DWORD get_value_type<::wil::unique_bstr>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }

                template <> constexpr DWORD set_value_type<const BSTR>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }

                template <> constexpr DWORD set_value_type<const ::wil::unique_bstr>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }
#endif // #if defined(__WIL_OLEAUTO_H_)
                ...
                template <> constexpr DWORD get_value_type<::wil::shared_bstr>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }

                template <> constexpr DWORD set_value_type<const ::wil::shared_bstr>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }
#endif // #if defined(__WIL_OLEAUTO_H_STL)
...
#if defined(__WIL_OBJBASE_H_)
                template <> constexpr DWORD get_value_type<::wil::unique_cotaskmem_string>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }

                template <> constexpr DWORD set_value_type<const ::wil::unique_cotaskmem_string>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }
...
#if defined(__WIL_OBJBASE_H_STL)
                template <> constexpr DWORD get_value_type<::wil::shared_cotaskmem_string>() WI_NOEXCEPT
                {
                    return get_value_flags_from_value_type(REG_SZ);
                }

                template <> constexpr DWORD set_value_type<const ::wil::shared_cotaskmem_string>() WI_NOEXCEPT
                {
                    return REG_SZ;
                }
#endif // #if defined(__WIL_OBJBASE_H_STL)
            ...
            template <typename err_policy = ::wil::err_exception_policy> class reg_view_t
            {
              public:
                ...
                typename err_policy::result open_key(_In_opt_ _In_opt_ PCWSTR subKey, _Out_ HKEY *hkey,
                                                     ::wil::reg::key_access access = ::wil::reg::key_access::read) const
                {
                    constexpr DWORD zero_options{0};
                    return err_policy::HResult(HRESULT_FROM_WIN32(
                        ::RegOpenKeyExW(m_key, subKey, zero_options, get_access_flags(access), hkey)));
                }

                typename err_policy::result create_key(
                    PCWSTR subKey, _Out_ HKEY *hkey, ::wil::reg::key_access access = ::wil::reg::key_access::read) const
                {
                    *hkey = nullptr;
                    ...
                    DWORD disposition{0};
                    return err_policy::HResult(HRESULT_FROM_WIN32(
                        ::RegCreateKeyExW(m_key, subKey, zero_reserved, null_class, zero_options,
                                          get_access_flags(access), null_security_attributes, hkey, &disposition)));
                }
                ...
                template <typename R>
                typename err_policy::result get_value(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
                                                      R &return_value,
                                                      DWORD type = reg_value_type_info::get_value_type<R>()) const
                {
                    return get_value_with_type(subkey, value_name, return_value, type);
                }

                // typename D supports unsigned 32-bit values; i.e. allows the caller to pass a DWORD* as well as
                // uint32_t*
                ...
                typename err_policy::result get_value_char_array(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
                                                                 WCHAR (&return_value)[Length], DWORD type,
                                                                 _Out_opt_ DwordType *requiredBytes) const
                {
                    ...
                    const auto hr = HRESULT_FROM_WIN32(::RegGetValueW(
                        m_key, subkey, value_name, ::wil::reg::reg_view_details::get_value_flags_from_value_type(type),
                        nullptr, return_value, &data_size_bytes));
                ...
                template <typename R>
                ::std::optional<R> try_get_value(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
                                                 DWORD type = reg_value_type_info::get_value_type<R>()) const
                {
                    R value{};
                    const auto hr =
                        get_value_with_type<R, ::wil::err_returncode_policy>(subkey, value_name, value, type);
                    if (SUCCEEDED(hr))
                    {
                ...
                template <typename R>
                typename err_policy::result set_value(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
                                                      const R &value,
                                                      DWORD type = reg_value_type_info::set_value_type<R>()) const
                {
              ...
              private:
                ...
                template <typename R>
                typename err_policy::result set_value_with_type(_In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name,
                                                                const R &value, DWORD type) const
                {
                    return err_policy::HResult(HRESULT_FROM_WIN32(::RegSetKeyValueW(
                        m_key, subkey, value_name, type, static_cast<uint8_t *>(reg_value_type_info::get_buffer(value)),
                ...
                typename get_value_with_type_policy::result get_value_with_type(
                    _In_opt_ PCWSTR subkey, _In_opt_ PCWSTR value_name, R &return_value,
                    DWORD type = reg_value_type_info::get_value_type<R>()) const
                {
                    if
                    ...
                    for (;;)
                    {
                        ...
                        get_value_hresult = HRESULT_FROM_WIN32(
                            ::RegGetValueW(m_key, subkey, value_name, get_value_flags_from_value_type(type), null_type,
                                           reg_value_type_info::get_buffer(return_value), &data_size_bytes));
                            ...
                            (reg_value_type_info::supports_resize_buffer_bytes<R>())
                        {
                            ...
                            // GetRegValueW will indicate the caller allocate the returned number of bytes in one of two
                            // cases:
                            ...
                            const bool shouldReallocate =
                                ...
                                (SUCCEEDED(get_value_hresult) &&
                                 (reg_value_type_info::get_buffer(return_value) == nullptr) && (data_size_bytes > 0));
                            if (shouldReallocate)
                            {
                                // verify if resize_buffer succeeded allocation
                                const auto resize_buffer_hr =
                                    reg_value_type_info::resize_buffer_bytes(return_value, data_size_bytes);
                    ...
                    // some types (generally string types) require trimming its internal buffer after RegGetValueW
                    // successfully wrote into its buffer
                    if
#if defined(__cpp_if_constexpr)
                        constexpr
#endif
        ...
        namespace reg_iterator_details
        {
            ...
            // function overloads to allow *_enumerator objects to be constructed from all 3 types of HKEY
            // representatives
            ...
            template <typename T> PWSTR address_of_name(const T &name) WI_NOEXCEPT
            {
                return static_cast<PWSTR>(::wil::reg::reg_view_details::reg_value_type_info::get_buffer(name));
            }

            template <typename T> bool can_derive_length(const T &name) WI_NOEXCEPT
            {
                return static_cast<bool>(address_of_name(name));
            }

            template <typename T> bool compare_name(const T &name, PCWSTR comparand) WI_NOEXCEPT
            {
            ...
            template <typename T> void clear_name(const T &name, size_t length) WI_NOEXCEPT
            {
            ...
            template <typename T> size_t resize_name_cch(T &name, size_t current_length, size_t new_length) WI_NOEXCEPT
            {
                ...
                // continue to use the existing buffer since the requested length is less than or equals to the current
                // length
                clear_name(name, current_length);
                return current_length;
            }

            template <typename T> T copy_name(const T &name, size_t length) WI_NOEXCEPT
            {
        ...
        template <typename T> class key_iterator_data
        {
          ...
          private:
            ...
            HRESULT enumerate_current_index() WI_NOEXCEPT
            {
                ...
                auto string_length =
                    static_cast<DWORD>(m_name_length) > 0
                ...
                for (;;)
                {
                    ...
                    const auto error = ::RegEnumKeyExW(
                        m_hkey,  // hKey
                        m_index, // dwIndex
                        string_length == 0 ? nullptr
                                           : ::wil::reg::reg_iterator_details::address_of_name(name), // lpName
        ...
        // all methods must be noexcept - to be usable with any iterator type (throwing or non-throwing)
        template <typename T> class value_iterator_data
        {
          public:
            T name{};
            DWORD type = REG_NONE;
            ...
            HRESULT enumerate_current_index() WI_NOEXCEPT
            {
                ...
                for (;;)
                {
                    ...
                    const auto error = ::RegEnumValueW(
                        ...
                        m_index, // dwIndex
                        string_length == 0 ? nullptr
                                           : ::wil::reg::reg_iterator_details::address_of_name(name), // lpValueName
