...
//*********************************************************
//! @file
//! WIL Error Handling Helpers: a family of macros and functions designed to uniformly handle and report errors across
//! return codes, fail fast, exceptions and logging.
#ifndef __WIL_RESULT_INCLUDED
#define __WIL_RESULT_INCLUDED

// Most functionality is picked up from result_macros.h.  This file specifically provides higher level processing of
// errors when they are encountered by the underlying macros.
...
// The updated behavior of running init-list ctors during placement new is proper & correct, disable the warning that
// requests developers verify they want it
#pragma warning(push)
...
#define __WI_SEMAHPORE_VERSION L"_p0"
        ...
        class SemaphoreValue
        {
          public:
                                                                 ...
                                                                 m_semaphoreHigh(wistd::move(other.m_semaphoreHigh))
            {
                static_assert(sizeof(m_semaphore) == sizeof(HANDLE),
                              "unique_any must be a direct representation of the HANDLE to be used across module");
            }
            ...
            template <typename T> HRESULT CreateFromValue(PCWSTR name, T value)
            {
                return CreateFromValueInternal(name, (sizeof(value) > sizeof(unsigned long)),
          ...
          private:
            HRESULT CreateFromValueInternal(PCWSTR name, bool is64Bit, unsigned __int64 value)
            {
                ...
                // We set the count of the semaphore equal to the max (the value we're storing).  The only exception to
                // that is ZERO, where you can't create a semaphore of value ZERO, where we push the max to one and use
                // a count of ZERO.

                __WIL_PRIVATE_RETURN_IF_FAILED(m_semaphore.create(
                    static_cast<LONG>(lowPart), static_cast<LONG>((lowPart > 0) ? lowPart : 1), localName));
                if (is64Bit)
                {
                    WI_VERIFY_SUCCEEDED(StringCchCatW(localName, ARRAYSIZE(localName), L"h"));
                    __WIL_PRIVATE_RETURN_IF_FAILED(m_semaphoreHigh.create(
                        static_cast<LONG>(highPart), static_cast<LONG>((highPart > 0) ? highPart : 1), localName));
                }
            ...
            static HRESULT GetValueFromSemaphore(HANDLE semaphore, _Out_ LONG *count)
            {
                ...
                if (result == WAIT_OBJECT_0)
                {
                    ...
                    __WIL_PRIVATE_RETURN_HR_IF(E_UNEXPECTED,
            ...
            static HRESULT TryGetValueInternal(PCWSTR name, bool is64Bit, _Out_ unsigned __int64 *value,
                                               _Out_opt_ bool *retrieved)
            {
                ...
                *value = 0;
                ...
                {
                    WI_VERIFY_SUCCEEDED(StringCchCatW(localName, ARRAYSIZE(localName), L"h"));
                    wil::unique_semaphore_nothrow semaphoreHigh(
                        ::OpenSemaphoreW(SEMAPHORE_ALL_ACCESS, FALSE, localName));
        ...
        template <typename T> class ProcessLocalStorageData
        {
          public:
            ...
            {
                static_assert(sizeof(m_mutex) == sizeof(HANDLE),
                              "unique_any must be equivalent to the handle size to safely use across module");
            }
            ...
            static HRESULT Acquire(PCSTR staticNameWithVersion,
                                   _Outptr_result_nullonfailure_ ProcessLocalStorageData<T> **data)
            {
                *data = nullptr;
                ...
                WI_VERIFY(SUCCEEDED(StringCchPrintfW(name, ARRAYSIZE(name), L"Local\\SM0:%lu:%lu:%hs",
                                                     ::GetCurrentProcessId(), size, staticNameWithVersion)));
                ...
                else
                {
                    __WIL_PRIVATE_RETURN_IF_FAILED(MakeAndInitialize(
                        name, wistd::move(mutex),
                        data)); // Assumes mutex handle ownership on success ('lock' will still be released)
                }
          ...
          private:
            ...
            static HRESULT MakeAndInitialize(PCWSTR name, unique_mutex_nothrow &&mutex,
                                             _Outptr_result_nullonfailure_ ProcessLocalStorageData<T> **data)
            {
                *data = nullptr;
        ...
        template <typename T> class ProcessLocalStorage
        {
          public:
...
#pragma warning(pop)
                    ...
                    if (auto pNewRaw = details::ProcessHeapAlloc(0, sizeof(Node)))
                    {
                        ...
                        do
                        {
                            ...
                            pNew->pNext = pFirst;
                                     ...
                                     reinterpret_cast<PVOID volatile *>(m_hashArray + index), pNew, pFirst) != pFirst);
        ...
        struct ThreadLocalData
        {
            ...
            WI_NODISCARD bool GetLastError(_Inout_ wil::FailureInfo &info, unsigned int minSequenceId,
                                           HRESULT matchRequirement) const
            {
                if (!errors)
                {
                    return false;
            ...
            bool GetCaughtExceptionError(_Inout_ wil::FailureInfo &info, unsigned int minSequenceId,
                                         _In_opt_ const DiagnosticsInfo *diagnostics, HRESULT matchRequirement,
                                         void *returnAddress)
            {
                ...
                // NOTE:  FailureType::Log as it's only informative (no action) and SupportedExceptions::All as it's not
                // a barrier, only recognition.
                wchar_t message[2048]{};
                const HRESULT hr = details::ReportFailure_CaughtExceptionCommon<FailureType::Log>(
                                       __R_DIAGNOSTICS_RA(source, returnAddress), message, ARRAYSIZE(message),
                                       SupportedExceptions::All)
                                       .hr;
    ...
    /** Returns a sequence token that can be used with wil::GetLastError to limit errors to those that occur after this
    token was retrieved. General usage pattern:  use wil::GetCurrentErrorSequenceId to cache a token, execute your code,
    on failure use wil::GetLastError with the token to provide information on the error that occurred while executing
    your code.  Prefer to use wil::ThreadErrorContext over this approach when possible.  */
    ...
    Most people will never need to do this explicitly as failure information is automatically made available per-thread
    across a process when errors are encountered naturally through the WIL macros. */
    ...
    restricts the error returned to one that occurred after the given sequence ID.  Passing matchRequirement also
    filters the returned result to the given error code. */
    inline bool GetLastError(_Inout_ wil::FailureInfo &info, unsigned int minSequenceId = 0,
                             HRESULT matchRequirement = S_OK)
    {
        auto data = details_abi::GetThreadLocalData(false); // false = avoids allocation if not already present
        if (data)
        {
    ...
    inline __declspec(noinline) bool GetCaughtExceptionError(_Inout_ wil::FailureInfo &info,
                                                             unsigned int minSequenceId = 0,
                                                             const DiagnosticsInfo *diagnostics = nullptr,
                                                             HRESULT matchRequirement = S_OK)
    {
        auto data = details_abi::GetThreadLocalData();
        if (data)
    ...
    class ThreadErrorContext
    {
      public:
                                                                 ...
                                                                 HRESULT matchRequirement = S_OK)
        {
            if (m_data)
            {
                return m_data->GetCaughtExceptionError(info, m_sequenceIdStart, diagnostics, matchRequirement,
                                                       _ReturnAddress());
            }
            return false;
        }
    ...
    namespace details
    {
        ...
        __declspec(selectany) details_abi::ThreadLocalStorage<ThreadFailureCallbackHolder *>
            *g_pThreadFailureCallbacks = nullptr;

        class ThreadFailureCallbackHolder
        {
          public:
            ThreadFailureCallbackHolder(_In_opt_ IFailureCallback *pCallbackParam,
                                        _In_opt_ CallContextInfo *pCallContext = nullptr,
                                        bool watchNow = true) WI_NOEXCEPT : m_ppThreadList(nullptr),
            ...
            void StopWatching()
            {
                ...
                {
                    // The thread-specific failure holder cannot be stopped on a different thread than it was started on
                    // or the internal book-keeping list will be corrupted.  To fix this change the telemetry pattern in
                    // the calling code to match one of the patterns available here:
                    //    https://microsoft.sharepoint.com/teams/osg_development/Shared%20Documents/Windows%20TraceLogging%20Helpers.docx

                    WI_USAGE_ERROR("MEMORY CORRUPTION: Calling code is leaking an activity thread-watcher and "
            ...
            static bool GetThreadContext(_Inout_ FailureInfo *pFailure, _In_opt_ ThreadFailureCallbackHolder *pCallback,
                                         ...
                                         _Pre_satisfies_(callContextStringLength > 0) size_t callContextStringLength)
            {
                *callContextString = '\0';
                ...
                {
                    foundContext =
                        GetThreadContext(pFailure, pCallback->m_pNext, callContextString, callContextStringLength);
            ...
            static void GetContextAndNotifyFailure(_Inout_ FailureInfo *pFailure,
                                                   _Out_writes_(callContextStringLength)
                                                       _Post_z_ PSTR callContextString,
                                                   _Pre_satisfies_(callContextStringLength > 0)
                                                       size_t callContextStringLength) WI_NOEXCEPT
            {
                *callContextString = '\0';
                bool reportedTelemetry = false;

                ThreadFailureCallbackHolder **ppListeners =
                    g_pThreadFailureCallbacks ? g_pThreadFailureCallbacks->GetLocal() : nullptr;
                ...
                if (g_pfnTelemetryCallback != nullptr)
                {
                    ...
                    g_pfnTelemetryCallback(reportedTelemetry ||
                                               WI_IsFlagSet(pFailure->flags, FailureFlags::RequestSuppressTelemetry),
                                           *pFailure);
                }
            }
        ...
        template <typename TLambda> class ThreadFailureCallbackFn final : public IFailureCallback
        {
          public:
            explicit ThreadFailureCallbackFn(_In_opt_ CallContextInfo *pContext,
                                             _Inout_ TLambda &&errorFunction) WI_NOEXCEPT
        ...
        inline void __stdcall GetContextAndNotifyFailure(_Inout_ FailureInfo *pFailure,
                                                         _Out_writes_(callContextStringLength)
                                                             _Post_z_ PSTR callContextString,
                                                         _Pre_satisfies_(callContextStringLength > 0)
                                                             size_t callContextStringLength) WI_NOEXCEPT
        {
            ThreadFailureCallbackHolder::GetContextAndNotifyFailure(pFailure, callContextString,
    ...
    */
    ...
    namespace details
    {
        ...
        __declspec(selectany) ::wil::details_abi::ProcessLocalStorage<::wil::details_abi::ProcessLocalData>
            g_processLocalData("WilError_03");
        __declspec(selectany) ::wil::details_abi::ThreadLocalStorage<ThreadFailureCallbackHolder *>
    ...
    // Much like ThreadFailureCallback, this class will receive WIL failure notifications from the time it's
    // instantiated until the time that it's destroyed.  At any point during that time you can ask for the last failure
    // that was seen by any of the WIL macros (RETURN_XXX, THROW_XXX, LOG_XXX, etc) on the current thread.
    //
    ...
    //                 // like (std::bad_alloc from the STL).  Use a reasonable default like:
    ...
    class ThreadFailureCache final : public details::IFailureCallback
    {
      public:
        ...
        ThreadFailureCache(ThreadFailureCache &&rhs) WI_NOEXCEPT : m_failure(wistd::move(rhs.m_failure)),
