...
#pragma region HSTRING Helpers
    ...
    namespace details
    {
        ...
        template <bool InhibitStringArrays, bool IgnoreCase> struct hstring_compare
        {
            ...
            static const wchar_t *get_buffer(const Microsoft::WRL::Wrappers::HStringReference &hstr,
            ...
            static wistd::enable_if_t<Enable, const wchar_t *> get_buffer(const wchar_t *str,
                                                                          UINT32 *length) WI_NOEXCEPT
            {
            ...
            static wistd::enable_if_t<
                wistd::conjunction<wistd::is_pointer<StringT>,
                                   wistd::is_same<wistd::decay_t<wistd::remove_pointer_t<StringT>>, wchar_t>,
                                   wistd::bool_constant<Enable>>::value,
                const wchar_t *>
            ...
            static wistd::enable_if_t<Enable, const wchar_t *> get_buffer(const wchar_t (&str)[Size],
                                                                          UINT32 *length) WI_NOEXCEPT
            {
                *length = Size - 1;
                return str;
            ...
            template <size_t Size, bool..., bool Enable = !InhibitStringArrays>
            static wistd::enable_if_t<Enable, const wchar_t *> get_buffer(wchar_t (&str)[Size],
                                                                          UINT32 *length) WI_NOEXCEPT
            {
                *length = static_cast<UINT32>(wcslen(str));
                return str;
            ...
            static auto compare(LhsT &&lhs, RhsT &&rhs) -> decltype(get_buffer(lhs, wistd::declval<UINT32 *>()),
                                                                    get_buffer(rhs, wistd::declval<UINT32 *>()), int())
            {
                UINT32 lhsLength;
                UINT32 rhsLength;
                ...
                const auto result =
                    ::CompareStringOrdinal(lhsBuffer, lhsLength, rhsBuffer, rhsLength, IgnoreCase ? TRUE : FALSE);
                WI_ASSERT(result != 0);
    ...
    */
    struct TwoPhaseHStringConstructor
    {
        ...
        //! Returns the HSTRING after it has been populated like Detach() or release(); be sure to put this in a RAII
        //! type to manage its lifetime.
        HSTRING Promote()
        {
            m_characterLength = 0;
        ...
        */
        WI_NODISCARD HRESULT Validate(ULONG bytesRead) const
        {
            ...
            RETURN_HR_IF(HRESULT_FROM_WIN32(ERROR_INVALID_DATA),
                         (bytesRead != ByteSize()) || (Get()[m_characterLength] != L'\0'));
            return S_OK;
        }
...
#pragma warning(pop)
        ...
        template <typename T>
        struct MapToSmartType<T, typename wistd::enable_if<
                                     wistd::is_base_of<IUnknown, typename wistd::remove_pointer<T>::type>::value>::type>
        {
        ...
        template <typename T, typename P> struct MapAsyncOpProgressResultType
        {
            template <typename TAsyncOperation, typename TResult>
            static TResult PeekGetResultsType(HRESULT (STDMETHODCALLTYPE TAsyncOperation::*)(TResult *));
            typedef decltype(PeekGetResultsType(
                &ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P>::GetResults)) type;
        };
...
@code
ComPtr<CollectionType> collection = GetCollection(); // can be IVector<HSTRING>, IVectorView<HSTRING> or
IIterable<HSTRING>
...
#pragma region exception and fail fast based IVector<>/IVectorView<>
    ...
    template <typename VectorType, typename err_policy = err_exception_policy> class vector_range
    {
      public:
...
#pragma region error code based IVector<>/IVectorView<>
    ...
    template <typename VectorType> class vector_range_nothrow
    {
      public:
        typedef typename details::MapVectorResultType<VectorType>::type TResult;
        typedef typename details::MapToSmartType<TResult>::type TSmart;
        ...
        class vector_iterator_nothrow
        {
          public:
            ...
            {
#if WIL_ITERATOR_DEBUG_LEVEL > 0
                WI_FAIL_FAST_ASSERT_MSG(m_version == m_range->m_version,
                                        "Dereferencing an out-of-date vector_iterator_nothrow");
                WI_FAIL_FAST_ASSERT_MSG(SUCCEEDED(*m_range->m_result),
                                        "Dereferencing a vector_iterator_nothrow in a failed state");
            ...
            vector_iterator_nothrow &operator+=(int n)
            {
                ...
                // This is _technically_ safe because we are not reading the out-of-date cached value, however having
                // two active copies of iterators is generally a sign of problematic code with a bug waiting to happen
                WI_FAIL_FAST_ASSERT_MSG(m_version == m_range->m_version,
                                        "Incrementing/decrementing an out-of-date copy of a vector_iterator_nothrow");
#endif
                m_i += n;
#if WIL_ITERATOR_DEBUG_LEVEL > 0
                // NOTE: 'm_i' is unsigned, hence the single check for increment/decrement
                WI_FAIL_FAST_ASSERT_MSG(m_i <= m_range->m_size,
                                        "Incrementing/decrementing a vector_iterator_nothrow out of range");
#endif
        ...
        {
            // NOTE: Calling 'begin()' more than once is explicitly allowed as a way to reset the range, however because
            // state is shared between all iterators, this invalidates previously created iterators
            get_at_current(0);
      ...
      private:
        ...
        HRESULT *m_result;
        HRESULT m_resultStorage =
            S_OK; // for the case where the caller does not provide the location to store the result
...
#pragma region exception and fail fast based IIterable<>
    ...
    template <typename T, typename err_policy = err_exception_policy> class iterable_range
    {
      public:
    ...
    */
    template <typename VectorType> auto to_vector(VectorType *src)
    {
        ...
        if (expected > 0)
        {
            ...
            UINT32 fetched = 0;
            THROW_IF_FAILED(src->GetMany(0, static_cast<UINT32>(output.size()),
                                         reinterpret_cast<TResult *>(output.data()), &fetched));
...
#pragma region error code base IIterable<>
    ...
    template <typename T> class iterable_range_nothrow
    {
      public:
        typedef typename details::MapIteratorResultType<T>::type TResult;
        typedef typename details::MapToSmartType<TResult>::type TSmart;
        ...
        iterable_range_nothrow(_In_ ABI::Windows::Foundation::Collections::IIterable<T> *iterable,
        ...
        class iterable_iterator_nothrow
        {
          public:
...
#if WIL_ITERATOR_DEBUG_LEVEL > 0
                WI_FAIL_FAST_ASSERT_MSG(SUCCEEDED(*m_range->m_result),
                                        "Dereferencing an iterable_iterator_nothrow in a failed state");
                WI_FAIL_FAST_ASSERT_MSG(m_i >= 0, "Dereferencing an 'end' iterator");
                WI_FAIL_FAST_ASSERT_MSG(m_i == m_range->m_index,
                                        "Dereferencing an out-of-date iterable_iterator_nothrow");
#endif
            ...
            iterable_iterator_nothrow &operator++()
            {
                ...
                WI_FAIL_FAST_ASSERT_MSG(m_i == m_range->m_index,
                                        "Incrementing an out-of-date copy of an iterable_iterator_nothrow");
#endif
                boolean hasCurrent;
        ...
        {
#if WIL_ITERATOR_DEBUG_LEVEL == 2
            // The IIterator we hold only advances forward; we can't reset it back to the beginning. Calling this method
            // more than once will very likely lead to unexpected behavior.
...
#pragma endregion
    ...
    template <typename T>
    vector_range<ABI::Windows::Foundation::Collections::IVector<T>> get_range(
    ...
    template <typename T>
    vector_range<ABI::Windows::Foundation::Collections::IVectorView<T>> get_range(
    ...
    template <typename T>
    iterable_range_nothrow<T> get_range_nothrow(ABI::Windows::Foundation::Collections::IIterable<T> *v,
                                                HRESULT *result = nullptr)
    {
...
#pragma region WinRT Async API helpers
    ...
    namespace details
    {
        ...
        template <typename TFunc, typename... Args> HRESULT CallAndHandleErrors(TFunc &&func, Args &&...args)
        {
            return CallAndHandleErrorsWithReturnType<decltype(wistd::forward<TFunc>(func)(
                wistd::forward<Args>(args)...))>(wistd::forward<TFunc>(func), wistd::forward<Args>(args)...);
        }
        ...
        template <typename... Ts> struct LastType
        {
            ...
            template <typename... OtherTs> static typename LastTypeOfTs<OtherTs...>::type LastTypeOfTsFunc()
            {
            }
        ...
        typename wistd::remove_pointer<typename LastType<P...>::type>::type GetReturnParamPointerType(
            HRESULT (STDMETHODCALLTYPE I::*)(P...));
        ...
        template <typename T>
        typename wil::details::MapAsyncOpResultType<T>::type GetAsyncResultType(
        ...
        ABI::Windows::Foundation::IAsyncActionCompletedHandler *GetAsyncDelegateType(
        ...
        template <typename T>
        ABI::Windows::Foundation::IAsyncOperationCompletedHandler<T> *GetAsyncDelegateType(
            ABI::Windows::Foundation::IAsyncOperation<T> *);
        template <typename T, typename P>
        ...
        HRESULT RunWhenCompleteAction(_In_ TIOperation operation, TFunction &&func) WI_NOEXCEPT
        {
            ...
            auto callback = Callback<Implements<RuntimeClassFlags<ClassicCom>, TIDelegate, TBaseAgility>>(
                [func = wistd::forward<TFunction>(func)](
                    TIOperation operation, ABI::Windows::Foundation::AsyncStatus status) mutable -> HRESULT {
                    HRESULT hr = S_OK;
                    if (status !=
                        ABI::Windows::Foundation::AsyncStatus::Completed) // avoid a potentially costly marshaled QI /
                                                                          // call if we completed successfully
                    {
        ...
        HRESULT RunWhenComplete(_In_ TIOperation operation, TFunction &&func) WI_NOEXCEPT
        {
            ...
            auto callback = Callback<Implements<RuntimeClassFlags<ClassicCom>, TIDelegate, TBaseAgility>>(
                [func = wistd::forward<TFunction>(func)](
                    TIOperation operation, ABI::Windows::Foundation::AsyncStatus status) mutable -> HRESULT {
                    typename details::MapToSmartType<typename GetAbiType<
                        typename wistd::remove_pointer<TIOperation>::type::TResult_complex>::type>::type result;
        ...
        HRESULT WaitForCompletion(_In_ TIOperation operation, COWAIT_FLAGS flags, DWORD timeoutValue,
                                  _Out_opt_ bool *timedOut) WI_NOEXCEPT
        {
            typedef wistd::remove_pointer_t<decltype(GetAsyncDelegateType(operation))> TIDelegate;

            class CompletionDelegate
                : public Microsoft::WRL::RuntimeClass<
                      Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::RuntimeClassType::Delegate>, TIDelegate,
                      Microsoft::WRL::FtmBase>
            {
              public:
                ...
                HRESULT STDMETHODCALLTYPE Invoke(_In_ TIOperation,
              ...
              private:
                volatile ABI::Windows::Foundation::AsyncStatus m_status =
            ...
            // If the caller is listening for timedOut, and we actually timed out, set the bool and return S_OK.
            // Otherwise, fail.
    ...
    */
    ...
    //! Run a function when an async operation completes. Use Microsoft::WRL::FtmBase for TAgility to make the
    //! completion handler agile and run on the async thread.
    ...
    template <typename TAgility = IUnknown, typename TResult, typename TFunc,
              typename TAsyncResult = typename wil::details::MapAsyncOpResultType<TResult>::type>
    HRESULT run_when_complete_nothrow(_In_ ABI::Windows::Foundation::IAsyncOperation<TResult> *operation,
                                      TFunc &&func) WI_NOEXCEPT
    {
    ...
    template <typename TAgility = IUnknown, typename TResult, typename TProgress, typename TFunc,
              typename TAsyncResult = typename wil::details::MapAsyncOpProgressResultType<TResult, TProgress>::type>
    HRESULT run_when_complete_nothrow(
    ...
    HRESULT run_when_complete_nothrow(_In_ ABI::Windows::Foundation::IAsyncActionWithProgress<TProgress> *operation,
                                      TFunc &&func) WI_NOEXCEPT
    {
        return details::RunWhenCompleteAction<TAgility>(operation, wistd::forward<TFunc>(func));
    }
    ...
    template <typename TAgility = IUnknown, typename TResult, typename TProgress, typename TFunc,
              typename TAsyncResult = typename wil::details::MapAsyncOpProgressResultType<TResult, TProgress>::type>
    void run_when_complete(_In_ ABI::Windows::Foundation::IAsyncOperationWithProgress<TResult, TProgress> *operation,
                           TFunc &&func)
    {
        THROW_IF_FAILED((details::RunWhenComplete<TAgility>(operation, wistd::forward<TFunc>(func))));
    }
    ...
    */
    ...
    inline HRESULT wait_for_completion_nothrow(_In_ TAsync *operation,
    ...
    template <typename TAsync = ABI::Windows::Foundation::IAsyncAction>
    inline HRESULT wait_for_completion_or_timeout_nothrow(_In_ TAsync *operation, DWORD timeoutValue,
                                                          _Out_ bool *timedOut,
                                                          COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT
    {
    ...
    HRESULT wait_for_completion_or_timeout_nothrow(_In_ ABI::Windows::Foundation::IAsyncOperation<TResult> *operation,
                                                   _Out_
                                                   typename wil::details::MapAsyncOpResultType<TResult>::type *result,
                                                   DWORD timeoutValue, _Out_ bool *timedOut,
                                                   COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT
    {
    ...
    HRESULT wait_for_completion_or_timeout_nothrow(
        _In_ ABI::Windows::Foundation::IAsyncOperationWithProgress<TResult, TProgress> *operation,
        _Out_ typename wil::details::MapAsyncOpProgressResultType<TResult, TProgress>::type *result, DWORD timeoutValue,
        _Out_ bool *timedOut, COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS) WI_NOEXCEPT
    {
        return details::WaitForCompletion(operation, result, flags, timeoutValue, timedOut);
    }
    ...
    template <typename TResult, typename TReturn = typename wil::details::MapToSmartType<
                                    typename wil::details::MapAsyncOpResultType<TResult>::type>::type>
    TReturn wait_for_completion(_In_ ABI::Windows::Foundation::IAsyncOperation<TResult> *operation,
    ...
    template <typename TResult, typename TProgress,
              typename TReturn = typename wil::details::MapToSmartType<
                  typename wil::details::MapAsyncOpProgressResultType<TResult, TProgress>::type>::type>
    TReturn wait_for_completion(
        _In_ ABI::Windows::Foundation::IAsyncOperationWithProgress<TResult, TProgress> *operation,
        COWAIT_FLAGS flags = COWAIT_DISPATCH_CALLS)
    {
        TReturn result;
    ...
    */
    ...
    {
        Microsoft::WRL::ComPtr<typename wistd::remove_pointer<
            typename wistd::remove_pointer<typename details::LastType<P...>::type>::type>::type>
            op;
...
#pragma region WinRT object construction
    ...
    template <typename TInterface> com_ptr<TInterface> GetActivationFactory(PCWSTR runtimeClass)
    {
        ...
        THROW_IF_FAILED(RoGetActivationFactory(Microsoft::WRL::Wrappers::HStringReference(runtimeClass).Get(),
                                               IID_PPV_ARGS(&result)));
        return result;
    }
...
#pragma region Async production helpers
    ...
    namespace details
    {
                  ...
                  Microsoft::WRL::AsyncBase<ABI::Windows::Foundation::IAsyncOperationCompletedHandler<TResult>>>
        {
          ...
          public:
            ...
            IFACEMETHODIMP put_Completed(
                ABI::Windows::Foundation::IAsyncOperationCompletedHandler<TResult> *competed) override
            {
        ...
        class SyncAsyncActionOp WrlFinal
            : public Microsoft::WRL::RuntimeClass<
                  ABI::Windows::Foundation::IAsyncAction,
                  Microsoft::WRL::AsyncBase<ABI::Windows::Foundation::IAsyncActionCompletedHandler,
                                            Microsoft::WRL::Details::Nil, Microsoft::WRL::AsyncResultType::SingleResult
    ...
    //! Creates a WinRT async operation object that implements IAsyncOperation<TResult>. Use mostly for testing and for
    //! mocking APIs.
    template <typename TResult>
    HRESULT make_synchronous_async_operation_nothrow(ABI::Windows::Foundation::IAsyncOperation<TResult> **result,
                                                     const TResult &value)
    {
        return Microsoft::WRL::MakeAndInitialize<details::SyncAsyncOp<TResult>>(result, value);
    }

    //! Creates a WinRT async operation object that implements IAsyncAction. Use mostly for testing and for mocking
    //! APIs.
    ...
    template <typename TResult, typename TRealResult = typename details::MapAsyncOpResultType<TResult>::type,
              typename TSmartResult = typename details::MapToSmartType<TRealResult>::type>
    void make_synchronous_async_operation(ABI::Windows::Foundation::IAsyncOperation<TResult> **result,
                                          const TResult &value)
    {
...
#pragma region EventRegistrationToken RAII wrapper
    ...
    // unique_winrt_event_token[_cx] is an RAII wrapper around EventRegistrationToken. When the
    // unique_winrt_event_token[_cx] is destroyed, the event is automatically unregistered. Declare a
    // wil::unique_winrt_event_token[_cx]<T> at the scope the event should be registered for (often they are tied to
    // object lifetime), where T is the type of the event sender
    //     wil::unique_winrt_event_token_cx<Windows::UI::Xaml::Controls::Button> m_token;
    //
    // Macros have been defined to register for handling the event and then returning an unique_winrt_event_token[_cx].
    // These macros simply hide the function references for adding and removing the event.
    //     C++/CX  m_token = WI_MakeUniqueWinRtEventTokenCx(ExampleEventName, sender, handler);
    //     ABI     m_token = WI_MakeUniqueWinRtEventToken(ExampleEventName, sender, handler, &m_token); // Exception and
    //     failfast ABI     RETURN_IF_FAILED(WI_MakeUniqueWinRtEventTokenNoThrow(ExampleEventName, sender, handler,
    //     &m_token));  // No throw variant
    //
...
private:
    ...
    template <typename T> class unique_winrt_event_token
    {
        ...
        addition_func additionFunc, removal_func removalFunc, handler^ h)
    {
       ...
       // clang-format on
        ...
        // Handles registration of the event handler to the subscribing object and then wraps the EventRegistrationToken
        // in unique_winrt_event_token. Not intended to be directly called. Use the WI_MakeUniqueWinRtEventToken macro
        // to abstract away specifying the functions that handle addition and removal.
        template <typename err_policy = wil::err_returncode_policy, typename T, typename addition_func,
                  typename removal_func, typename handler>
        inline auto make_unique_winrt_event_token(T *sender, addition_func additionFunc, removal_func removalFunc,
                                                  handler h, wil::unique_winrt_event_token<T> *token_reference)
        {
        ...
        template <typename err_policy = wil::err_returncode_policy, typename T, typename addition_func,
                  typename removal_func, typename handler>
        inline typename wistd::enable_if<!wistd::is_same<err_policy, wil::err_returncode_policy>::value,
                                         wil::unique_winrt_event_token<T>>::type
...
#define WI_MakeUniqueWinRtEventTokenCx(_event, _object, _handler)                                                      \
    wil::details::make_unique_winrt_event_token_cx(                                                                    \
        _object, &wil::details::remove_reference<decltype(_object)>::type::##_event## ::add,                           \
        &wil::details::remove_reference<decltype(_object)>::type::##_event## ::remove, _handler)

#define WI_MakeUniqueWinRtStaticEventTokenCx(_event, _baseType, _handler)                                              \
    wil::details::make_unique_winrt_static_event_token_cx<_baseType>(&##_baseType## ::##_event## ::add,                \
                                                                     &##_baseType## ::##_event## ::remove, _handler)
...
#define WI_MakeUniqueWinRtEventToken(_event, _object, _handler)                                                        \
    wil::details::make_unique_winrt_event_token<wil::err_exception_policy>(                                            \
        _object, &wistd::remove_pointer<decltype(_object)>::type::add_##_event,                                        \
        &wistd::remove_pointer<decltype(_object)>::type::remove_##_event, _handler)
...
#define WI_MakeUniqueWinRtEventTokenNoThrow(_event, _object, _handler, _token_reference)                               \
    wil::details::make_unique_winrt_event_token(                                                                       \
        _object, &wistd::remove_pointer<decltype(_object)>::type::add_##_event,                                        \
        &wistd::remove_pointer<decltype(_object)>::type::remove_##_event, _handler, _token_reference)
...
struct ABI::Windows::Foundation::IAsyncOperationWithProgress<
    ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P> *, Z>
    : ABI::Windows::Foundation::IAsyncOperationWithProgress_impl<
          ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P> *, Z>
{
    static const wchar_t *z_get_rc_name_impl()
    {
...
template <typename P>
struct ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler<ABI::Windows::Foundation::IAsyncAction *,
                                                                             P>
    : ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler_impl<
          ABI::Windows::Foundation::IAsyncAction *, P>
{
    static const wchar_t *z_get_rc_name_impl()
    {
...
struct ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler<
    ABI::Windows::Foundation::IAsyncOperation<T> *, P>
    : ABI::Windows::Foundation::IAsyncOperationWithProgressCompletedHandler_impl<
          ABI::Windows::Foundation::IAsyncOperation<T> *, P>
{
    static const wchar_t *z_get_rc_name_impl()
...
template <typename T, typename P>
struct ABI::Windows::Foundation::IAsyncOperationCompletedHandler<
    ABI::Windows::Foundation::IAsyncOperationWithProgress<T, P> *>
...
namespace std
{
    ...
    template <> struct less<Microsoft::WRL::Wrappers::HString> : public wil::hstring_less
    {
    };

    //! Specialization of `std::less` for `wil::unique_hstring` that uses `hstring_less` for the comparison function
    //! object.
    template <> struct less<wil::unique_hstring> : public wil::hstring_less
    {
    };
} // namespace std
