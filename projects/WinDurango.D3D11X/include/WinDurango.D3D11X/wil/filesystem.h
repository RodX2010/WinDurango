...
namespace wil
{
    ...
    inline PCWSTR find_last_path_segment(_In_ PCWSTR path)
    {
        ...
        auto const limitedLength =
            ((pathLength > 0) && (path[pathLength - 1] == L'\\')) ? (pathLength - 1) : pathLength;
    ...
    //! Returns the drive number, if it has one. Returns true if there is a drive number, false otherwise. Supports
    //! regular and extended length paths.
    ...
    HRESULT GetFullPathNameW(PCWSTR file, string_type &path, _Outptr_opt_ PCWSTR *filePart = nullptr)
    {
        ...
        const auto hr = AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
            ...
            [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
                _Out_ size_t *valueLengthNeededWithNull) -> HRESULT {
    ...
    inline wil::unique_hfile TryCreateFileCanRecurseIntoDirectory(PCWSTR path, PWIN32_FIND_DATAW fileFindData,
                                                                  DWORD access = GENERIC_READ | /*DELETE*/ 0x00010000L,
                                                                  DWORD share = FILE_SHARE_READ)
    {
        wil::unique_hfile result(CreateFileW(path, access, share, nullptr, OPEN_EXISTING,
    ...
    inline HRESULT RemoveDirectoryRecursiveNoThrow(PCWSTR inputPath,
                                                   RemoveDirectoryOptions options = RemoveDirectoryOptions::None,
                                                   HANDLE deleteHandle = INVALID_HANDLE_VALUE) WI_NOEXCEPT
    {
        ...
        for (;;)
        {
            ...
            if (!(WI_IsFlagSet(fd.dwFileAttributes, FILE_ATTRIBUTE_DIRECTORY) && path_is_dot_or_dotdot(fd.cFileName)))
            {
                ...
                {
                    // Get a handle to the directory to delete, preventing it from being replaced to prevent writes
                    // which could be used to bypass permission checks, and verify that it is not a name surrogate (e.g.
                    // symlink, mount point, etc).
                    wil::unique_hfile recursivelyDeletableDirectoryHandle =
                        TryCreateFileCanRecurseIntoDirectory(pathToDelete.get(), &fd);
                    if (recursivelyDeletableDirectoryHandle)
                    {
                        ...
                        RETURN_IF_FAILED(RemoveDirectoryRecursiveNoThrow(
                            pathToDelete.get(), WI_ClearFlag(localOptions, RemoveDirectoryOptions::KeepRootDirectory),
                            recursivelyDeletableDirectoryHandle.get()));
                    }
                    else if (WI_IsFlagSet(fd.dwFileAttributes, FILE_ATTRIBUTE_REPARSE_POINT))
                    {
                        // This is a directory reparse point that should not be recursed. Delete it without traversing
                        // into it.
                ...
                else
                {
                    ...
                    if (!::DeleteFileW(pathToDelete.get()))
                    {
                        ...
                        bool potentiallyFixableReadOnlyProblem =
                            WI_IsFlagSet(options, RemoveDirectoryOptions::RemoveReadOnly) &&
        ...
        if (WI_IsFlagClear(options, RemoveDirectoryOptions::KeepRootDirectory))
        {
            if (deleteHandle != INVALID_HANDLE_VALUE)
            {
                ...
                // DeleteFile and RemoveDirectory use POSIX delete, falling back to non-POSIX on most errors. Do the
                // same here.
                ...
                {
                    auto const err = ::GetLastError();
                    // The real error we're looking for is STATUS_CANNOT_DELETE, but that's mapped to
                    // ERROR_ACCESS_DENIED.
                    if (err != ERROR_ACCESS_DENIED)
                    {
                        ...
                        RETURN_IF_WIN32_BOOL_FALSE(
                            SetFileInformationByHandle(deleteHandle, FileDispositionInfo, &fileInfo, sizeof(fileInfo)));
#if (NTDDI_VERSION >= NTDDI_WIN10_RS1)
                    }
                    else
                    {
    ...
    template <typename T> struct next_entry_offset_iterator
    {
    ...
    template <typename T> next_entry_offset_iterator<T> create_next_entry_offset_iterator(T *p)
    {
...
#pragma region Folder Watcher
    ...
    enum class FolderChangeEvents : DWORD
    {
        ...
        All = FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME | FILE_NOTIFY_CHANGE_ATTRIBUTES |
    ...
    namespace details
    {
        ...
        typedef resource_policy<folder_watcher_state *, decltype(&details::delete_folder_watcher_state),
                                details::delete_folder_watcher_state, details::pointer_access_none>
            folder_watcher_state_resource_policy;
    } // namespace details
    /// @endcond

    template <typename storage_t, typename err_policy = err_exception_policy> class folder_watcher_t : public storage_t
    {
      public:
        ...
        folder_watcher_t(PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
        ...
        result create(PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
                      wistd::function<void()> &&callback)
        {
      ...
      private:
        ...
        // to __stdcall
        static void __stdcall callback(PTP_CALLBACK_INSTANCE /*Instance*/, void *context, TP_WAIT *pThreadPoolWait,
        ...
        HRESULT create_common(PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
                              wistd::function<void()> &&callback)
        {
            wistd::unique_ptr<details::folder_watcher_state> watcherState(
                new (std::nothrow) details::folder_watcher_state(wistd::move(callback)));
            RETURN_IF_NULL_ALLOC(watcherState);

            watcherState->m_findChangeHandle.reset(
                FindFirstChangeNotificationW(folderToWatch, isRecursive, static_cast<DWORD>(filter)));
            RETURN_LAST_ERROR_IF(!watcherState->m_findChangeHandle);

            watcherState->m_threadPoolWait.reset(
                CreateThreadpoolWait(&folder_watcher_t::callback, watcherState.get(), __nullptr));
    ...
    typedef unique_any_t<
        folder_watcher_t<details::unique_storage<details::folder_watcher_state_resource_policy>, err_returncode_policy>>
        unique_folder_watcher_nothrow;

    inline unique_folder_watcher_nothrow make_folder_watcher_nothrow(PCWSTR folderToWatch, bool isRecursive,
                                                                     FolderChangeEvents filter,
...
#ifdef WIL_ENABLE_EXCEPTIONS
    typedef unique_any_t<
        folder_watcher_t<details::unique_storage<details::folder_watcher_state_resource_policy>, err_exception_policy>>
        unique_folder_watcher;

    inline unique_folder_watcher make_folder_watcher(PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
                                                     wistd::function<void()> &&callback)
    {
...
#pragma region Folder Reader
    ...
    namespace details
    {
        ...
        {
            folder_change_reader_state(bool isRecursive, FolderChangeEvents filter,
            ...
            HRESULT StartIo()
            {
                ...
                HRESULT hr =
                    ReadDirectoryChangesW(m_folderHandle.get(), m_readBuffer, sizeof(m_readBuffer), m_isRecursive,
                                          static_cast<DWORD>(m_filter), __nullptr, &m_overlapped, __nullptr)
                        ? S_OK
            ...
            unsigned char m_readBuffer[4096]{}; // Consider alternative buffer sizes. With 512 byte buffer i was not
                                                // able to observe overflow.
        };
        ...
        typedef resource_policy<folder_change_reader_state *, decltype(&details::delete_folder_change_reader_state),
                                details::delete_folder_change_reader_state, details::pointer_access_none>
    ...
    class folder_change_reader_t : public storage_t
    {
      public:
        ...
        // Exception-based constructors
        folder_change_reader_t(PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
                               wistd::function<void(FolderChangeEvent, PCWSTR)> &&callback)
        {
            static_assert(wistd::is_same<void, result>::value,
                          "this constructor requires exceptions; use the create method");
      ...
      private:
                                       ...
                                       ULONG result, ULONG_PTR /* BytesTransferred */, TP_IO * /* Io */)
        {
            ...
            {
                for (auto const &info : create_next_entry_offset_iterator(
                         reinterpret_cast<FILE_NOTIFY_INFORMATION *>(readerState->m_readBuffer)))
                {
                    wchar_t relativeFileName[MAX_PATH];
                    StringCchCopyNW(relativeFileName, ARRAYSIZE(relativeFileName), info.FileName,
                              ...
                              wistd::function<void(FolderChangeEvent, PCWSTR)> &&callback)
        {
            ...
            readerState->m_folderHandle.reset(
                CreateFileW(folderToWatch, FILE_LIST_DIRECTORY, FILE_SHARE_READ | FILE_SHARE_DELETE | FILE_SHARE_WRITE,
            ...
            readerState->m_tpIo = CreateThreadpoolIo(readerState->m_folderHandle.get(),
                                                     &folder_change_reader_t::callback, readerState.get(), __nullptr);
    ...
    typedef unique_any_t<folder_change_reader_t<
        details::unique_storage<details::folder_change_reader_state_resource_policy>, err_returncode_policy>>
        unique_folder_change_reader_nothrow;

    inline unique_folder_change_reader_nothrow make_folder_change_reader_nothrow(
        PCWSTR folderToWatch, bool isRecursive, FolderChangeEvents filter,
...
#ifdef WIL_ENABLE_EXCEPTIONS
    typedef unique_any_t<folder_change_reader_t<
        details::unique_storage<details::folder_change_reader_state_resource_policy>, err_exception_policy>>
        unique_folder_change_reader;
...
#pragma endregion
    ...
    enum class VolumePrefix
    {
        ...
        VolumeGuid =
            VOLUME_NAME_GUID, // \\?\Volume{588fb606-b95b-4eae-b3cb-1e49861aaf18}\Users\Chris\AppData\Local\Temp\wil8C31.tmp
        ...
        NtObjectName = VOLUME_NAME_NT, // Unique name used by Object Manager, e.g.
                                       // \Device\HarddiskVolume4\Users\Chris\AppData\Local\Temp\wil8C31.tmp
    };
        ...
        return AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
            ...
            [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
                ...
                *valueLengthNeededWithNull =
                    ::GetFinalPathNameByHandleW(fileHandle, value, static_cast<DWORD>(valueLength),
    ...
    template <typename string_type = wil::unique_cotaskmem_string, size_t stackBufferLength = 256>
    string_type GetFinalPathNameByHandleW(HANDLE fileHandle, wil::VolumePrefix volumePrefix = wil::VolumePrefix::Dos,
                                          wil::PathOptions options = wil::PathOptions::Normalized)
    {
        string_type result{};
        THROW_IF_FAILED(
            (GetFinalPathNameByHandleW<string_type, stackBufferLength>(fileHandle, result, volumePrefix, options)));
        return result;
    }
#endif
    ...
    //! Return a path in an allocated buffer for handling long paths.
    template <typename string_type, size_t stackBufferLength = 256> HRESULT GetCurrentDirectoryW(string_type &path)
    {
        return AdaptFixedSizeToAllocatedResult<string_type, stackBufferLength>(
            path,
            [&](_Out_writes_(valueLength) PWSTR value, size_t valueLength,
...
#define MAP_INFOCLASS_TO_STRUCT(InfoClass, InfoStruct, IsFixed, Extra)                                                 \
    ...
    {                                                                                                                  \
                                   ...
                                   _Outptr_result_maybenull_ void **result)
        {
            *result = nullptr;
            ...
            for (;;)
            {
                if (GetFileInformationByHandleEx(fileHandle, infoClass, resultHolder.get(),
    ...
    */
    template <FILE_INFO_BY_HANDLE_CLASS infoClass,
              typename wistd::enable_if<!details::MapInfoClassToInfoStruct<infoClass>::isFixed, int>::type = 0>
    HRESULT GetFileInfoNoThrow(HANDLE fileHandle,
        ...
        WI_NOEXCEPT
    {
        ...
        HRESULT hr = details::GetFileInfo(fileHandle, infoClass,
                                          sizeof(typename details::MapInfoClassToInfoStruct<infoClass>::type) +
                                              details::MapInfoClassToInfoStruct<infoClass>::extraSize,
                                          &rawResult);
    ...
    */
    template <FILE_INFO_BY_HANDLE_CLASS infoClass,
              typename wistd::enable_if<details::MapInfoClassToInfoStruct<infoClass>::isFixed, int>::type = 0>
    HRESULT GetFileInfoNoThrow(HANDLE fileHandle,
                               _Out_ typename details::MapInfoClassToInfoStruct<infoClass>::type *result) WI_NOEXCEPT
    {
    ...
    inline HRESULT CreateFileAndEnsureNotLinked(PCWSTR path, wil::unique_hfile &fileHandle)
    {
        ...
        fileHandle.reset(CreateFileW(path, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr, OPEN_EXISTING,
                                     FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, nullptr));
        ...
        wil::unique_hfile finalPathHandle(
            CreateFileW(path, 0, 0, nullptr, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, nullptr));
        ...
        // The low and high indices and volume serial number uniquely identify a file. These must match if they are the
        // same file.
        const bool isSoftLink = ((fileInfo.nFileIndexLow != finalFileInfo.nFileIndexLow) ||
    ...
    */
                                               ...
                                               bool inheritHandle = false) noexcept
    {
        ...
        auto handle = CreateFileW(path, dwDesiredAccess, dwShareMode, &secAttributes, OPEN_EXISTING,
    ...
    */
    inline wil::unique_hfile open_file(PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ,
                                       DWORD dwShareMode = FILE_SHARE_READ,
                                       DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL, bool inheritHandle = false)
    {
    ...
    namespace details
    {
        ...
        file_and_error_result create_file(PCWSTR path, DWORD dwDesiredAccess, DWORD dwShareMode,
                                          LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwFlagsAndAttributes,
                                          HANDLE hTemplateFile) noexcept
        {
            auto handle = CreateFileW(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes, dwCreateDisposition,
    ...
    ~~~
    */
    inline file_and_error_result try_create_new_file(PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ | GENERIC_WRITE,
                                                     DWORD dwShareMode = FILE_SHARE_READ,
                                                     LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr,
                                                     DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
                                                     HANDLE hTemplateFile = nullptr) noexcept
    {
        return details::create_file<CREATE_NEW>(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                                dwFlagsAndAttributes, hTemplateFile);
    }
    ...
    */
    inline file_and_error_result try_open_or_create_file(PCWSTR path,
                                                         ...
                                                         HANDLE hTemplateFile = nullptr) noexcept
    {
        return details::create_file<OPEN_ALWAYS>(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                                 dwFlagsAndAttributes, hTemplateFile);
    }
    ...
    */
    inline file_and_error_result try_open_or_truncate_existing_file(
        PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ | GENERIC_WRITE, DWORD dwShareMode = FILE_SHARE_READ,
        LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr, DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
        HANDLE hTemplateFile = nullptr) noexcept
    {
        return details::create_file<CREATE_ALWAYS>(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                                   dwFlagsAndAttributes, hTemplateFile);
    }
    ...
    */
    inline file_and_error_result try_truncate_existing_file(
        PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ | GENERIC_WRITE | GENERIC_WRITE,
        DWORD dwShareMode = FILE_SHARE_READ, LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr,
        DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL, HANDLE hTemplateFile = nullptr) noexcept
    {
        return details::create_file<TRUNCATE_EXISTING>(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                                       dwFlagsAndAttributes, hTemplateFile);
    }
    ...
    ~~~
    */
    inline wil::unique_hfile create_new_file(PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ | GENERIC_WRITE,
                                             DWORD dwShareMode = FILE_SHARE_READ,
                                             LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr,
                                             DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
                                             HANDLE hTemplateFile = nullptr)
    {
        auto result = try_create_new_file(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                          dwFlagsAndAttributes, hTemplateFile);
        THROW_WIN32_IF(result.last_error, !result.file.is_valid());
        return wistd::move(result.file);
    }
    ...
    ~~~
    */
    inline wil::unique_hfile open_or_create_file(PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ | GENERIC_WRITE,
                                                 DWORD dwShareMode = FILE_SHARE_READ,
                                                 LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr,
                                                 DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
                                                 HANDLE hTemplateFile = nullptr)
    {
        auto result = try_open_or_create_file(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                              dwFlagsAndAttributes, hTemplateFile);
        THROW_WIN32_IF(result.last_error, !result.file.is_valid());
        return wistd::move(result.file);
    }
    ...
    */
    inline wil::unique_hfile open_or_truncate_existing_file(PCWSTR path,
                                                            ...
                                                            HANDLE hTemplateFile = nullptr)
    {
        auto result = try_open_or_truncate_existing_file(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                                         dwFlagsAndAttributes, hTemplateFile);
        THROW_WIN32_IF(result.last_error, !result.file.is_valid());
        return wistd::move(result.file);
    }
    ...
    ~~~
    */
    inline wil::unique_hfile truncate_existing_file(PCWSTR path, DWORD dwDesiredAccess = GENERIC_READ | GENERIC_WRITE,
                                                    DWORD dwShareMode = FILE_SHARE_READ,
                                                    LPSECURITY_ATTRIBUTES lpSecurityAttributes = nullptr,
                                                    DWORD dwFlagsAndAttributes = FILE_ATTRIBUTE_NORMAL,
                                                    HANDLE hTemplateFile = nullptr)
    {
        auto result = try_truncate_existing_file(path, dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                                 dwFlagsAndAttributes, hTemplateFile);
        THROW_WIN32_IF(result.last_error, !result.file.is_valid());
        return wistd::move(result.file);
    }
