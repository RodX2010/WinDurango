...
//*********************************************************
//! @file
//! WIL Resource Wrappers (RAII): Provides a family of smart pointer patterns and resource wrappers to enable customers
//! to consistently use RAII in all code.
...
namespace wil
{
    ...
    namespace details
    {
        typedef wistd::integral_constant<size_t, 0>
        ...
        using close_invoker = close_invoke_helper<
            wistd::is_pointer_v<close_fn_t> ? wistd::is_function_v<wistd::remove_pointer_t<close_fn_t>> : false,
        ...
        template <typename pointer_t,  // The handle type
                  ...
                  typename pointer_access_t =
                      pointer_access_all, // all, noaddress or none to control pointer method access
                  ...
                  typename pointer_invalid_t = wistd::nullptr_t> // nullptr_t if the invalid handle value is compatible
        ...
        template <typename Policy> class unique_storage
        {
          ...
          public:
            ...
            {
                static_assert(wistd::is_same<typename policy::pointer_invalid, wistd::nullptr_t>::value,
            ...
            {
                static_assert(!wistd::is_same<typename policy::pointer_access, pointer_access_none>::value,
            ...
            {
                static_assert(wistd::is_same<typename policy::pointer_access, pointer_access_all>::value,
        ...
        template <typename T> struct needs_destruction
        {
            template <typename U>
            static auto invoke(int)
                -> wistd::bool_constant<sizeof(U) >= 0>; // Always true, but SFINAE's if incomplete type
            template <typename U> static auto invoke(float) -> wistd::false_type;
            ...
            static constexpr const bool value =
                wistd::conjunction_v<decltype(invoke<wistd::remove_extent_t<T>>(0)),
                                     wistd::is_destructible<wistd::remove_extent_t<T>>,
                                     wistd::negation<wistd::is_trivially_destructible<wistd::remove_extent_t<T>>>>;
        };

        template <typename T> constexpr bool needs_destruction_v = needs_destruction<T>::value;
        ...
        template <typename T> struct ensure_trivially_destructible
        {
            ...
            static_assert(!needs_destruction_v<T>, "Resource type has a non-trivial destructor, but is used in a "
                                                   "context where its destructor will not be run");
#endif
            using type = T;
        };

        template <typename T> using ensure_trivially_destructible_t = typename ensure_trivially_destructible<T>::type;
    } // namespace details
    /// @endcond
    ...
    template <typename storage_t> class unique_any_t : public storage_t
    {
      public:
        ...
        // Note that the default constructor really shouldn't be needed (taken care of by the forwarding constructor
        // below), but the forwarding constructor causes an internal compiler error when the class is used in a C++
        // array.  Defining the default constructor independent of the forwarding constructor removes the compiler
        // limitation.
        ...
        unique_any_t &operator=(unique_any_t &&other) WI_NOEXCEPT
        {
            ...
            {
                // cast to base_storage to 'skip' calling the (optional) specialization class that provides
        ...
        {
            static_assert(wistd::is_same<typename policy::pointer_access, details::pointer_access_all>::value,
        ...
        // void reset(pointer_storage ptr = policy::invalid_value()) WI_NOEXCEPT
        // void reset(wistd::nullptr_t) WI_NOEXCEPT
        // pointer_storage release() WI_NOEXCEPT                                        // not exposed for some resource
        // types pointer_storage *addressof() WI_NOEXCEPT                                     // not exposed for some
        // resource types
    };

    template <typename policy> void swap(unique_any_t<policy> &left, unique_any_t<policy> &right) WI_NOEXCEPT
    {
        left.swap(right);
    }
    ...
    template <typename policy> bool operator==(const unique_any_t<policy> &left, wistd::nullptr_t) WI_NOEXCEPT
    {
        static_assert(wistd::is_same<typename unique_any_t<policy>::policy::pointer_invalid, wistd::nullptr_t>::value,
                      "the resource class does not use nullptr as an invalid value");
        return !left;
    }

    template <typename policy> bool operator==(wistd::nullptr_t, const unique_any_t<policy> &right) WI_NOEXCEPT
    {
        static_assert(wistd::is_same<typename unique_any_t<policy>::policy::pointer_invalid, wistd::nullptr_t>::value,
                      "the resource class does not use nullptr as an invalid value");
        return !right;
    ...
    // unique_any provides a template alias for easily building a unique_any_t from a unique_storage class with the
    // given template parameters for resource_policy.

    template <
        typename pointer,    // The handle type
        typename close_fn_t, // The handle close function type
        close_fn_t close_fn, //      * and function pointer
        typename pointer_access =
            details::pointer_access_all,    // all, noaddress or none to control pointer method access
        ...
        typename pointer_invalid =
            wistd::nullptr_t> // nullptr_t if the invalid handle value is compatible with nullptr, otherwise pointer
    using unique_any = unique_any_t<details::unique_storage<details::resource_policy<
        pointer, close_fn_t, close_fn, pointer_access, pointer_storage, invalid_t, invalid, pointer_invalid>>>;

    /// @cond
    namespace details
    {
        template <typename TLambda> class lambda_call
        {
          public:
            ...
            {
                static_assert(wistd::is_same<decltype(lambda()), void>::value,
                              "scope_exit lambdas must not have a return value");
                static_assert(!wistd::is_lvalue_reference<TLambda>::value &&
        ...
        template <typename TLambda> class lambda_call_log
        {
          public:
    ...
    template <typename TLambda> WI_NODISCARD inline auto scope_exit(TLambda &&lambda) WI_NOEXCEPT
    {
    ...
    WI_NODISCARD inline __declspec(noinline) auto scope_exit_log(const DiagnosticsInfo &diagnostics,
                                                                 TLambda &&lambda) WI_NOEXCEPT
    {
    ...
    // Forward declaration...
    template <typename T, typename err_policy> class com_ptr_t;

    //! Type traits class that identifies the inner type of any smart pointer.
    template <typename Ptr> struct smart_pointer_details
    {
        typedef typename Ptr::pointer pointer;
    };

    /// @cond
    template <typename T> struct smart_pointer_details<Microsoft::WRL::ComPtr<T>>
    {
        typedef T *pointer;
    };
    /// @endcond
    ...
    /// @cond
    // Generically detaches a raw pointer from any smart pointer
    template <typename T, typename err> WI_NODISCARD T *detach_from_smart_pointer(wil::com_ptr_t<T, err> &smartPtr)
    {
        return smartPtr.detach();
    }

    // Generically detaches a raw pointer from any smart pointer
    template <typename T> WI_NODISCARD T *detach_from_smart_pointer(Microsoft::WRL::ComPtr<T> &smartPtr)
    {
        return smartPtr.Detach();
    }
    ...
    namespace details
    {
        ...
        template <typename T, typename err> wistd::false_type use_reset(wil::com_ptr_t<T, err> *)
        {
            return wistd::false_type();
        }
        template <typename T> wistd::true_type use_reset(T *)
        {
    ...
    template <typename TSmartPointer, typename EnableResetForm = wistd::enable_if_t<
                                          decltype(details::use_reset(static_cast<TSmartPointer *>(nullptr)))::value>>
    ...
    // Generically attach a raw pointer to a compatible smart pointer.
    template <typename T, typename err> void attach_to_smart_pointer(wil::com_ptr_t<T, err> &smartPtr, T *rawPtr)
    {
        smartPtr.attach(rawPtr);
    }

    // Generically attach a raw pointer to a compatible smart pointer.
    template <typename T> void attach_to_smart_pointer(Microsoft::WRL::ComPtr<T> &smartPtr, T *rawPtr)
    {
        smartPtr.Attach(rawPtr);
    }
    /// @endcond
    ...
    namespace details
    {
        template <typename T> struct out_param_t
        {
        ...
        template <typename Tcast, typename T> struct out_param_ptr_t
        {
            typedef typename wil::smart_pointer_details<T>::pointer pointer;
            T &wrapper;
            pointer pRaw;
    ...
    template <typename T> details::out_param_t<T> out_param(T &p)
    {
    ...
    '&' operator. Use only when the smart pointer's &handle is not equal to the output type a function requires,
    necessitating a cast. Example: `wil::out_param_ptr<PSECURITY_DESCRIPTOR*>(securityDescriptor)` */
    template <typename Tcast, typename T> details::out_param_ptr_t<Tcast, T> out_param_ptr(T &p)
    {
    ...
    Unique_struct wraps a trivial struct using a custom clean up function and, optionally, custom initializer function.
    If no custom initialier function is defined in the template then ZeroMemory is used. Unique_struct is modeled off of
    std::unique_ptr. However, unique_struct inherits from the defined type instead of managing the struct through a
    private member variable.

    If the type you're wrapping is a system type, you can share the code by declaring it in this file (Resource.h).
    Submit pull requests to [GitHub](https://github.com/microsoft/wil/). Otherwise, if the type is local to your
    project, declare it locally.
    @tparam struct_t The struct you want to manage
    @tparam close_fn_t The type of the function to clean up the struct. Takes one parameter: a pointer of struct_t.
    Return values are ignored.
    @tparam close_fn The function of type close_fn_t. This is called in the destructor and reset functions.
    @tparam init_fn_t Optional:The type of the function to initialize the struct.  Takes one parameter: a pointer of
    struct_t. Return values are ignored.
    @tparam init_fn Optional:The function of type init_fn_t. This is called in the constructor, reset, and release
    functions. The default is ZeroMemory to initialize the struct.

    Defined using the default zero memory initializer
    ~~~
    typedef wil::unique_struct<PROPVARIANT, decltype(&::PropVariantClear), ::PropVariantClear>
    ...
    ~~~
    */
    template <typename struct_t, typename close_fn_t, close_fn_t close_fn, typename init_fn_t = wistd::nullptr_t,
              init_fn_t init_fn = wistd::nullptr_t()>
    class unique_struct : public struct_t
    {
        using closer = details::close_invoker<close_fn_t, close_fn, struct_t *>;

      public:
        //! Initializes the managed struct using the user-provided initialization function, or ZeroMemory if no function
        //! is specified
        unique_struct()
        {
        ...
        //! Resets this managed struct by calling the custom close function and takes ownership of the other managed
        //! struct Then resets the other managed struct by calling the custom close function
        ...
        //! Then initializes this managed struct using the user-provided initialization function, or ZeroMemory if no
        ...
        //! Resets this managed struct by calling the custom close function
        //! Then initializes this managed struct using the user-provided initialization function, or ZeroMemory if no
        //! function is specified. Returns address of the managed struct
    ...
    struct empty_deleter
    {
        ...
        {
            static_assert(!details::needs_destruction_v<T>, "Resource type has a non-trivial destructor, but is used "
                                                            "in a context where its destructor will not be run");
        }
    };

    /** unique_any_array_ptr is a RAII type for managing conformant arrays that need to be freed and have elements that
    may need to be freed. The intended use for this RAII type would be to capture out params from API like
    IPropertyValue::GetStringArray. This class also maintains the size of the array, so it can iterate over the members
    and deallocate them before it deallocates the base array pointer.

    If the type you're wrapping is a system type, you can share the code by declaring it in this file (Resource.h). Send
    pull requests to [GitHub](https://github.com/microsoft/wil/). Otherwise, if the type is local to your project,
    declare it locally.

    @tparam ValueType: The type of array you want to manage.
    @tparam ArrayDeleter: The type of the function to clean up the array. Takes one parameter of type T[] or T*. Return
    values are ignored. This is called in the destructor and reset functions.
    @tparam ElementDeleter: The type of the function to clean up the array elements. Takes one parameter of type T.
    Return values are ignored. This is called in the destructor and reset functions.
    ...
    class unique_any_array_ptr
    {
      public:
        ...
        template <typename TSize> struct size_address_ptr
        {
        ...
        template <typename T> size_address_ptr<T> size_address() WI_NOEXCEPT
        {
      ...
      private:
        ...
        template <typename T> void reset_array(const T &deleter)
        {
    ...
    namespace details
    {
        template <typename UniqueAnyType> struct unique_any_array_deleter
        {
            template <typename T> void operator()(_Pre_opt_valid_ _Frees_ptr_opt_ T *p) const
            {
                UniqueAnyType::policy::close_reset(p);
            }
        };

        template <typename close_fn_t, close_fn_t close_fn> struct unique_struct_array_deleter
        {
            template <typename T> void operator()(_Pre_opt_valid_ _Frees_ptr_opt_ T &p) const
            {
        ...
        template <typename storage_t> struct element_traits<unique_any_t<storage_t>>
        {
            typedef unique_any_array_deleter<unique_any_t<storage_t>> deleter;
            typedef typename unique_any_t<storage_t>::pointer type;
        };

        template <typename T, typename err_policy> struct element_traits<com_ptr_t<T, err_policy>>
        {
    ...
    using unique_array_ptr = unique_any_array_ptr<typename details::element_traits<T>::type, ArrayDeleter,
    ...
    */
    ...
    using unique_my_interface_token =
        wil::unique_com_token<IMyInterface, DWORD, decltype(MyInterfaceCloseFunction), MyInterfaceCloseFunction,
    0xFFFFFFFF>;
    ~~~ */
    template <typename interface_t, typename token_t, typename close_fn_t, close_fn_t close_fn,
              token_t invalid_token = token_t()>
    ...
    /** Use unique_com_call to define an RAII type that demands a particular parameter-less method be called on a COM
    interface. This allows implementing an RAII type that can call a Close() method (think IClosable) or a
    SetSite(nullptr) method (think IObjectWithSite) or some other method when a basic interface call is required as part
    of the RAII contract. see wil::com_set_site in wil/com.h for the IObjectWithSite support.
    ...
    ~~~ */
    template <typename interface_t, typename close_fn_t, close_fn_t close_fn> class unique_com_call
    {
      public:
    ...
    ~~~ */
    template <typename close_fn_t, close_fn_t close_fn, bool default_value = true> class unique_call
    {
      public:
        unique_call() = default;
    ...
    // Overloads in this file support any string that is implicitly convertible to a PCWSTR, HSTRING, and any
    // unique_any_t that points to any other supported type (this covers unique_hstring, unique_cotaskmem_string, and
    // similar). An overload for std::wstring is available in stl.h.
    inline PCWSTR str_raw_ptr(PCWSTR str)
    {
        return str;
    }

    template <typename T> PCWSTR str_raw_ptr(const unique_any_t<T> &ua)
    {
    ...
    namespace details
    {
        ...
        // Concatenate any number of strings together and store it in an automatically allocated string.  If a string is
        // present in the input buffer, it is overwritten.
        ...
        HRESULT str_build_nothrow(string_type &result, _In_reads_(strCount) PCWSTR *strList, size_t strCount)
        {
            ...
            for (auto &string : make_range(strList, strCount))
            {
                ...
                {
                    RETURN_IF_FAILED(
                        StringCchCopyExW(buffer, (bufferEnd - buffer), string, &buffer, nullptr, STRSAFE_IGNORE_NULLS));
                }
            }
        ...
        // NOTE: 'Strings' must all be PCWSTR, or convertible to PCWSTR, but C++ doesn't allow us to express that
        // cleanly
    ...
    // Concatenate any number of strings together and store it in an automatically allocated string.  If a string is
    // present in the input buffer, the remaining strings are appended to it.
    template <typename string_type, typename... strings>
    ...
    template <typename string_type, typename... arguments> string_type str_concat(arguments &&...args)
    {
        string_type result{};
    ...
    // Concatenate any number of strings together and store it in an automatically allocated string.
    template <typename string_type, typename... arguments> string_type str_concat_failfast(arguments &&...args)
    {
        string_type result{};
    ...
    namespace details
    {
        // Wraps StringCchPrintFExW and stores it in an automatically allocated string.  Takes a buffer followed by the
        // same format arguments that StringCchPrintfExW takes.
        template <typename string_type>
        HRESULT str_vprintf_nothrow(string_type &result, _Printf_format_string_ PCWSTR pszFormat, va_list &argsVL)
        {
            ...
            auto buffer = maker.buffer();
            RETURN_IF_FAILED(StringCchVPrintfExW(buffer, lengthRequiredWithoutNull + 1, nullptr, nullptr,
                                                 STRSAFE_NULL_ON_FAILURE, pszFormat, argsVL));
    ...
    // Wraps StringCchPrintFExW and stores it in an automatically allocated string.  Takes a buffer followed by the same
    // format arguments that StringCchPrintfExW takes.
    template <typename string_type> string_type str_printf(_Printf_format_string_ PCWSTR pszFormat, ...)
    {
        string_type result{};
        va_list argsVL;
        va_start(argsVL, pszFormat);
    ...
    // Wraps StringCchPrintFExW and stores it in an automatically allocated string.  Takes a buffer followed by the same
    // format arguments that StringCchPrintfExW takes.
    template <typename string_type> string_type str_printf_failfast(_Printf_format_string_ PCWSTR pszFormat, ...)
    {
        string_type result{};
        va_list argsVL;
        va_start(argsVL, pszFormat);
...
namespace std
{
    template <typename storage_t> struct hash<wil::unique_any_t<storage_t>>
    {
...
#if (defined(_MEMORY_) && defined(WIL_ENABLE_EXCEPTIONS) && !defined(WIL_RESOURCE_STL) &&                              \
     !defined(RESOURCE_SUPPRESS_STL)) ||                                                                               \
    defined(WIL_DOXYGEN)
/// @cond
...
namespace wil
{
    ...
    template <typename storage_t> class weak_any;
    ...
    namespace details
    {
        ...
        template <typename UniqueT> class shared_storage
        {
          protected:
          ...
          private:
            template <typename storage_t> friend class ::wil::weak_any;
    ...
    template <typename storage_t> class shared_any_t : public storage_t
    {
      public:
        typedef typename storage_t::policy policy;
        typedef typename policy::pointer_storage pointer_storage;
        ...
        // default and forwarding constructor: forwards default, all 'explicit' and multi-arg constructors to the base
        // class
        ...
        // void reset(pointer_storage ptr = policy::invalid_value()) WI_NOEXCEPT
        // void reset(wistd::nullptr_t) WI_NOEXCEPT
        // pointer_storage *addressof() WI_NOEXCEPT                                // (note: not exposed for opaque
        // resource types)
    };

    template <typename unique_t> void swap(shared_any_t<unique_t> &left, shared_any_t<unique_t> &right) WI_NOEXCEPT
    {
        left.swap(right);
    }
    ...
    template <typename unique_t> bool operator==(const shared_any_t<unique_t> &left, wistd::nullptr_t) WI_NOEXCEPT
    {
        static_assert(wistd::is_same<typename shared_any_t<unique_t>::policy::pointer_invalid, wistd::nullptr_t>::value,
                      "the resource class does not use nullptr as an invalid value");
        return !left;
    }

    template <typename unique_t> bool operator==(wistd::nullptr_t, const shared_any_t<unique_t> &right) WI_NOEXCEPT
    {
        static_assert(wistd::is_same<typename shared_any_t<unique_t>::policy::pointer_invalid, wistd::nullptr_t>::value,
                      "the resource class does not use nullptr as an invalid value");
        return !right;
    ...
    // This class provides weak_ptr<> support for shared_any<>, bringing the same weak reference counting and lock()
    // acquire semantics to shared_any.

    template <typename SharedT> class weak_any
    {
      public:
    ...
    template <typename shared_t> void swap(weak_any<shared_t> &left, weak_any<shared_t> &right) WI_NOEXCEPT
    {
        left.swap(right);
    }

    template <typename unique_t> using shared_any = shared_any_t<details::shared_storage<unique_t>>;

} // namespace wil
#endif

#if (defined(WIL_RESOURCE_STL) && (defined(_UNORDERED_SET_) || defined(_UNORDERED_MAP_)) &&                            \
     !defined(__WIL_RESOURCE_SHARED_HASH)) ||                                                                          \
    defined(WIL_DOXYGEN)
/// @cond
...
namespace std
{
    template <typename storage_t> struct hash<wil::shared_any_t<storage_t>>
    {
...
#define __WIL__NOTHROW_T_DEFINED
    /// @endcond
    /** Provides `std::make_unique()` semantics for resources allocated in a context that may not throw upon allocation
    failure. `wil::make_unique_nothrow()` is identical to `std::make_unique()` except for the following:
    ...
    Note that `wil::make_unique_nothrow()` is not marked WI_NOEXCEPT as it may be used to create an exception-based
    class that may throw in its constructor.
    ~~~
    ...
    */
    ...
    inline typename wistd::enable_if<!wistd::is_array<_Ty>::value, wistd::unique_ptr<_Ty>>::type make_unique_nothrow(
        _Types &&..._Args)
    {
        return (wistd::unique_ptr<_Ty>(new (std::nothrow) _Ty(wistd::forward<_Types>(_Args)...)));
    }

    /** Provides `std::make_unique()` semantics for array resources allocated in a context that may not throw upon
    allocation failure. See the overload of `wil::make_unique_nothrow()` for non-array types for more details.
    ~~~
    ...
    */
    template <class _Ty>
    inline typename wistd::enable_if<wistd::is_array<_Ty>::value && wistd::extent<_Ty>::value == 0,
                                     wistd::unique_ptr<_Ty>>::type
    make_unique_nothrow(size_t _Size)
    {
...
#if !defined(__WIL_MIN_KERNEL) && !defined(WIL_KERNEL_MODE)
    /** Provides `std::make_unique()` semantics for resources allocated in a context that must fail fast upon allocation
    ...
    */
    template <class _Ty, class... _Types>
    inline typename wistd::enable_if<!wistd::is_array<_Ty>::value, wistd::unique_ptr<_Ty>>::type make_unique_failfast(
        _Types &&..._Args)
    {
#pragma warning(suppress : 28193) // temporary must be inspected (it is within the called function)
        return (
            wistd::unique_ptr<_Ty>(FAIL_FAST_IF_NULL_ALLOC(new (std::nothrow) _Ty(wistd::forward<_Types>(_Args)...))));
    }

    /** Provides `std::make_unique()` semantics for array resources allocated in a context that must fail fast upon
    allocation failure. See the overload of `wil::make_unique_nothrow()` for non-array types for more details.
    ~~~
    const size_t size = 42;
    auto foos = wil::make_unique_nothrow<Foo[]>(size); // the default constructor will be called on each Foo object
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<_Ty>::value && wistd::extent<_Ty>::value == 0,
                                     wistd::unique_ptr<_Ty>>::type
    make_unique_failfast(size_t _Size)
    {
        typedef typename wistd::remove_extent<_Ty>::type _Elem;
#pragma warning(suppress : 28193) // temporary must be inspected (it is within the called function)
...
#define __WIL_WINBASE_
    namespace details
    {
        ...
        template <PendingCallbackCancellationBehavior cancellationBehavior> struct DestroyThreadPoolWait
        {
            static void Destroy(_In_ PTP_WAIT threadPoolWait) WI_NOEXCEPT
            {
                ::SetThreadpoolWait(threadPoolWait, nullptr, nullptr);
                ::WaitForThreadpoolWaitCallbacks(threadPoolWait,
                                                 (cancellationBehavior == PendingCallbackCancellationBehavior::Cancel));
                ::CloseThreadpoolWait(threadPoolWait);
            }
        };

        template <> struct DestroyThreadPoolWait<PendingCallbackCancellationBehavior::NoWait>
        {
            static void Destroy(_In_ PTP_WAIT threadPoolWait) WI_NOEXCEPT
            {
                ::CloseThreadpoolWait(threadPoolWait);
        ...
        template <PendingCallbackCancellationBehavior cancellationBehavior> struct DestroyThreadPoolWork
        {
            static void Destroy(_In_ PTP_WORK threadpoolWork) WI_NOEXCEPT
            {
                ::WaitForThreadpoolWorkCallbacks(threadpoolWork,
                                                 (cancellationBehavior == PendingCallbackCancellationBehavior::Cancel));
                ::CloseThreadpoolWork(threadpoolWork);
            }
        };

        template <> struct DestroyThreadPoolWork<PendingCallbackCancellationBehavior::NoWait>
        {
            static void Destroy(_In_ PTP_WORK threadpoolWork) WI_NOEXCEPT
            {
                ::CloseThreadpoolWork(threadpoolWork);
        ...
        {
            static void WINAPI SetThreadpoolTimer(_Inout_ PTP_TIMER Timer, _In_opt_ PFILETIME DueTime,
                                                  _In_ DWORD Period, _In_ DWORD WindowLength) WI_NOEXCEPT
            {
                ::SetThreadpoolTimer(Timer, DueTime, Period, WindowLength);
            }
            static void WaitForThreadpoolTimerCallbacks(_Inout_ PTP_TIMER Timer,
                                                        _In_ BOOL CancelPendingCallbacks) WI_NOEXCEPT
            {
...
#pragma warning(suppress : 4127) // conditional expression is constant
        ...
        template <PendingCallbackCancellationBehavior cancellationBehavior> struct DestroyThreadPoolIo
        {
            static void Destroy(_In_ PTP_IO threadpoolIo) WI_NOEXCEPT
            {
                ::WaitForThreadpoolIoCallbacks(threadpoolIo,
                                               (cancellationBehavior == PendingCallbackCancellationBehavior::Cancel));
                ::CloseThreadpoolIo(threadpoolIo);
            }
        };

        template <> struct DestroyThreadPoolIo<PendingCallbackCancellationBehavior::NoWait>
        {
            static void Destroy(_In_ PTP_IO threadpoolIo) WI_NOEXCEPT
            {
                ::CloseThreadpoolIo(threadpoolIo);
        ...
        typedef resource_policy<HANDLE, decltype(&details::CloseHandle), details::CloseHandle,
                                details::pointer_access_all>
            handle_resource_policy;
    } // namespace details
    /// @endcond
    ...
    template <typename close_fn_t, close_fn_t close_fn>
    using unique_any_handle_null =
        unique_any_t<details::unique_storage<details::handle_null_resource_policy<close_fn_t, close_fn>>>;
    ...
    typedef unique_struct<TOKEN_LINKED_TOKEN, decltype(&details::CloseTokenLinkedToken), details::CloseTokenLinkedToken>
        unique_token_linked_token;

#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
    typedef unique_any<PSID, decltype(&::FreeSid), ::FreeSid> unique_sid;
    typedef unique_any_handle_null_only<decltype(&::DeleteBoundaryDescriptor), ::DeleteBoundaryDescriptor>
        unique_boundary_descriptor;

    /// @cond
    namespace details
    {
        template <ULONG flags> inline void __stdcall ClosePrivateNamespaceHelper(HANDLE h) WI_NOEXCEPT
        {
    ...
    using unique_private_namespace = unique_any_handle_null_only<void(__stdcall *)(HANDLE) WI_PFN_NOEXCEPT,
                                                                 &details::ClosePrivateNamespaceHelper<flags>>;
    ...
    typedef unique_any<PTP_WAIT, void (*)(PTP_WAIT),
                       details::DestroyThreadPoolWait<details::PendingCallbackCancellationBehavior::Cancel>::Destroy>
        unique_threadpool_wait;
    typedef unique_any<PTP_WAIT, void (*)(PTP_WAIT),
                       details::DestroyThreadPoolWait<details::PendingCallbackCancellationBehavior::Wait>::Destroy>
        unique_threadpool_wait_nocancel;
    typedef unique_any<PTP_WAIT, void (*)(PTP_WAIT),
                       details::DestroyThreadPoolWait<details::PendingCallbackCancellationBehavior::NoWait>::Destroy>
        unique_threadpool_wait_nowait;
    typedef unique_any<PTP_WORK, void (*)(PTP_WORK),
                       details::DestroyThreadPoolWork<details::PendingCallbackCancellationBehavior::Cancel>::Destroy>
        unique_threadpool_work;
    typedef unique_any<PTP_WORK, void (*)(PTP_WORK),
                       details::DestroyThreadPoolWork<details::PendingCallbackCancellationBehavior::Wait>::Destroy>
        unique_threadpool_work_nocancel;
    typedef unique_any<PTP_WORK, void (*)(PTP_WORK),
                       details::DestroyThreadPoolWork<details::PendingCallbackCancellationBehavior::NoWait>::Destroy>
        unique_threadpool_work_nowait;
    typedef unique_any<PTP_TIMER, void (*)(PTP_TIMER),
                       details::DestroyThreadPoolTimer<details::SystemThreadPoolMethods,
                                                       details::PendingCallbackCancellationBehavior::Cancel>::Destroy>
        unique_threadpool_timer;
    typedef unique_any<PTP_TIMER, void (*)(PTP_TIMER),
                       details::DestroyThreadPoolTimer<details::SystemThreadPoolMethods,
                                                       details::PendingCallbackCancellationBehavior::Wait>::Destroy>
        unique_threadpool_timer_nocancel;
    typedef unique_any<PTP_TIMER, void (*)(PTP_TIMER),
                       details::DestroyThreadPoolTimer<details::SystemThreadPoolMethods,
                                                       details::PendingCallbackCancellationBehavior::NoWait>::Destroy>
        unique_threadpool_timer_nowait;
    typedef unique_any<PTP_IO, void (*)(PTP_IO),
                       details::DestroyThreadPoolIo<details::PendingCallbackCancellationBehavior::Cancel>::Destroy>
        unique_threadpool_io;
    typedef unique_any<PTP_IO, void (*)(PTP_IO),
                       details::DestroyThreadPoolIo<details::PendingCallbackCancellationBehavior::Wait>::Destroy>
        unique_threadpool_io_nocancel;
    typedef unique_any<PTP_IO, void (*)(PTP_IO),
                       details::DestroyThreadPoolIo<details::PendingCallbackCancellationBehavior::NoWait>::Destroy>
        unique_threadpool_io_nowait;

#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
    typedef unique_any_handle_invalid<decltype(&::FindCloseChangeNotification), ::FindCloseChangeNotification>
        unique_hfind_change;
#endif

    typedef unique_any<HANDLE, decltype(&details::SetEvent), details::SetEvent, details::pointer_access_noaddress>
        event_set_scope_exit;
    typedef unique_any<HANDLE, decltype(&details::ResetEvent), details::ResetEvent, details::pointer_access_noaddress>
        event_reset_scope_exit;

    // Guarantees a SetEvent on the given event handle when the returned object goes out of scope
    // Note: call SetEvent early with the reset() method on the returned object or abort the call with the release()
    // method
    ...
    // Note: call ResetEvent early with the reset() method on the returned object or abort the call with the release()
    // method
    ...
    // Checks to see if the given *manual reset* event is currently signaled.  The event must not be an auto-reset
    // event. Use when the event will only be set once (cancellation-style) or will only be reset by the polling thread
    inline bool event_is_signaled(HANDLE hEvent) WI_NOEXCEPT
    {
        ...
        __FAIL_FAST_ASSERT__(
            (status == WAIT_TIMEOUT) ||
            ((status == WAIT_OBJECT_0) && (WAIT_OBJECT_0 == ::WaitForSingleObjectEx(hEvent, 0, FALSE))));
    ...
    template <typename storage_t, typename err_policy = err_exception_policy> class event_t : public storage_t
    {
      public:
        ...
        {
            static_assert(wistd::is_same<void, result>::value,
                          "this constructor requires exceptions or fail fast; use the create method");
            create(options);
        }
        ...
        // Guarantees a ResetEvent on the given event handle when the returned object goes out of scope
        // Note: call ResetEvent early with the reset() method on the returned object or abort the call with the
        // release() method
        ...
        // Tries to create a named event -- returns false if unable to do so (gle may still be inspected with
        // return=false)
        bool try_create(EventOptions options, PCWSTR name, _In_opt_ LPSECURITY_ATTRIBUTES securityAttributes = nullptr,
                        _Out_opt_ bool *alreadyExists = nullptr)
        {
            auto handle =
                ::CreateEventExW(securityAttributes, name,
        ...
        result create(EventOptions options = EventOptions::None, PCWSTR name = nullptr,
        ...
        // Tries to open the named event -- returns false if unable to do so (gle may still be inspected with
        // return=false)
        bool try_open(_In_ PCWSTR name, DWORD desiredAccess = SYNCHRONIZE | EVENT_MODIFY_STATE,
        ...
        // Returns HRESULT for unique_event_nothrow, void with exceptions for shared_event and unique_event
        result open(_In_ PCWSTR name, DWORD desiredAccess = SYNCHRONIZE | EVENT_MODIFY_STATE,
                    bool inheritHandle = false)
        {
    ...
    typedef unique_any_t<event_t<details::unique_storage<details::handle_resource_policy>, err_returncode_policy>>
        unique_event_nothrow;
    typedef unique_any_t<event_t<details::unique_storage<details::handle_resource_policy>, err_failfast_policy>>
        unique_event_failfast;
#ifdef WIL_ENABLE_EXCEPTIONS
    typedef unique_any_t<event_t<details::unique_storage<details::handle_resource_policy>, err_exception_policy>>
        unique_event;
#endif
    ...
    ~~~~ */
    template <SlimEventType Type> class slim_event_t
    {
      public:
...
#pragma warning(suppress : 4127) // conditional expression is constant
    ...
    /** An event object that will atomically revert to an unsignaled state anytime a `wait()` call succeeds (i.e.
     * returns true). */
    ...
    typedef unique_any<HANDLE, decltype(&details::ReleaseMutex), details::ReleaseMutex, details::pointer_access_none>
    ...
    template <typename storage_t, typename err_policy = err_exception_policy> class mutex_t : public storage_t
    {
      public:
        // forward all base class constructors...
        template <typename... args_t>
        ...
        WI_NODISCARD mutex_release_scope_exit acquire(_Out_opt_ DWORD *pStatus = nullptr,
                                                      DWORD dwMilliseconds = INFINITE,
                                                      BOOL bAlertable = FALSE) const WI_NOEXCEPT
        {
            ...
            __FAIL_FAST_ASSERT__((status == WAIT_TIMEOUT) || (status == WAIT_OBJECT_0) || (status == WAIT_ABANDONED) ||
                                 (bAlertable && (status == WAIT_IO_COMPLETION)));
            return mutex_release_scope_exit(((status == WAIT_OBJECT_0) || (status == WAIT_ABANDONED)) ? handle
                                                                                                      : nullptr);
        }

        // Tries to create a named mutex -- returns false if unable to do so (gle may still be inspected with
        // return=false)
                      ...
                      _In_opt_ PSECURITY_ATTRIBUTES mutexAttributes = nullptr, _Out_opt_ bool *alreadyExists = nullptr)
        {
            return err_policy::LastErrorIfFalse(
                try_create(name, dwFlags, desiredAccess, mutexAttributes, alreadyExists));
        }

        // Tries to open a named mutex -- returns false if unable to do so (gle may still be inspected with
        // return=false)
        bool try_open(_In_ PCWSTR name, DWORD desiredAccess = SYNCHRONIZE | MUTEX_MODIFY_STATE,
                      bool inheritHandle = false)
        {
        ...
        // Returns HRESULT for unique_mutex_nothrow, void with exceptions for shared_mutex and unique_mutex
        result open(_In_ PCWSTR name, DWORD desiredAccess = SYNCHRONIZE | MUTEX_MODIFY_STATE,
                    bool inheritHandle = false)
        {
            return err_policy::LastErrorIfFalse(try_open(name, desiredAccess, inheritHandle));
        }
    ...
    typedef unique_any_t<mutex_t<details::unique_storage<details::handle_resource_policy>, err_returncode_policy>>
        unique_mutex_nothrow;
    typedef unique_any_t<mutex_t<details::unique_storage<details::handle_resource_policy>, err_failfast_policy>>
        unique_mutex_failfast;
#ifdef WIL_ENABLE_EXCEPTIONS
    typedef unique_any_t<mutex_t<details::unique_storage<details::handle_resource_policy>, err_exception_policy>>
        unique_mutex;
#endif

    typedef unique_any<HANDLE, decltype(&details::ReleaseSemaphore), details::ReleaseSemaphore,
                       details::pointer_access_none>
        semaphore_release_scope_exit;
    ...
    template <typename storage_t, typename err_policy = err_exception_policy> class semaphore_t : public storage_t
    {
      public:
        // forward all base class constructors...
        template <typename... args_t>
        ...
        // Note that for custom-constructors the type given the constructor has to match exactly as not all implicit
        // conversions will make it through the forwarding constructor.  This constructor, for example, uses 'int'
        // instead of 'LONG' as the count to ease that particular issue (const numbers are int by default).
        explicit semaphore_t(int initialCount, int maximumCount, _In_opt_ PCWSTR name = nullptr,
        ...
        WI_NODISCARD semaphore_release_scope_exit acquire(_Out_opt_ DWORD *pStatus = nullptr,
                                                          DWORD dwMilliseconds = INFINITE,
                                                          BOOL bAlertable = FALSE) WI_NOEXCEPT
        {
            auto handle = storage_t::get();
            DWORD status = ::WaitForSingleObjectEx(handle, dwMilliseconds, bAlertable);
            assign_to_opt_param(pStatus, status);
        ...
        // Tries to create a named event -- returns false if unable to do so (gle may still be inspected with
        // return=false)
        bool try_create(LONG lInitialCount, LONG lMaximumCount, _In_opt_ PCWSTR name,
                        DWORD desiredAccess = SEMAPHORE_ALL_ACCESS,
                        _In_opt_ PSECURITY_ATTRIBUTES pSemaphoreAttributes = nullptr,
                        _Out_opt_ bool *alreadyExists = nullptr)
        {
            auto handle =
                ::CreateSemaphoreExW(pSemaphoreAttributes, lInitialCount, lMaximumCount, name, 0, desiredAccess);
            if (handle == nullptr)
            {
                assign_to_opt_param(alreadyExists, false);
                return false;
        ...
        result create(LONG lInitialCount, LONG lMaximumCount, _In_opt_ PCWSTR name = nullptr,
                      DWORD desiredAccess = SEMAPHORE_ALL_ACCESS,
                      _In_opt_ PSECURITY_ATTRIBUTES pSemaphoreAttributes = nullptr,
                      _Out_opt_ bool *alreadyExists = nullptr)
        {
        ...
        // Tries to open the named semaphore -- returns false if unable to do so (gle may still be inspected with
        // return=false)
        bool try_open(_In_ PCWSTR name, DWORD desiredAccess = SYNCHRONIZE | SEMAPHORE_MODIFY_STATE,
                      bool inheritHandle = false)
        {
        ...
        result open(_In_ PCWSTR name, DWORD desiredAccess = SYNCHRONIZE | SEMAPHORE_MODIFY_STATE,
                    bool inheritHandle = false)
        {
            return err_policy::LastErrorIfFalse(try_open(name, desiredAccess, inheritHandle));
        }
    ...
    typedef unique_any_t<semaphore_t<details::unique_storage<details::handle_resource_policy>, err_returncode_policy>>
        unique_semaphore_nothrow;
    typedef unique_any_t<semaphore_t<details::unique_storage<details::handle_resource_policy>, err_failfast_policy>>
        unique_semaphore_failfast;
#ifdef WIL_ENABLE_EXCEPTIONS
    typedef unique_any_t<semaphore_t<details::unique_storage<details::handle_resource_policy>, err_exception_policy>>
        unique_semaphore;
#endif

    typedef unique_any<SRWLOCK *, decltype(&::ReleaseSRWLockExclusive), ::ReleaseSRWLockExclusive,
                       details::pointer_access_noaddress>
        rwlock_release_exclusive_scope_exit;
    typedef unique_any<SRWLOCK *, decltype(&::ReleaseSRWLockShared), ::ReleaseSRWLockShared,
                       details::pointer_access_noaddress>
    ...
    WI_NODISCARD inline rwlock_release_exclusive_scope_exit TryAcquireSRWLockExclusive(_Inout_ SRWLOCK *plock)
        WI_NOEXCEPT
    {
    ...
    typedef unique_any<CRITICAL_SECTION *, decltype(&::LeaveCriticalSection), ::LeaveCriticalSection,
                       details::pointer_access_noaddress>
        cs_leave_scope_exit;
    ...
    class condition_variable
    {
      public:
        ...
        {
            bool result =
                !!::SleepConditionVariableSRW(&m_cv, lock.get(), timeoutMs, CONDITION_VARIABLE_LOCKMODE_SHARED);
            __FAIL_FAST_ASSERT__(result || ::GetLastError() == ERROR_TIMEOUT);
            return result;
        }
    ...
    namespace details
    {
        template <typename string_class> struct string_allocator
        {
            ...
            {
                static_assert(!wistd::is_same<string_class, string_class>::value,
                              "This type did not provide a string_allocator, add a specialization of string_allocator "
    ...
    template <typename string_type> PCWSTR string_get_not_null(const string_type &string)
    {
...
#define MAKE_UNIQUE_STRING_MAX_CCH 2147483647 // max buffer size, in characters, that we support (same as INT_MAX)
    ...
    /** Copies a string (up to the given length) into memory allocated with a specified allocator returning null on
    failure. Use `wil::make_unique_string_nothrow()` for string resources returned from APIs that must satisfy a memory
    allocation contract that requires use of a specific allocator and free function (CoTaskMemAlloc/CoTaskMemFree,
    LocalAlloc/LocalFree, GlobalAlloc/GlobalFree, etc.).
    ~~~
    ...
    */
    template <typename string_type>
    string_type make_unique_string_nothrow(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                  _In_reads_(length))
    ...
    template <typename string_type>
    string_type make_unique_ansistring_nothrow(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                      _In_reads_(length))
                                                   _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                          _In_z_) PCSTR source,
                                               size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
    {
        if (length == static_cast<size_t>(-1))
    ...
    */
    template <typename string_type>
    string_type make_unique_string_failfast(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                   _In_reads_(length))
                                                _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                                    PCWSTR source,
                                            size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
...
#ifndef WIL_NO_ANSI_STRINGS
    template <typename string_type>
    string_type make_unique_ansistring_failfast(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                       _In_reads_(length))
                                                    _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                           _In_z_) PCSTR source,
                                                size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
    ...
    */
    template <typename string_type>
    string_type make_unique_string(_When_((source != nullptr) && length != static_cast<size_t>(-1), _In_reads_(length))
                                       _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                           PCWSTR source,
...
#ifndef WIL_NO_ANSI_STRINGS
    template <typename string_type>
    string_type make_unique_ansistring(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                              _In_reads_(length))
                                           _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                               PCSTR source,
                                       size_t length = static_cast<size_t>(-1))
    ...
    namespace details
    {
        ...
        template <typename string_type> struct string_maker
        {
            HRESULT make(_When_((source != nullptr) && length != static_cast<size_t>(-1), _In_reads_(length))
                             _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                 const wchar_t *source,
                         size_t length)
            {
    ...
    typedef unique_any<void *, decltype(&details::SecureZeroData::Close), details::SecureZeroData::Close,
                       details::pointer_access_all, details::SecureZeroData>
        secure_zero_memory_scope_exit;

    WI_NODISCARD inline secure_zero_memory_scope_exit SecureZeroMemory_scope_exit(
        _In_reads_bytes_(sizeBytes) void *pSource, size_t sizeBytes)
    {
    ...
    {
        return SecureZeroMemory_scope_exit(static_cast<void *>(initializedString),
                                           wcslen(initializedString) * sizeof(initializedString[0]));
    }
    ...
    {
        template <typename T> void operator()(_Pre_valid_ _Frees_ptr_ T *p) const
        {
    ...
    */
    ...
    using unique_virtualalloc_ptr =
        wistd::unique_ptr<details::ensure_trivially_destructible_t<T>, virtualalloc_deleter>;
...
#define __WIL_WINBASE_NOTHROW_T_DEFINED
    ...
    namespace details
    {
        ...
        typedef resource_policy<event_watcher_state *, decltype(&delete_event_watcher_state),
                                delete_event_watcher_state, details::pointer_access_none>
            event_watcher_state_resource_policy;
    } // namespace details
    /// @endcond

    template <typename storage_t, typename err_policy = err_exception_policy> class event_watcher_t : public storage_t
    {
      public:
        // forward all base class constructors...
        template <typename... args_t>
        ...
        event_watcher_t(unique_any_t<event_t<details::unique_storage<details::handle_resource_policy>, from_err_policy>>
                            &&eventHandle,
        ...
        result create(unique_any_t<event_t<details::unique_storage<details::handle_resource_policy>, event_err_policy>>
                          &&eventHandle,
                      wistd::function<void()> &&callback)
        {
        ...
        result create(_In_ HANDLE eventHandle, wistd::function<void()> &&callback)
        {
            ...
            if (!DuplicateHandle(GetCurrentProcess(), eventHandle, GetCurrentProcess(), &ownedHandle, 0, FALSE,
      ...
      private:
        // Had to move this from a Lambda so it would compile in C++/CLI (which thought the Lambda should be a managed
        // function for some reason).
        static void CALLBACK wait_callback(PTP_CALLBACK_INSTANCE, void *context, TP_WAIT *pThreadPoolWait,
                                           TP_WAIT_RESULT)
        {
    ...
    typedef unique_any_t<
        event_watcher_t<details::unique_storage<details::event_watcher_state_resource_policy>, err_returncode_policy>>
        unique_event_watcher_nothrow;
    typedef unique_any_t<
        event_watcher_t<details::unique_storage<details::event_watcher_state_resource_policy>, err_failfast_policy>>
    ...
    inline unique_event_watcher_nothrow make_event_watcher_nothrow(_In_ HANDLE eventHandle,
                                                                   wistd::function<void()> &&callback) WI_NOEXCEPT
    {
        unique_event_watcher_nothrow watcher;
    ...
    inline unique_event_watcher_failfast make_event_watcher_failfast(_In_ HANDLE eventHandle,
                                                                     wistd::function<void()> &&callback)
    {
...
#ifdef WIL_ENABLE_EXCEPTIONS
    typedef unique_any_t<
        event_watcher_t<details::unique_storage<details::event_watcher_state_resource_policy>, err_exception_policy>>
...
#if (defined(_HEAPAPI_H_) && !defined(__WIL__WIL_HEAP_API) &&                                                          \
...
#define __WIL__WIL_HEAP_API
    ...
    template <typename T = void>
    using unique_process_heap_ptr =
        wistd::unique_ptr<details::ensure_trivially_destructible_t<T>, process_heap_deleter>;
...
#ifndef WIL_NO_ANSI_STRINGS
    typedef unique_any<PSTR, decltype(&wil::details::FreeProcessHeap), wil::details::FreeProcessHeap>
        unique_process_heap_ansistring;
#endif // WIL_NO_ANSI_STRINGS

    /// @cond
    namespace details
    {
        template <> struct string_allocator<wil::unique_process_heap_string> : heap_allocator
        {
        };

#ifndef WIL_NO_ANSI_STRINGS
        template <> struct string_allocator<unique_process_heap_ansistring> : heap_allocator
        {
        };
#endif
    } // namespace details
    ...
    inline auto make_process_heap_string_nothrow(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                        _In_reads_(length))
                                                     _When_((source != nullptr) && length == static_cast<size_t>(-1),
    ...
    inline auto make_process_heap_string_failfast(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                         _In_reads_(length))
                                                      _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                             _In_z_) PCWSTR source,
                                                  size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
...
#ifdef WIL_ENABLE_EXCEPTIONS
    inline auto make_process_heap_string(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                _In_reads_(length))
                                             _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                                 PCWSTR source,
                                         size_t length = static_cast<size_t>(-1))
...
#ifndef WIL_NO_ANSI_STRINGS
    inline auto make_process_heap_ansistring(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                    _In_reads_(length))
                                                 _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                        _In_z_) PCSTR source,
                                             size_t length = static_cast<size_t>(-1))
...
#if (defined(__WIL_WINBASE_) && !defined(__WIL_WINBASE_DESKTOP) &&                                                     \
...
#define __WIL_WINBASE_DESKTOP
    namespace details
    {
        inline void __stdcall DestroyPrivateObjectSecurity(
            _Pre_opt_valid_ _Frees_ptr_opt_ PSECURITY_DESCRIPTOR pObjectDescriptor) WI_NOEXCEPT
        {
    ...
    template <typename T> using unique_hlocal_array_ptr = wil::unique_array_ptr<T, wil::hlocal_deleter>;

    /** Provides `std::make_unique()` semantics for resources allocated with `LocalAlloc()` in a context that may not
    throw upon allocation failure. Use `wil::make_unique_hlocal_nothrow()` for resources returned from APIs that must
    satisfy a memory allocation contract that requires the use of `LocalAlloc()` / `LocalFree()`. Use
    `wil::make_unique_nothrow()` when `LocalAlloc()` is not required.

    Allocations are initialized with placement new and will call constructors (if present), but this does not guarantee
    initialization.

    Note that `wil::make_unique_hlocal_nothrow()` is not marked WI_NOEXCEPT as it may be used to create an
    exception-based class that may throw in its constructor.
    @code
    ...
    */
    ...
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_hlocal_ptr<T>>::type make_unique_hlocal_nothrow(
        Args &&...args)
    {
    ...
    /** Provides `std::make_unique()` semantics for array resources allocated with `LocalAlloc()` in a context that may
    not throw upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array types for
    more details.
    @code
    const size_t size = 42;
    ...
    */
    template <typename T>
    inline
        typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0, unique_hlocal_ptr<T>>::type
        make_unique_hlocal_nothrow(size_t size)
    {
    ...
    /** Provides `std::make_unique()` semantics for resources allocated with `LocalAlloc()` in a context that must fail
    fast upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array types for more
    details.
    @code
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_hlocal_ptr<T>>::type
    make_unique_hlocal_failfast(Args &&...args)
    {
    ...
    /** Provides `std::make_unique()` semantics for array resources allocated with `LocalAlloc()` in a context that must
    fail fast upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array types for
    more details.
    @code
    const size_t size = 42;
    ...
    */
    ...
    inline
        typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0, unique_hlocal_ptr<T>>::type
        make_unique_hlocal_failfast(size_t size)
    {
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_hlocal_ptr<T>>::type make_unique_hlocal(
        Args &&...args)
    {
        unique_hlocal_ptr<T> result(make_unique_hlocal_nothrow<T>(wistd::forward<Args>(args)...));
        THROW_IF_NULL_ALLOC(result);
    ...
    */
    ...
    inline
        typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0, unique_hlocal_ptr<T>>::type
        make_unique_hlocal(size_t size)
    {
        unique_hlocal_ptr<T> result(make_unique_hlocal_nothrow<T>(size));
        THROW_IF_NULL_ALLOC(result);
        return result;
    ...
    inline auto make_hlocal_string_nothrow(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                  _In_reads_(length))
                                               _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                                   PCWSTR source,
                                           size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
    ...
    inline auto make_hlocal_string_failfast(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                   _In_reads_(length))
                                                _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                                    PCWSTR source,
                                            size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
...
#ifndef WIL_NO_ANSI_STRINGS
    inline auto make_hlocal_ansistring_nothrow(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                      _In_reads_(length))
                                                   _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                          _In_z_) PCSTR source,
                                               size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
    ...
    inline auto make_hlocal_ansistring_failfast(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                       _In_reads_(length))
                                                    _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                           _In_z_) PCSTR source,
                                                size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
...
#ifdef WIL_ENABLE_EXCEPTIONS
    inline auto make_hlocal_string(_When_((source != nullptr) && length != static_cast<size_t>(-1), _In_reads_(length))
                                       _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                           PCWSTR source,
                                   size_t length = static_cast<size_t>(-1))
    {
...
#ifndef WIL_NO_ANSI_STRINGS
    inline auto make_hlocal_ansistring(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                              _In_reads_(length))
                                           _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                               PCSTR source,
                                       size_t length = static_cast<size_t>(-1))
...
#pragma warning(suppress : 26006 26007)                // LocalSize() ensures proper buffer length
    ...
    template <typename T = void>
    using unique_hlocal_secure_ptr =
        wistd::unique_ptr<details::ensure_trivially_destructible_t<T>, hlocal_secure_deleter>;

    /** Provides `std::make_unique()` semantics for secure resources allocated with `LocalAlloc()` in a context that may
    not throw upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array types for
    more details.
    @code
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_hlocal_secure_ptr<T>>::type
    make_unique_hlocal_secure_nothrow(Args &&...args)
    {
        return unique_hlocal_secure_ptr<T>(make_unique_hlocal_nothrow<T>(wistd::forward<Args>(args)...).release());
    }

    /** Provides `std::make_unique()` semantics for secure array resources allocated with `LocalAlloc()` in a context
    that may not throw upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array
    ...
    */
    template <typename T>
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_hlocal_secure_ptr<T>>::type
    make_unique_hlocal_secure_nothrow(size_t size)
    {
        return unique_hlocal_secure_ptr<T>(make_unique_hlocal_nothrow<T>(size).release());
    }

    /** Provides `std::make_unique()` semantics for secure resources allocated with `LocalAlloc()` in a context that
    must fail fast upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array types
    for more details.
    @code
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_hlocal_secure_ptr<T>>::type
    make_unique_hlocal_secure_failfast(Args &&...args)
    {
    ...
    /** Provides `std::make_unique()` semantics for secure array resources allocated with `LocalAlloc()` in a context
    that must fail fast upon allocation failure. See the overload of `wil::make_unique_hlocal_nothrow()` for non-array
    types for more details.
    @code
    const size_t size = 42;
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_hlocal_secure_ptr<T>>::type
    make_unique_hlocal_secure_failfast(size_t size)
    {
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_hlocal_secure_ptr<T>>::type
    make_unique_hlocal_secure(Args &&...args)
    {
        unique_hlocal_secure_ptr<T> result(make_unique_hlocal_secure_nothrow<T>(wistd::forward<Args>(args)...));
        THROW_IF_NULL_ALLOC(result);
        return result;
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_hlocal_secure_ptr<T>>::type
    make_unique_hlocal_secure(size_t size)
    {
        unique_hlocal_secure_ptr<T> result(make_unique_hlocal_secure_nothrow<T>(size));
        THROW_IF_NULL_ALLOC(result);
        return result;
    ...
    /** Copies a given string into secure memory allocated with `LocalAlloc()` in a context that may not throw upon
    allocation failure. See the overload of `wil::make_hlocal_string_nothrow()` with supplied length for more details.
    ~~~
    ...
    */
    ...
    /** Copies a given string into secure memory allocated with `LocalAlloc()` in a context that must fail fast upon
    allocation failure. See the overload of `wil::make_hlocal_string_nothrow()` with supplied length for more details.
    ~~~
    ...
    */
    ...
    typedef unique_any<DWORD, decltype(&::TlsFree), ::TlsFree, details::pointer_access_all, DWORD, DWORD,
                       TLS_OUT_OF_INDEXES, DWORD>
        unique_tls;
    typedef unique_any<PSECURITY_DESCRIPTOR, decltype(&::LocalFree), ::LocalFree> unique_hlocal_security_descriptor;
    typedef unique_any<PSECURITY_DESCRIPTOR, decltype(&details::DestroyPrivateObjectSecurity),
                       details::DestroyPrivateObjectSecurity>
...
/// @endcond
    ...
    typedef unique_any<CO_MTA_USAGE_COOKIE, decltype(&::CoDecrementMTAUsage), ::CoDecrementMTAUsage>
...
#if (defined(__WIL__COMBASEAPI_H_) && !defined(__WIL__COMBASEAPI_H__STL) && defined(WIL_RESOURCE_STL) &&               \
...
#define __WIL__COMBASEAPI_H_APP
    ...
    WI_NODISCARD inline unique_couninitialize_call CoInitializeEx_failfast(
        DWORD coinitFlags = 0 /*COINIT_MULTITHREADED*/)
    {
...
#define __WIL__ROAPI_H_APP
    ...
    typedef unique_any<RO_REGISTRATION_COOKIE, decltype(&::RoRevokeActivationFactories), ::RoRevokeActivationFactories>
        unique_ro_registration_cookie;
...
#define __WIL__WINSTRING_H_
    ...
    typedef unique_any<HSTRING_BUFFER, decltype(&::WindowsDeleteStringBuffer), ::WindowsDeleteStringBuffer>
        unique_hstring_buffer;

    /** Promotes an hstring_buffer to an HSTRING.
    When an HSTRING_BUFFER object is promoted to a real string it must not be passed to WindowsDeleteString. The caller
    owns the HSTRING afterwards.
    ~~~
        ...
        template <> struct string_maker<unique_hstring>
        {
                         ...
                         size_t length)
            {
                if (source)
                {
                    ...
                    m_bufferHandle.reset(); // do this after WindowsCreateString so we can trim_at_existing_null() from
                                            // our own buffer
                }
                else
                {
                    ...
                    m_value.reset();
                    RETURN_IF_FAILED(
                        WindowsPreallocateStringBuffer(static_cast<UINT32>(length), &m_charBuffer, &m_bufferHandle));
                }
                return S_OK;
            }
...
// if language extensions (/Za) disabled, PropVariantInit will not exist, PROPVARIANT has forward declaration only
    ...
    using unique_prop_variant = wil::unique_struct<PROPVARIANT, decltype(&::PropVariantClear), ::PropVariantClear,
...
#define __WIL_OLEAUTO_H_
    ...
    using unique_variant =
        wil::unique_struct<VARIANT, decltype(&::VariantClear), ::VariantClear, decltype(&::VariantInit), ::VariantInit>;
...
#define __WIL_WINSOCKAPI_
    /// @endcond
    typedef unique_any<SOCKET, int(WINAPI *)(SOCKET), ::closesocket, details::pointer_access_all, SOCKET, SOCKET,
...
#define __WIL_WINSOCKAPI_STL
    ...
    typedef weak_any<shared_socket> weak_socket;
     ...
     !defined(NOGDI) && !defined(WIL_KERNEL_MODE)) ||                                                                  \
    defined(WIL_DOXYGEN)
/// @cond
#define __WIL_WINGDI_
    ...
    typedef unique_any<HDC, decltype(&window_dc::close), window_dc::close, details::pointer_access_all, window_dc>
        unique_hdc_window;
    ...
    typedef unique_any<HDC, decltype(&paint_dc::close), paint_dc::close, details::pointer_access_all, paint_dc>
        unique_hdc_paint;
    ...
    typedef unique_any<HGDIOBJ, decltype(&select_result::close), select_result::close, details::pointer_access_all,
                       select_result>
...
#define __WIL_WTSAPI
    /// @endcond
    template <typename T>
    using unique_wtsmem_ptr = wistd::unique_ptr<details::ensure_trivially_destructible_t<T>,
                                                function_deleter<decltype(&WTSFreeMemory), WTSFreeMemory>>;
#endif // __WIL_WTSAPI
...
#define __WIL__WINCRYPT_H__
    namespace details
    {
        ...
        inline void __stdcall CryptReleaseContextNoParam(_Pre_opt_valid_ _Frees_ptr_opt_ HCRYPTPROV hCryptCtx)
            WI_NOEXCEPT
        {
    ...
    struct cert_context_t
        : details::unique_storage<details::resource_policy<PCCERT_CONTEXT, decltype(&::CertFreeCertificateContext),
    ...
    typedef unique_any<PCCERT_CHAIN_CONTEXT, decltype(&::CertFreeCertificateChain), ::CertFreeCertificateChain>
        unique_cert_chain_context;
    typedef unique_any<HCERTSTORE, decltype(&details::CertCloseStoreNoParam), details::CertCloseStoreNoParam>
        unique_hcertstore;
    typedef unique_any<HCRYPTPROV, decltype(&details::CryptReleaseContextNoParam), details::CryptReleaseContextNoParam>
        unique_hcryptprov;
...
#define __WIL_BCRYPT_H__
    namespace details
    {
        inline void __stdcall BCryptCloseAlgorithmProviderNoFlags(
            _Pre_opt_valid_ _Frees_ptr_opt_ BCRYPT_ALG_HANDLE hAlgorithm) WI_NOEXCEPT
        {
            if (hAlgorithm)
            {
    ...
    typedef unique_any<BCRYPT_ALG_HANDLE, decltype(&details::BCryptCloseAlgorithmProviderNoFlags),
                       details::BCryptCloseAlgorithmProviderNoFlags>
    ...
    typedef unique_any<BCRYPT_SECRET_HANDLE, decltype(&::BCryptDestroySecret), ::BCryptDestroySecret>
...
#define __WIL_OBJBASE_H_
    ...
    template <typename T> using unique_cotaskmem_array_ptr = unique_array_ptr<T, cotaskmem_deleter>;

    /** Provides `std::make_unique()` semantics for resources allocated with `CoTaskMemAlloc()` in a context that may
    not throw upon allocation failure. Use `wil::make_unique_cotaskmem_nothrow()` for resources returned from APIs that
    must satisfy a memory allocation contract that requires the use of `CoTaskMemAlloc()` / `CoTaskMemFree()`. Use
    `wil::make_unique_nothrow()` when `CoTaskMemAlloc()` is not required.

    Allocations are initialized with placement new and will call constructors (if present), but this does not guarantee
    initialization.

    Note that `wil::make_unique_cotaskmem_nothrow()` is not marked WI_NOEXCEPT as it may be used to create an
    exception-based class that may throw in its constructor.
    @code
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_cotaskmem_ptr<T>>::type
    make_unique_cotaskmem_nothrow(Args &&...args)
    {
    ...
    /** Provides `std::make_unique()` semantics for array resources allocated with `CoTaskMemAlloc()` in a context that
    may not throw upon allocation failure. See the overload of `wil::make_unique_cotaskmem_nothrow()` for non-array
    types for more details.
    @code
    const size_t size = 42;
    ...
    */
    template <typename T>
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_cotaskmem_ptr<T>>::type
    make_unique_cotaskmem_nothrow(size_t size)
    {
        typedef typename wistd::remove_extent<T>::type E;
        FAIL_FAST_IF((__WI_SIZE_MAX / sizeof(E)) < size);
        size_t allocSize = sizeof(E) * size;
    ...
    /** Provides `std::make_unique()` semantics for resources allocated with `CoTaskMemAlloc()` in a context that must
    fail fast upon allocation failure. See the overload of `wil::make_unique_cotaskmem_nothrow()` for non-array types
    for more details.
    @code
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_cotaskmem_ptr<T>>::type
    make_unique_cotaskmem_failfast(Args &&...args)
    {
    ...
    /** Provides `std::make_unique()` semantics for array resources allocated with `CoTaskMemAlloc()` in a context that
    must fail fast upon allocation failure. See the overload of `wil::make_unique_cotaskmem_nothrow()` for non-array
    types for more details.
    @code
    const size_t size = 42;
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_cotaskmem_ptr<T>>::type
    make_unique_cotaskmem_failfast(size_t size)
    {
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_cotaskmem_ptr<T>>::type make_unique_cotaskmem(
        Args &&...args)
    {
        unique_cotaskmem_ptr<T> result(make_unique_cotaskmem_nothrow<T>(wistd::forward<Args>(args)...));
        THROW_IF_NULL_ALLOC(result);
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_cotaskmem_ptr<T>>::type
    make_unique_cotaskmem(size_t size)
    {
        unique_cotaskmem_ptr<T> result(make_unique_cotaskmem_nothrow<T>(size));
        THROW_IF_NULL_ALLOC(result);
        return result;
    ...
    inline auto make_cotaskmem_string_nothrow(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                     _In_reads_(length))
                                                  _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                         _In_z_) PCWSTR source,
                                              size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
    ...
    inline auto make_cotaskmem_string_failfast(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                                      _In_reads_(length))
                                                   _When_((source != nullptr) && length == static_cast<size_t>(-1),
                                                          _In_z_) PCWSTR source,
                                               size_t length = static_cast<size_t>(-1)) WI_NOEXCEPT
...
#ifdef WIL_ENABLE_EXCEPTIONS
    inline auto make_cotaskmem_string(_When_((source != nullptr) && length != static_cast<size_t>(-1),
                                             _In_reads_(length))
                                          _When_((source != nullptr) && length == static_cast<size_t>(-1), _In_z_)
                                              PCWSTR source,
                                      size_t length = static_cast<size_t>(-1))
...
#define __WIL_OBJBASE_AND_WINBASE_H_
    ...
    template <typename T = void>
    using unique_cotaskmem_secure_ptr =
        wistd::unique_ptr<details::ensure_trivially_destructible_t<T>, cotaskmem_secure_deleter>;

    /** Provides `std::make_unique()` semantics for secure resources allocated with `CoTaskMemAlloc()` in a context that
    may not throw upon allocation failure. See the overload of `wil::make_unique_cotaskmem_nothrow()` for non-array
    types for more details.
    @code
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_cotaskmem_secure_ptr<T>>::type
    make_unique_cotaskmem_secure_nothrow(Args &&...args)
    {
        return unique_cotaskmem_secure_ptr<T>(
            make_unique_cotaskmem_nothrow<T>(wistd::forward<Args>(args)...).release());
    }

    /** Provides `std::make_unique()` semantics for secure array resources allocated with `CoTaskMemAlloc()` in a
    context that may not throw upon allocation failure. See the overload of `wil::make_unique_cotaskmem_nothrow()` for
    ...
    */
    template <typename T>
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_cotaskmem_secure_ptr<T>>::type
    make_unique_cotaskmem_secure_nothrow(size_t size)
    {
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_cotaskmem_secure_ptr<T>>::type
    make_unique_cotaskmem_secure_failfast(Args &&...args)
    {
    ...
    /** Provides `std::make_unique()` semantics for secure array resources allocated with `CoTaskMemAlloc()` in a
    context that must fail fast upon allocation failure. See the overload of `wil::make_unique_cotaskmem_nothrow()` for
    non-array types for more details.
    @code
    const size_t size = 42;
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_cotaskmem_secure_ptr<T>>::type
    make_unique_cotaskmem_secure_failfast(size_t size)
    {
    ...
    */
    template <typename T, typename... Args>
    inline typename wistd::enable_if<!wistd::is_array<T>::value, unique_cotaskmem_secure_ptr<T>>::type
    make_unique_cotaskmem_secure(Args &&...args)
    {
        unique_cotaskmem_secure_ptr<T> result(make_unique_cotaskmem_secure_nothrow<T>(wistd::forward<Args>(args)...));
        THROW_IF_NULL_ALLOC(result);
        return result;
    ...
    */
    ...
    inline typename wistd::enable_if<wistd::is_array<T>::value && wistd::extent<T>::value == 0,
                                     unique_cotaskmem_secure_ptr<T>>::type
    make_unique_cotaskmem_secure(size_t size)
    {
        unique_cotaskmem_secure_ptr<T> result(make_unique_cotaskmem_secure_nothrow<T>(size));
        THROW_IF_NULL_ALLOC(result);
        return result;
    ...
    /** Copies a given string into secure memory allocated with `CoTaskMemAlloc()` in a context that may not throw upon
    allocation failure. See the overload of `wil::make_cotaskmem_string_nothrow()` with supplied length for more
    details.
    ~~~
...
#define __WIL_OLE2_H_
    ...
    struct unique_hglobal_locked : public unique_any<void *, decltype(&::GlobalUnlock), ::GlobalUnlock>
    {
        ...
        explicit unique_hglobal_locked(HGLOBAL global)
            : unique_any<void *, decltype(&::GlobalUnlock), ::GlobalUnlock>(global)
        {
...
#define __WIL_INC_USERENV
    /// @endcond
    typedef unique_any<void *, decltype(&::DestroyEnvironmentBlock), ::DestroyEnvironmentBlock>
        unique_environment_block;
#endif // __WIL_INC_USERENV
#pragma warning(pop)

#if (defined(__WINEVT_H__) && !defined(__WIL_INC_EVT_HANDLE) &&                                                        \
     WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_PKG_EVENTLOGSERVICE) &&                       \
     !defined(WIL_KERNEL_MODE)) ||                                                                                     \
    defined(WIL_DOXYGEN)
/// @cond
...
#if (defined(_WINSVC_) && !defined(__WIL_HANDLE_H_WINSVC) && WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP) &&      \
     !defined(WIL_KERNEL_MODE)) ||                                                                                     \
    defined(WIL_DOXYGEN)
/// @cond
...
#define __WIL_LSALOOKUP_
    ...
    template <typename T>
    using unique_lsalookupmem_ptr =
        wistd::unique_ptr<details::ensure_trivially_destructible_t<T>, lsalookup_freemem_deleter>;
#endif // _LSALOOKUP_
...
#if (defined(_NTLSA_IFS_) && !defined(__WIL_HANDLE_H_NTLSA_IFS_) &&                                                    \
     WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)) ||                                                             \
    defined(WIL_DOXYGEN)
/// @cond
...
#define __WIL_RPCDCE_H__
    ...
    typedef unique_any<RPC_BINDING_HANDLE, decltype(&details::WpRpcBindingFree), details::WpRpcBindingFree>
        unique_rpc_binding;
    typedef unique_any<RPC_BINDING_VECTOR *, decltype(&details::WpRpcBindingVectorFree),
                       details::WpRpcBindingVectorFree>
...
#if (defined(_NETIOAPI_H_) && defined(_WS2IPDEF_) && defined(MIB_INVALID_TEREDO_PORT_NUMBER) &&                        \
     !defined(__WIL_NETIOAPI_H_) && WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)) ||                              \
    defined(WIL_DOXYGEN)
/// @cond
...
#if (defined(_WLAN_WLANAPI_H) && !defined(__WIL_WLAN_WLANAPI_H) &&                                                     \
     WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)) ||                                                             \
    defined(WIL_DOXYGEN)
/// @cond
#define __WIL_WLAN_WLANAPI_H
    ...
    typedef unique_any<HANDLE, decltype(&details::CloseWlanHandle), details::CloseWlanHandle,
                       details::pointer_access_all, HANDLE, INT_PTR, -1>
        unique_wlan_handle;
#endif
...
#define __WIL_PROCESSTHREADSAPI_H_DESK_SYS
    ...
    ~~~
    */
    using unique_process_information = unique_struct<PROCESS_INFORMATION, decltype(&details::CloseProcessInformation),
                                                     details::CloseProcessInformation>;
#endif

#if (defined(_PROCESSENV_) && !defined(__WIL__PROCESSENV_) &&                                                          \
     WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)) ||                                       \
    defined(WIL_DOXYGEN)
/// @cond
...
#define __WIL_APPMODEL_H_
    /// @endcond
    typedef unique_any<PACKAGE_INFO_REFERENCE, decltype(&::ClosePackageInfo), ::ClosePackageInfo>
        unique_package_info_reference;
#if NTDDI_VERSION >= NTDDI_WIN10_CO
    typedef unique_any<PACKAGEDEPENDENCY_CONTEXT, decltype(&::RemovePackageDependency), ::RemovePackageDependency>
        unique_package_dependency_context;
...
#define __WIL_MSIXDYNAMICDEPENDENCY_H
    /// @endcond
    typedef unique_any<MDD_PACKAGEDEPENDENCY_CONTEXT, decltype(&::MddRemovePackageDependency),
                       ::MddRemovePackageDependency>
        unique_mdd_package_dependency_context;
#endif // __WIL_MSIXDYNAMICDEPENDENCY_H
#if (defined(MSIXDYNAMICDEPENDENCY_H) && !defined(__WIL_MSIXDYNAMICDEPENDENCY_H_STL) && defined(WIL_RESOURCE_STL)) ||  \
    defined(WIL_DOXYGEN)
/// @cond
...
/// @endcond
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_APP | WINAPI_PARTITION_SYSTEM)
    typedef unique_any<DLL_DIRECTORY_COOKIE, decltype(&::RemoveDllDirectory), ::RemoveDllDirectory>
        unique_dll_directory_cookie;
#endif
#endif // __WIL_APISETLIBLOADER_
#if (defined(_APISETLIBLOADER_) && !defined(__WIL_APISETLIBLOADER_STL) && defined(WIL_RESOURCE_STL)) ||                \
    defined(WIL_DOXYGEN)
/// @cond
...
#define __WIL_WDFAPI
    namespace details
    {
        ...
        using wdf_object_resource_policy =
            resource_policy<TWDFOBJECT, decltype(&::WdfObjectDelete), &::WdfObjectDelete>;
    }
    /// @endcond
    ...
    WI_NODISCARD inline _IRQL_requires_max_(PASSIVE_LEVEL) _Acquires_lock_(lock) wdf_wait_lock_release_scope_exit
    ...
    WI_NODISCARD inline _IRQL_requires_max_(APC_LEVEL)
        _When_(return, _Acquires_lock_(lock)) wdf_wait_lock_release_scope_exit
    ...
    WI_NODISCARD inline _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_raises_(DISPATCH_LEVEL)
        _Acquires_lock_(lock) wdf_spin_lock_release_scope_exit acquire_wdf_spin_lock(WDFSPINLOCK lock) WI_NOEXCEPT
    {
    ...
    namespace details
    {
        ...
        class unique_wdf_spin_lock_storage : public unique_wdf_lock_storage<WDFSPINLOCK>
        {
          ...
          public:
            ...
            template <typename... args_t>
            explicit unique_wdf_spin_lock_storage(args_t &&...args) WI_NOEXCEPT
                : wdf_lock_storage_t(wistd::forward<args_t>(args)...)
            {
            }
            ...
            WI_NODISCARD
            _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_raises_(DISPATCH_LEVEL) wdf_spin_lock_release_scope_exit
                acquire() WI_NOEXCEPT
            {
        ...
        class unique_wdf_wait_lock_storage : public unique_wdf_lock_storage<WDFWAITLOCK>
        {
          ...
          public:
            ...
            // Forward all base class constructors, but have it be explicit.
            template <typename... args_t>
            explicit unique_wdf_wait_lock_storage(args_t &&...args) WI_NOEXCEPT
                : wdf_lock_storage_t(wistd::forward<args_t>(args)...)
            {
            }
            ...
            WI_NODISCARD
            _IRQL_requires_max_(PASSIVE_LEVEL) wdf_wait_lock_release_scope_exit acquire() WI_NOEXCEPT
            {
                return wil::acquire_wdf_wait_lock(wdf_lock_storage_t::get());
            }

            WI_NODISCARD
            _IRQL_requires_max_(APC_LEVEL) wdf_wait_lock_release_scope_exit try_acquire() WI_NOEXCEPT
            {
    ...
    template <typename wdf_object_t> class unique_wdf_object_reference
    {
      public:
        ...
        explicit unique_wdf_object_reference(const wil::unique_wdf_any<wdf_object_t> &wdfObject,
                                             void *tag = nullptr) WI_NOEXCEPT
...
#define __WIL_CFGMGR32_H_
    /// @endcond
    typedef unique_any<HCMNOTIFICATION, decltype(&::CM_Unregister_Notification), ::CM_Unregister_Notification>
...
#define __WIL_RESOURCE_WDM
    ...
    namespace details
    {
        struct kspin_lock_saved_irql
        {
            ...
            _IRQL_requires_(DISPATCH_LEVEL) static void Release(
                _In_ _IRQL_restores_ const kspin_lock_saved_irql &spinLockSavedIrql)
            {
        ...
        inline _IRQL_requires_min_(DISPATCH_LEVEL) void __stdcall ReleaseSpinLockFromDpcLevel(
            _Inout_ PKSPIN_LOCK spinLock) WI_NOEXCEPT
        {
    ...
    using kspin_lock_guard = unique_any<PKSPIN_LOCK, decltype(details::kspin_lock_saved_irql::Release),
                                        &details::kspin_lock_saved_irql::Release, details::pointer_access_none,
                                        details::kspin_lock_saved_irql>;

    using kspin_lock_at_dpc_guard = unique_any<PKSPIN_LOCK, decltype(details::ReleaseSpinLockFromDpcLevel),
                                               &details::ReleaseSpinLockFromDpcLevel, details::pointer_access_none>;

    WI_NODISCARD
    inline _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_saves_ _IRQL_raises_(DISPATCH_LEVEL) kspin_lock_guard
    ...
    WI_NODISCARD
    inline _IRQL_requires_min_(DISPATCH_LEVEL) kspin_lock_at_dpc_guard
    ...
    class kernel_spin_lock
    {
      public:
        ...
        WI_NODISCARD
        _IRQL_requires_max_(DISPATCH_LEVEL) _IRQL_saves_ _IRQL_raises_(DISPATCH_LEVEL) kspin_lock_guard
            acquire() WI_NOEXCEPT
        {
        ...
        WI_NODISCARD
        _IRQL_requires_min_(DISPATCH_LEVEL) kspin_lock_at_dpc_guard acquire_at_dpc() WI_NOEXCEPT
        {
    ...
    namespace details
    {
        template <EVENT_TYPE eventType> class kernel_event_t
        {
          public:
    ...
    */
    ...
    using fast_mutex_guard =
        unique_any<FAST_MUTEX *, decltype(::ExReleaseFastMutex), &::ExReleaseFastMutex, details::pointer_access_none>;

    WI_NODISCARD
    inline _IRQL_requires_max_(APC_LEVEL) fast_mutex_guard acquire_fast_mutex(FAST_MUTEX *fastMutex) WI_NOEXCEPT
    {
    ...
    WI_NODISCARD
    inline _IRQL_requires_max_(APC_LEVEL) fast_mutex_guard try_acquire_fast_mutex(FAST_MUTEX *fastMutex) WI_NOEXCEPT
    {
    ...
    class fast_mutex
    {
      public:
        ...
        // destruction.
        WI_NODISCARD
        _IRQL_requires_max_(APC_LEVEL) fast_mutex_guard acquire() WI_NOEXCEPT
        {
        ...
        WI_NODISCARD
        _IRQL_requires_max_(APC_LEVEL) fast_mutex_guard try_acquire() WI_NOEXCEPT
        {
    ...
    namespace details
    {
        _IRQL_requires_max_(APC_LEVEL) inline void release_fast_mutex_with_critical_region(FAST_MUTEX *fastMutex)
            WI_NOEXCEPT
        {
    ...
    using fast_mutex_with_critical_region_guard =
        unique_any<FAST_MUTEX *, decltype(details::release_fast_mutex_with_critical_region),
                   &details::release_fast_mutex_with_critical_region, details::pointer_access_none>;

    WI_NODISCARD
    inline _IRQL_requires_max_(APC_LEVEL) fast_mutex_with_critical_region_guard
        acquire_fast_mutex_with_critical_region(FAST_MUTEX *fastMutex) WI_NOEXCEPT
    {
    ...
    class fast_mutex_with_critical_region
    {
      public:
        ...
        WI_NODISCARD
        _IRQL_requires_max_(APC_LEVEL) fast_mutex_with_critical_region_guard acquire() WI_NOEXCEPT
        {
...
//! WDM version of EX_PUSH_LOCK is available starting with Windows 10 1809
    ...
    namespace details
    {
        _IRQL_requires_max_(APC_LEVEL) inline void release_push_lock_exclusive(EX_PUSH_LOCK *pushLock) WI_NOEXCEPT
        {
            ::ExReleasePushLockExclusive(pushLock);
            ::KeLeaveCriticalRegion();
        }

        _IRQL_requires_max_(APC_LEVEL) inline void release_push_lock_shared(EX_PUSH_LOCK *pushLock) WI_NOEXCEPT
        {
    ...
    using push_lock_exclusive_guard =
        unique_any<EX_PUSH_LOCK *, decltype(&details::release_push_lock_exclusive),
                   &details::release_push_lock_exclusive, details::pointer_access_noaddress>;

    using push_lock_shared_guard = unique_any<EX_PUSH_LOCK *, decltype(&details::release_push_lock_shared),
                                              &details::release_push_lock_shared, details::pointer_access_noaddress>;

    WI_NODISCARD
    inline _IRQL_requires_max_(APC_LEVEL) push_lock_exclusive_guard
        acquire_push_lock_exclusive(EX_PUSH_LOCK *pushLock) WI_NOEXCEPT
    {
        ::KeEnterCriticalRegion();
    ...
    WI_NODISCARD
    inline _IRQL_requires_max_(APC_LEVEL) push_lock_shared_guard
        acquire_push_lock_shared(EX_PUSH_LOCK *pushLock) WI_NOEXCEPT
    {
        ::KeEnterCriticalRegion();
    ...
    class push_lock
    {
      public:
        ...
        WI_NODISCARD
        _IRQL_requires_max_(APC_LEVEL) push_lock_exclusive_guard acquire_exclusive() WI_NOEXCEPT
        {
            return acquire_push_lock_exclusive(&m_pushLock);
        }

        WI_NODISCARD
        _IRQL_requires_max_(APC_LEVEL) push_lock_shared_guard acquire_shared() WI_NOEXCEPT
        {
    ...
    namespace details
    {
        ...
        template <typename pointer, ULONG tag> struct pool_helpers
        {
            static inline _IRQL_requires_max_(DISPATCH_LEVEL) void __stdcall FreePoolWithTag(pointer value) WI_NOEXCEPT
            {
                if (value)
                {
    ...
    using unique_tagged_pool_ptr = unique_any<pointer, decltype(details::pool_helpers<pointer, tag>::FreePoolWithTag),
                                              &details::pool_helpers<pointer, tag>::FreePoolWithTag>;

    // For use with IRPs that need to be IoFreeIrp'ed when done, typically allocated using IoAllocateIrp.
    using unique_allocated_irp =
        wil::unique_any<PIRP, decltype(&::IoFreeIrp), ::IoFreeIrp, details::pointer_access_noaddress>;
...
#if (defined(WINTRUST_H) && defined(SOFTPUB_H) && WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP) &&                 \
     !defined(__WIL_WINTRUST)) ||                                                                                      \
    defined(WIL_DOXYGEN)
/// @cond
#define __WIL_WINTRUST
    ...
    /// @endcond
    typedef wil::unique_struct<WINTRUST_DATA, decltype(&details::CloseWintrustData), details::CloseWintrustData>
        unique_wintrust_data;
...
#define __WIL_MSCAT
    namespace details
    {
        inline void __stdcall CryptCATAdminReleaseContextNoFlags(_Pre_opt_valid_ _Frees_ptr_opt_ HCATADMIN handle)
            WI_NOEXCEPT
        {
    ...
    /// @endcond
    typedef wil::unique_any<HCATADMIN, decltype(&details::CryptCATAdminReleaseContextNoFlags),
                            details::CryptCATAdminReleaseContextNoFlags>
        unique_hcatadmin;
    ...
    namespace details
    {
        class crypt_catalog_enumerator
        {
            ...
            bool move_next() WI_NOEXCEPT
            {
                ...
                m_hCatInfo.reset(::CryptCATAdminEnumCatalogFromHash(hcatadmin().get(), const_cast<BYTE *>(m_hash),
                                                                    m_hashLen, 0, &prevCatInfo));
                return !!m_hCatInfo;
            }
    ...
    inline details::crypt_catalog_enumerator make_crypt_catalog_enumerator(wil::shared_hcatadmin &hCatAdmin,
                                                                           _In_count_(hashLen) const BYTE *hash,
...
#define __WIL_RESOURCE_LOCK_ENFORCEMENT
    ...
    namespace details
    {
        ...
        // read_lock_required to be constructed. The allows_exclusive value indicates if the type represents an
        // exclusive, write-safe lock acquisition, or a shared, read-only lock acquisition.
        template <typename T> struct lock_proof_traits
        {
        };
    ...
    */
    struct write_lock_required
    {
        ...
        */
        ...
        write_lock_required(const TLockProof &,
                            wistd::enable_if_t<details::lock_proof_traits<TLockProof>::allows_exclusive, int> = 0)
        {
        }
    ...
    */
    struct read_lock_required
    {
        ...
        */
        template <typename TLockProof>
        read_lock_required(const TLockProof &,
                           wistd::enable_if_t<details::lock_proof_traits<TLockProof>::allows_shared, int> = 0)
        {
        }
...
#define __WIL__RESOURCE_LOCKPROOF_WINBASE
    ...
    namespace details
    {
        ...
        template <> struct lock_proof_traits<rwlock_release_shared_scope_exit> : shared_lock_proof
        {
        };

        template <> struct lock_proof_traits<rwlock_release_exclusive_scope_exit> : exclusive_lock_proof
        {
        };

        // Specialization for critical_section
        template <> struct lock_proof_traits<cs_leave_scope_exit> : exclusive_lock_proof
        {
        };
    } // namespace details
    /// @endcond
...
#define __WIL__RESOURCE_LOCKPROOF_MUTEX
    ...
    namespace details
    {
        template <typename TMutex> struct lock_proof_traits<std::unique_lock<TMutex>> : exclusive_lock_proof
        {
        };

        template <typename TMutex> struct lock_proof_traits<std::lock_guard<TMutex>> : exclusive_lock_proof
        {
        };
    } // namespace details
    /// @endcond
...
#if defined(_SHARED_MUTEX_) && !defined(__WIL__RESOURCE_LOCKPROOF_SHAREDMUTEX) &&                                      \
    defined(__WIL_RESOURCE_LOCK_ENFORCEMENT)
#define __WIL__RESOURCE_LOCKPROOF_SHAREDMUTEX

    /// @cond
    namespace details
    {
        template <typename TMutex> struct lock_proof_traits<std::shared_lock<TMutex>> : shared_lock_proof
        {
        };
    } // namespace details
    /// @endcond
