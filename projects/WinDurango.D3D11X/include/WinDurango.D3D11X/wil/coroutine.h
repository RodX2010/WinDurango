...
namespace wil
{
    ...
    template <typename T> struct com_task;
} // namespace wil
...
namespace wil::details::coro
{
    ...
    __declspec(selectany) void(__stdcall *g_pfnRestoreRestrictedErrorInformation)(void *restricted_error)
        WI_PFN_NOEXCEPT = nullptr;
    __declspec(selectany) void(__stdcall *g_pfnDestroyRestrictedErrorInformation)(void *restricted_error)
        WI_PFN_NOEXCEPT = nullptr;
    ...
    template <typename T> struct result_wrapper
    {
        T value;
        T get_value()
        {
    ...
    template <> struct result_wrapper<void>
    {
        void get_value()
        {
        }
    ...
    template <typename T> struct task_promise : promise_base<T>
    {
    ...
    template <> struct task_promise<void> : promise_base<void>
    {
        void return_void()
        {
            this->emplace_value();
    ...
    template <typename T> struct promise_deleter
    {
    ...
    template <typename T> using promise_ptr = wistd::unique_ptr<promise_base<T>, promise_deleter<T>>;

    template <typename T> struct agile_awaiter
    {
    ...
    template <typename T> struct task_base
    {
      ...
      protected:
        ...
        template <typename D> D &assign(D *self, task_base &&other) noexcept
        {
...
namespace wil
{
    ...
    template <typename T> struct task : details::coro::task_base<T>
    {
        using base = details::coro::task_base<T>;
        // Constructing from task_promise<T>* cannot be explicit because get_return_object relies on implicit
        // conversion.
    ...
    template <typename T> struct com_task : details::coro::task_base<T>
    {
        using base = details::coro::task_base<T>;
        // Constructing from task_promise<T>* cannot be explicit because get_return_object relies on implicit
        // conversion.
    ...
    template <typename T> task(com_task<T> &&) -> task<T>;
    template <typename T> com_task(task<T> &&) -> com_task<T>;
} // namespace wil

template <typename T, typename... Args> struct __WI_COROUTINE_NAMESPACE::coroutine_traits<wil::task<T>, Args...>
{
    using promise_type = wil::details::coro::task_promise<T>;
};

template <typename T, typename... Args> struct __WI_COROUTINE_NAMESPACE::coroutine_traits<wil::com_task<T>, Args...>
{
    using promise_type = wil::details::coro::task_promise<T>;
};
...
namespace wil::details::coro
{
    template <typename T> decltype(auto) task_base<T>::get() &&
    {
    ...
    template <typename T> void __stdcall task_base<T>::wake_by_address(void *completed)
    {
...
namespace wil::details::coro
{
    ...
    struct apartment_resumer
    {
        ...
        void resume_context_sync()
        {
            ...
            auto result = local_context->ContextCallback(resume_apartment_callback, &data,
    ...
    template <typename T> struct com_awaiter : agile_awaiter<T>
    {
    ...
    template <typename T> auto task_base<T>::resume_same_apartment() && noexcept
    {
...
WI_HEADER_INITIALIZATION_FUNCTION(CoroutineRestrictedErrorInitialize, [] {
    ::wil::details::coro::g_pfnCaptureRestrictedErrorInformation =
        ::wil::details::coro::CaptureRestrictedErrorInformation;
    ::wil::details::coro::g_pfnRestoreRestrictedErrorInformation =
        ::wil::details::coro::RestoreRestrictedErrorInformation;
    ::wil::details::coro::g_pfnDestroyRestrictedErrorInformation =
        ::wil::details::coro::DestroyRestrictedErrorInformation;
    return 1;
})
