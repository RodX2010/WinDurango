...
#ifndef RESULT_DEBUG
#if (DBG || defined(DEBUG) || defined(_DEBUG)) && !defined(RESULT_NO_DEBUG) &&                                         \
    (defined(WIL_KERNEL_MODE) || !defined(NDEBUG))
#define RESULT_DEBUG
#endif
#endif
...
#define WI_ASSERT(condition)                                                                                           \
    (__WI_ANALYSIS_ASSUME(condition), ((!(condition)) ? (WI_ASSERT_FAIL(#condition), FALSE) : TRUE))
#define WI_ASSERT_MSG(condition, msg)                                                                                  \
    (__WI_ANALYSIS_ASSUME(condition),                                                                                  \
     ((!(condition)) ? (__WI_ASSERT_FAIL_ANNOTATION(L##msg), DbgRaiseAssertionFailure(), FALSE) : TRUE))
...
#define WI_VERIFY_SUCCEEDED(condition)                                                                                 \
    (__WI_ANALYSIS_ASSUME(SUCCEEDED(condition)), ((SUCCEEDED(condition)) ? TRUE : FALSE))
#endif // RESULT_DEBUG
...
#define __NTSTATUS_FROM_WIN32(x)                                                                                       \
    ((NTSTATUS)(x) <= 0 ? ((NTSTATUS)(x))                                                                              \
                        : ((NTSTATUS)(((x) & 0x0000FFFF) | (FACILITY_WIN32 << 16) | ERROR_SEVERITY_ERROR)))
#endif
...
#define __R_INFO_NOFILE_ONLY(CODE)                                                                                     \
    __R_IF_CALLERADDRESS(_ReturnAddress() __R_IF_COMMA)                                                                \
    __R_IF_LINE(__R_LINE_VALUE)                                                                                        \
    __R_IF_FILE(__R_COMMA "wil") __R_IF_FUNCTION(__R_COMMA __FUNCTION__) __R_IF_CODE(__R_COMMA CODE)
...
#define __R_FN_CALL_ONLY                                                                                               \
    ...
    __R_IF_FILE(__R_COMMA fileName) __R_IF_FUNCTION(__R_COMMA functionName) __R_IF_CODE(__R_COMMA code)
...
#define __R_FN_UNREFERENCED                                                                                            \
    ...
    __R_IF_FILE((void)fileName;) __R_IF_FUNCTION((void)functionName;) __R_IF_CODE((void)code;)
// 1) Direct Methods
...
#define __R_DIRECT_METHOD(RetType, MethodName)                                                                         \
    template <unsigned int optimizerCounter> inline __declspec(noinline) RetType MethodName
#define __R_DIRECT_NORET_METHOD(RetType, MethodName)                                                                   \
    template <unsigned int optimizerCounter> inline __declspec(noinline) RESULT_NORETURN RetType MethodName
#else
...
//      * 'inline' when (RESULT_INLINE_ERROR_TESTS = 0 and RESULT_DIAGNOSTICS_LEVEL != 1), otherwise noinline (directly
//      called by code when branching is forceinlined)
//      * May be template-driven to create unique call sites if (RESULT_DIAGNOSTICS_LEVEL == 1 and
//      RESULT_INLINE_ERROR_TESTS = 1)
#if (RESULT_DIAGNOSTICS_LEVEL == 1)
...
#define __R_INTERNAL_INLINE_METHOD(MethodName)                                                                         \
    template <unsigned int optimizerCounter> inline __declspec(noinline) void MethodName
#define __R_INTERNAL_INLINE_NORET_METHOD(MethodName)                                                                   \
    template <unsigned int optimizerCounter> inline __declspec(noinline) RESULT_NORETURN void MethodName
...
#define __R_CONDITIONAL_INLINE_METHOD(RetType, MethodName)                                                             \
    template <unsigned int optimizerCounter> __forceinline RetType MethodName
...
#define __R_NS_ASSEMBLE2(ri, rd)                                                                                       \
    in##ri##diag##rd // Differing internal namespaces eliminate ODR violations between modes
...
//       RESULT_DIAGNOSTICS_LEVEL_FAIL_FAST), (RESULT_INLINE_ERROR_TESTS -> RESULT_INLINE_ERROR_TESTS_FAIL_FAST) and
//       (RESULT_INCLUDE_CALLER_RETURNADDRESS -> RESULT_INCLUDE_CALLER_RETURNADDRESS_FAIL_FAST)
#define __RFF_COMMA ,
...
#define __RFF_INFO_ONLY(CODE)                                                                                          \
    ...
    __RFF_IF_FUNCTION(__RFF_COMMA __FUNCTION__) __RFF_IF_CODE(__RFF_COMMA CODE)
...
// 2) Internal Methods
//      * Only called by Conditional routines
//      * 'inline' when (RESULT_INLINE_ERROR_TESTS_FAIL_FAST = 0 and RESULT_DIAGNOSTICS_LEVEL_FAIL_FAST != 1), otherwise
//      noinline (directly called by code when branching is forceinlined)
...
// Macro call-site helpers
#define __RFF_NS_ASSEMBLE2(ri, rd)                                                                                     \
    in##ri##diag##rd // Differing internal namespaces eliminate ODR violations between modes
...
// Conditionally returns failures (HRESULT) - use for failures that are expected in common use - failures are not logged
// - macros are only for control flow pattern
...
#define LOG_IF_WIN32_BOOL_FALSE(win32BOOL)                                                                             \
    __R_FN(Log_IfWin32BoolFalse)(__R_INFO(#win32BOOL) wil::verify_BOOL(win32BOOL))
...
#define LOG_HR_IF(hr, condition)                                                                                       \
    __R_FN(Log_HrIf)(__R_INFO(#condition) wil::verify_hresult(hr), wil::verify_bool(condition))
...
#define LOG_HR_MSG(hr, fmt, ...)                                                                                       \
    __R_FN(Log_HrMsg)(__R_INFO(#hr) wil::verify_hresult(hr), __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
#define LOG_LAST_ERROR_MSG(fmt, ...)                                                                                   \
    __R_FN(Log_GetLastErrorMsg)(__R_INFO(nullptr) __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define LOG_IF_WIN32_BOOL_FALSE_MSG(win32BOOL, fmt, ...)                                                               \
    __R_FN(Log_IfWin32BoolFalseMsg)(__R_INFO(#win32BOOL) wil::verify_BOOL(win32BOOL),                                  \
                                    __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define LOG_LAST_ERROR_IF_MSG(condition, fmt, ...)                                                                     \
    __R_FN(Log_GetLastErrorIfMsg)(__R_INFO(#condition) wil::verify_bool(condition),                                    \
                                  __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define LOG_IF_FAILED_WITH_EXPECTED(hr, hrExpected, ...)                                                               \
    ...
    (__R_INFO(#hr) wil::verify_hresult(hr), WI_ARGS_COUNT(__VA_ARGS__) + 1,                                            \
...
#define FAIL_FAST_LAST_ERROR_IF(condition)                                                                             \
    __RFF_FN(FailFast_GetLastErrorIf)(__RFF_INFO(#condition) wil::verify_bool(condition))
...
#define FAIL_FAST_HR_IF_MSG(hr, condition, fmt, ...)                                                                   \
    ...
    (__RFF_INFO(#condition) wil::verify_hresult(hr), wil::verify_bool(condition),                                      \
     __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
#define FAIL_FAST_HR_IF_NULL_MSG(hr, ptr, fmt, ...)                                                                    \
    __RFF_FN(FailFast_HrIfNullMsg)(__RFF_INFO(#ptr) wil::verify_hresult(hr), ptr,                                      \
                                   __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define FAIL_FAST_MSG(fmt, ...)                                                                                        \
    __RFF_FN(FailFast_UnexpectedMsg)(__RFF_INFO(nullptr) __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define THROW_IF_WIN32_BOOL_FALSE(win32BOOL)                                                                           \
    __R_FN(Throw_IfWin32BoolFalse)(__R_INFO(#win32BOOL) wil::verify_BOOL(win32BOOL))
...
#define THROW_HR_IF(hr, condition)                                                                                     \
    __R_FN(Throw_HrIf)(__R_INFO(#condition) wil::verify_hresult(hr), wil::verify_bool(condition))
...
#define THROW_LAST_ERROR_MSG(fmt, ...)                                                                                 \
    __R_FN(Throw_GetLastErrorMsg)(__R_INFO(nullptr) __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define THROW_EXCEPTION_MSG(exception, fmt, ...)                                                                       \
    wil::details::ReportFailure_CustomExceptionMsg(__R_INFO(#exception) exception,                                     \
                                                   __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define THROW_IF_WIN32_BOOL_FALSE_MSG(win32BOOL, fmt, ...)                                                             \
    __R_FN(Throw_IfWin32BoolFalseMsg)(__R_INFO(#win32BOOL) wil::verify_BOOL(win32BOOL),                                \
                                      __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
#define THROW_WIN32_IF_MSG(win32err, condition, fmt, ...)                                                              \
    ...
    (__R_INFO(#condition) wil::verify_win32(win32err), wil::verify_bool(condition),                                    \
     __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
#define THROW_LAST_ERROR_IF_MSG(condition, fmt, ...)                                                                   \
    __R_FN(Throw_GetLastErrorIfMsg)(__R_INFO(#condition) wil::verify_bool(condition),                                  \
                                    __WI_CHECK_MSG_FMT(fmt, ##__VA_ARGS__))
...
// Use CATCH_LOG_RETURN instead of CATCH_LOG in a function-try block around a destructor.  CATCH_LOG in this specific
// case has an implicit throw at the end of scope. Due to a bug (DevDiv 441931), Warning 4297 (function marked noexcept
// throws exception) is detected even when the throwing code is unreachable, such as the end of scope after a return, in
...
#define WI_USAGE_ERROR_FORWARD(msg, ...)                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        ReportFailure_ReplaceMsg<FailureType::Log>(__R_FN_CALL_FULL, HRESULT_FROM_WIN32(ERROR_ASSERTION_FAILURE), msg, \
                                                   ##__VA_ARGS__);                                                     \
        WI_USAGE_ASSERT_STOP(false);                                                                                   \
    } while ((void)0, 0)
#else
...
namespace wil
{
    ...
    enum class SupportedExceptions
    {
        ...
        ThrownOrAlloc //!< [ThrownOrAlloc] exceptions thrown by wil (Platform::Exception^ or ResultException) or
                      //!< std::bad_alloc.
    };
    ...
    {
        long
            contextId; // incrementing ID for this call context (unique across an individual module load within process)
    ...
    struct FailureInfo
    {
        ...
        HRESULT hr;
        NTSTATUS status;
        long failureId;    // incrementing ID for this specific failure (unique across an individual module load within
                           // process)
    ...
    //! Note that typically wil hides diagnostics from users under the covers by passing them automatically to functions
    //! as parameters hidden behind a macro.  In some cases, the user needs to directly supply these, so this class
    //! provides the mechanism for that. We only use this for user-passed content as it can't be directly controlled by
    //! RESULT_DIAGNOSTICS_LEVEL to ensure there are no ODR violations (though that variable still controls what
    //! parameters within this structure would be available).
    ...
    namespace details
    {
        // 'FARPROC' is declared in such a way that it cannot safely be assumed cast-able to other function pointer
        // types. This function helps alleviate warnings that can arise from this
        template <typename FuncPtr> inline FuncPtr GetProcAddress(_In_ HMODULE module, _In_ LPCSTR procName) WI_NOEXCEPT
        {
    ...
    extern "C" __declspec(selectany) void(__stdcall *g_pfnResultLoggingCallback)(
        _Inout_ wil::FailureInfo *pFailure, _Inout_updates_opt_z_(cchDebugMessage) PWSTR pszDebugMessage,
        _Pre_satisfies_(cchDebugMessage > 0) size_t cchDebugMessage) WI_PFN_NOEXCEPT = nullptr;

    // [optional]
    // This can be explicitly set to control whether or not error messages will be output to OutputDebugString.  It can
    // also be set directly from within the debugger to force console logging for debugging purposes.
    ...
    // [optionally] Allows forcing WIL to believe a debugger is present. Useful for when a kernel debugger is attached
    // and ::IsDebuggerPresent returns false
    ...
    // [optionally] Set to false to a configure all THROW_XXX macros in C++/CX to throw ResultException rather than
    // Platform::Exception^
    __declspec(selectany) bool g_fResultThrowPlatformException = true;

    // [optionally] Set to false to a configure all CATCH_ and CAUGHT_ macros to NOT support (fail-fast) std::exception
    // based exceptions (other than std::bad_alloc and wil::ResultException)
    ...
    namespace details
    {
        ...
        _Success_(true) _Ret_range_(dest, destEnd) inline PWSTR
            LogStringPrintf(_Out_writes_to_ptr_(destEnd) _Always_(_Post_z_) PWSTR dest,
                            _Pre_satisfies_(destEnd >= dest) PCWSTR destEnd, _In_ _Printf_format_string_ PCWSTR format,
                            ...)
        {
            va_list argList;
                                       ...
                                       _In_ FailureInfo const &failure) WI_NOEXCEPT
    {
        ...
        {
            // older-form callback was a non-const FailureInfo*; conceptually this is const as callers should not be
            // modifying
        ...
        if (pszDest[0] == L'\0')
        {
            ...
            else
            {
                ...
                FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, failure.hr,
            ...
            {
                dest = details::LogStringPrintf(dest, destEnd, L"%hs(%u)\\%hs!%p: ", failure.pszFile,
                                                failure.uLineNumber, failure.pszModule, failure.returnAddress);
            }
            else
            {
            ...
            dest = details::LogStringPrintf(dest, destEnd, L"%hs(%d) tid(%x) %08X %ws", pszType, failure.cFailureCount,
                                            ::GetCurrentThreadId(), errorCode, szErrorText);

            if ((failure.pszMessage != nullptr) || (failure.pszCallContext != nullptr) ||
                (failure.pszFunction != nullptr))
            {
                ...
                {
                    dest =
                        details::LogStringPrintf(dest, destEnd, L"[%hs(%hs)]\n", failure.pszFunction, failure.pszCode);
                }
    ...
    namespace details
    {
        //! Interface used to wrap up code (generally a lambda or other functor) to run in an exception-managed context
        //! where exceptions or errors can be observed and logged.
        struct IFunctor
        {
        ...
        struct ResultStatus
        {
            ...
            {
                return {_failure.hr, _failure.status,
                        WI_IsFlagSet(_failure.flags, FailureFlags::NtStatus) ? Kind::NtStatus : Kind::HResult};
            }
            HRESULT hr = S_OK;
        ...
        __declspec(selectany) void(__stdcall *g_pfnTelemetryCallback)(
            bool alreadyReported, wil::FailureInfo const &failure) WI_PFN_NOEXCEPT = nullptr;

        // Result.h plug-in (WIL use only)
        __declspec(selectany) void(__stdcall *g_pfnNotifyFailure)(_Inout_ FailureInfo *pFailure)
            WI_PFN_NOEXCEPT = nullptr;
        __declspec(selectany) void(__stdcall *g_pfnGetContextAndNotifyFailure)(
            _Inout_ FailureInfo *pFailure, _Out_writes_(callContextStringLength) _Post_z_ PSTR callContextString,
            _Pre_satisfies_(callContextStringLength > 0) size_t callContextStringLength) WI_PFN_NOEXCEPT = nullptr;

        // Observe all errors flowing through the system with this callback (set with wil::SetResultLoggingCallback);
        // use with custom logging
        __declspec(selectany) void(__stdcall *g_pfnLoggingCallback)(wil::FailureInfo const &failure)
            WI_PFN_NOEXCEPT = nullptr;
        ...
        __declspec(selectany) bool(__stdcall *g_pfnGetModuleInformation)(void *address,
                                                                         _Out_opt_ unsigned int *addressOffset,
                                                                         _Out_writes_bytes_opt_(size) char *name,
                                                                         size_t size) WI_PFN_NOEXCEPT = nullptr;
        ...
        __declspec(selectany) void(__stdcall *g_pfnRaiseFailFastException)(PEXCEPTION_RECORD, PCONTEXT,
                                                                           DWORD) = nullptr;

        // Exception-based compiled additions
        __declspec(selectany) HRESULT(__stdcall *g_pfnRunFunctorWithExceptionFilter)(IFunctor &functor,
                                                                                     IFunctorHost &host,
                                                                                     void *returnAddress) = nullptr;
        ...
        extern "C" __declspec(selectany) void(__stdcall *g_pfnThrowPlatformException)(FailureInfo const &failure,
                                                                                      PCWSTR debugString) = nullptr;
        ...
        __declspec(selectany) _Always_(_Post_satisfies_(return < 0))
            HRESULT(__stdcall *g_pfnResultFromKnownExceptions_WinRt)(const DiagnosticsInfo &diagnostics,
                                                                     void *returnAddress, SupportedExceptions supported,
                                                                     IFunctor &functor) = nullptr;

        // Plugin to call RoOriginateError (WIL use only)
        __declspec(selectany) void(__stdcall *g_pfnOriginateCallback)(wil::FailureInfo const &failure)
            WI_PFN_NOEXCEPT = nullptr;

        // Plugin to call RoFailFastWithErrorContext (WIL use only)
        __declspec(selectany) void(__stdcall *g_pfnFailfastWithContextCallback)(wil::FailureInfo const &failure)
            WI_PFN_NOEXCEPT = nullptr;
            ...
            else if (!fetchedRtlDisownModuleHeapAllocation)
            {
                if (auto ntdllModule = ::GetModuleHandleW(L"ntdll.dll"))
                {
                    pfnRtlDisownModuleHeapAllocation =
                        details::GetProcAddress<decltype(pfnRtlDisownModuleHeapAllocation)>(
                            ntdllModule, "RtlDisownModuleHeapAllocation");
                }
        ...
        template <typename TFunctor> using functor_return_type = decltype((*static_cast<TFunctor *>(nullptr))());

        template <typename TFunctor> struct functor_wrapper_void : public IFunctor
        {
            TFunctor &&functor;
...
#pragma warning(pop)
        ...
        template <typename TFunctor> struct functor_wrapper_HRESULT : public IFunctor
        {
            TFunctor &&functor;
        ...
        template <typename TFunctor, typename TReturn> struct functor_wrapper_other : public IFunctor
        {
            TFunctor &&functor;
            TReturn &retVal;
            functor_wrapper_other(TFunctor &functor_, TReturn &retval_)
                : functor(wistd::forward<TFunctor>(functor_)), retVal(retval_)
            {
            }
#pragma warning(push)
#pragma warning(disable : 4702) // https://github.com/Microsoft/wil/issues/2
...
#pragma warning(pop)
        ...
        {
            template <typename TFunctor> using functor_wrapper = functor_wrapper_void<TFunctor>;
        };

        struct tag_return_HRESULT : public wistd::integral_constant<size_t, 1>
        {
            template <typename TFunctor> using functor_wrapper = functor_wrapper_HRESULT<TFunctor>;
        };
        ...
        template <ErrorReturn errorReturn, typename T> struct return_type
        {
            using type = tag_return_other;
        };

        template <> struct return_type<ErrorReturn::Auto, HRESULT>
        {
            using type = tag_return_HRESULT;
        };

        template <> struct return_type<ErrorReturn::Auto, void>
        {
            using type = tag_return_void;
        };

        template <> struct return_type<ErrorReturn::None, void>
        {
            using type = tag_return_void;
        };
        ...
        namespace __R_NS_NAME
        {
            ...
            _Post_satisfies_(return == hr)
                __R_DIRECT_METHOD(HRESULT, Log_HrMsg)(__R_DIRECT_FN_PARAMS HRESULT hr,
                                                      _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT;
            _Post_satisfies_(return == err)
                __R_DIRECT_METHOD(DWORD, Log_Win32Msg)(__R_DIRECT_FN_PARAMS DWORD err,
                                                       _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT;
        } // namespace __R_NS_NAME
        namespace __RFF_NS_NAME
        {
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(bool, FailFast_HrIf)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                              bool condition) WI_NOEXCEPT;
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(bool,
                                         FailFast_IfFalse)(__RFF_CONDITIONAL_FN_PARAMS bool condition) WI_NOEXCEPT;
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
        ...
        inline void LogFailure(__R_FN_PARAMS_FULL, FailureType type, const ResultStatus &resultPair,
                               _In_opt_ PCWSTR message, bool fWantDebugString,
                               ...
                               FailureFlags flags, _Out_ FailureInfo *failure) WI_NOEXCEPT;

        __declspec(noinline) inline void ReportFailure(__R_FN_PARAMS_FULL, FailureType type,
        ...
        __declspec(noinline) inline void ReportFailure_Base(__R_FN_PARAMS_FULL, const ResultStatus &resultPair,
                                                            _In_opt_ PCWSTR message = nullptr,
                                                            ReportFailureOptions options = ReportFailureOptions::None,
...
#define __FAIL_FAST_ASSERT_WIN32_BOOL_FALSE__(condition)                                                               \
    __RFF_FN(FailFast_IfWin32BoolFalse)(__RFF_INFO(#condition) wil::verify_BOOL(condition))
        ...
        inline shared_buffer make_shared_buffer_nothrow(_In_reads_bytes_opt_(countBytes) void *pData,
                                                        size_t countBytes) WI_NOEXCEPT
        {
        ...
        // A small mimic of the STL shared_ptr class, but unlike shared_ptr, a pointer is not attached to the class, but
        // is always simply contained within (it cannot be attached or detached).

        template <typename object_t> class shared_object
        {
          public:
            ...
            template <typename param_t> bool create(param_t &&param1)
            {
        ...
        __WI_CLANG_DISABLE_WARNING(-Wunused - but - set -
                                   variable) // s_hrErrorLast used for debugging. We intentionally only assign to it
#endif
        __WI_MSVC_DISABLE_WARNING(4746) // s_hrErrorLast' is subject to /volatile:<iso|ms> setting; consider using
                                        // __iso_volatile_load/store intrinsic functions
        ...
        inline RESULT_NORETURN void __stdcall WilRaiseFailFastException(_In_ PEXCEPTION_RECORD er, _In_opt_ PCONTEXT cr,
                                                                        _In_ DWORD flags)
        {
            // if we managed to load the pointer either through WilDynamicRaiseFailFastException (PARTITION_DESKTOP
            // etc.) or via direct linkage (e.g. UWP apps), then use it.
        ...
        inline bool __stdcall GetModuleInformation(_In_opt_ void *address, _Out_opt_ unsigned int *addressOffset,
                                                   _Out_writes_bytes_opt_(size) char *name, size_t size) WI_NOEXCEPT
        {
            HMODULE hModule = nullptr;
            if (address && !GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                                                   GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                                               reinterpret_cast<PCWSTR>(address), &hModule))
            {
            ...
            if (addressOffset)
            {
                *addressOffset = address ? static_cast<unsigned int>(static_cast<unsigned char *>(address) -
                                                                     reinterpret_cast<unsigned char *>(hModule))
                                         : 0;
            }
            if (name)
            {
        ...
        __WI_PUSH_WARNINGS
        __WI_MSVC_DISABLE_WARNING(4746) // s_fModuleValid' is subject to /volatile:<iso|ms> setting; consider using
                                        // __iso_volatile_load/store intrinsic functions
        inline PCSTR __stdcall GetCurrentModuleName() WI_NOEXCEPT
        {
            ...
            {
                GetModuleInformation(reinterpret_cast<void *>(&RecordFailFast), nullptr, s_szModule,
                                     ARRAYSIZE(s_szModule));
                s_fModuleValid = true;
            }
            return s_szModule;
        }
        ...
        inline void __stdcall WilDynamicLoadRaiseFailFastException(_In_ PEXCEPTION_RECORD er, _In_ PCONTEXT cr,
                                                                   _In_ DWORD flags)
        {
            auto k32handle = GetModuleHandleW(L"kernelbase.dll");
            _Analysis_assume_(k32handle != nullptr);
            auto pfnRaiseFailFastException = details::GetProcAddress<decltype(WilDynamicLoadRaiseFailFastException) *>(
                k32handle, "RaiseFailFastException");
        ...
        inline bool __stdcall GetModuleInformationFromAddress(_In_opt_ void *address,
                                                              _Out_opt_ unsigned int *addressOffset,
                                                              _Out_writes_bytes_opt_(size) char *buffer,
                                                              size_t size) WI_NOEXCEPT
        {
            if (size > 0)
            {
        ...
        __declspec(noinline) inline HRESULT NtStatusToHr(NTSTATUS status) WI_NOEXCEPT
        {
            ...
            {
                // RtlNtStatusToDosErrorNoTeb maps STATUS_NO_MEMORY to the less popular of two Win32 no memory error
                // codes resulting in an unexpected mapping
                return E_OUTOFMEMORY;
            }
            ...
            if (g_pfnRtlNtStatusToDosErrorNoTeb != nullptr)
            {
                ...
                // ERROR_MR_MID_NOT_FOUND indicates a bug in the originator of the error (failure to add a mapping to
                // the Win32 error codes). There are known instances of this bug which are unlikely to be fixed soon,
                // and it's always possible that additional instances could be added in the future. In these cases, it's
                // better to use HRESULT_FROM_NT rather than returning a meaningless error.
        ...
        // The following set of functions all differ only based upon number of arguments.  They are unified in their
        // handling of data from each of the various error-handling types (fast fail, exceptions, etc.).
        _Post_equals_last_error_ inline DWORD GetLastErrorFail(__R_FN_PARAMS_FULL) WI_NOEXCEPT
        {
            ...
            if (SUCCEEDED_WIN32(err))
            {
                ...
                //  1) Your code is using a macro (such as RETURN_IF_WIN32_BOOL_FALSE()) on a function that does not
                //  actually
                //      set the last error (consult MSDN).
                //  2) Your macro check against the error is not immediately after the API call.  Pushing it later can
                //  result
                //      in another API call between the previous one and the check resetting the last error.
                //  3) The API you're calling has a bug in it and does not accurately set the last error (there are a
                //  few
                //      examples here, such as SendMessageTimeout() that don't accurately set the last error).
                //      [MSFT internal] For these, please send mail to 'wildisc' when found and work-around with
                //      win32errorhelpers.

                WI_USAGE_ERROR_FORWARD(
                    "CALLER BUG: Macro usage error detected.  GetLastError() does not have an error.");
...
#pragma warning(disable : __WARNING_RETURNING_BAD_RESULT)
        // NOTE: The following two functions are unfortunate copies of strsafe.h functions that have been copied to
        // reduce the friction associated with using Result.h and ResultException.h in a build that does not have
        // WINAPI_PARTITION_DESKTOP defined (where these are conditionally enabled).

        inline HRESULT WilStringLengthWorkerA(_In_reads_or_z_(cchMax) PCNZCH psz,
        ...
        _Must_inspect_result_ inline HRESULT StringCchLengthA(_In_reads_or_z_(cchMax) PCNZCH psz,
...
#pragma warning(pop)
        ...
        _Post_satisfies_(cchDest > 0 && cchDest <= cchMax) inline HRESULT
            WilStringValidateDestA(_In_reads_opt_(cchDest) PCNZCH /*pszDest*/, _In_ size_t cchDest,
        ...
        inline HRESULT WilStringVPrintfWorkerA(_Out_writes_(cchDest) _Always_(_Post_z_) STRSAFE_LPSTR pszDest,
                                               _In_ _In_range_(1, STRSAFE_MAX_CCH) size_t cchDest,
                                               _Always_(_Out_opt_ _Deref_out_range_(<=, cchDest - 1))
...
#pragma warning(pop)
        ...
        inline HRESULT StringCchPrintfA(_Out_writes_(cchDest) _Always_(_Post_z_) STRSAFE_LPSTR pszDest,
                                        _In_ size_t cchDest, _In_ _Printf_format_string_ STRSAFE_LPCSTR pszFormat, ...)
        {
            HRESULT hr;
        ...
        _Ret_range_(sizeof(wchar_t),
                    (psz == nullptr) ? sizeof(wchar_t) : ((_String_length_(psz) + 1) * sizeof(wchar_t))) inline size_t
        ...
        _Ret_range_(pStart, pEnd) inline unsigned char *WriteResultString(
            _Pre_satisfies_(pStart <= pEnd) _When_((pStart == pEnd) || (pszString == nullptr) || (pszString[0] == 0),
                                                   _In_opt_)
                _When_((pStart != pEnd) && (pszString != nullptr) && (pszString[0] != 0),
                       _Out_writes_bytes_opt_(_String_length_(pszString) * sizeof(pszString[0]))) unsigned char *pStart,
            _Pre_satisfies_(pEnd >= pStart) unsigned char *pEnd, _In_opt_z_ TString pszString,
            _Outptr_result_maybenull_z_ TString *ppszBufferString)
        {
            ...
            assign_to_opt_param(
                ...
                reinterpret_cast<TString>(
                    pStart)); // lgtm[cpp/incorrect-string-type-conversion] False positive - The query is
                              // misinterpreting a buffer (char *) with a MBS string, the cast to TString is expected.
            return pStart + stringSize;
        }

        _Ret_range_(0, (cchMax > 0) ? cchMax - 1 : 0) inline size_t
            UntrustedStringLength(_In_ PCSTR psz, _In_ size_t cchMax)
        {
            size_t cbLength;
            return SUCCEEDED(wil::details::StringCchLengthA(psz, cchMax, &cbLength)) ? cbLength : 0;
        }
        _Ret_range_(0, (cchMax > 0) ? cchMax - 1 : 0) inline size_t
            UntrustedStringLength(_In_ PCWSTR psz, _In_ size_t cchMax)
        {
            size_t cbLength;
        ...
        template <typename TString>
        _Ret_range_(pStart,
                    pEnd) inline unsigned char *GetResultString(_In_reads_to_ptr_opt_(pEnd) unsigned char *pStart,
                                                                _Pre_satisfies_(pEnd >= pStart) unsigned char *pEnd,
                                                                _Out_ TString *ppszBufferString)
        {
            size_t cchLen = UntrustedStringLength(reinterpret_cast<TString>(pStart),
                                                  (pEnd - pStart) / sizeof((*ppszBufferString)[0]));
...
#if WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP | WINAPI_PARTITION_SYSTEM)
    //! Call this method to initialize WIL manually in a module where RESULT_SUPPRESS_STATIC_INITIALIZERS is required.
    //! WIL will only use publicly documented APIs.
...
/// @endcond
    ...
    inline bool ProcessShutdownInProgress()
    {
        return (details::g_processShutdownInProgress ||
                (details::g_pfnDllShutdownInProgress ? details::g_pfnDllShutdownInProgress() : false));
    }

    /** Use this object to wrap an object that wants to prevent its destructor from being run when the process is
    shutting down, but the hosting DLL doesn't support CRT initializers (such as kernelbase.dll).  The hosting DLL is
    responsible for calling Construct() and Destroy() to manually run the constructor and destructor during DLL load &
    unload. Upon process shutdown a method (ProcessShutdown()) is called that must be implemented on the object,
    otherwise the destructor is called as is typical. */
    template <class T> class manually_managed_shutdown_aware_object
    {
      public:
    ...
    /** Use this object to wrap an object that wants to prevent its destructor from being run when the process is
    shutting down. Upon process shutdown a method (ProcessShutdown()) is called that must be implemented on the object,
    otherwise the destructor is called as is typical. */
    template <class T> class shutdown_aware_object
    {
      public:
    ...
    /** Use this object to wrap an object that wants to prevent its destructor from being run when the process is
     * shutting down. */
    template <class T> class object_without_destructor_on_shutdown
    {
      public:
    ...
    inline void SetResultFromCaughtExceptionCallback(
        _In_opt_ decltype(wil::g_pfnResultFromCaughtException) callbackFunction)
    {
    ...
    inline void SetOriginateErrorCallback(_In_opt_ decltype(details::g_pfnOriginateCallback) callbackFunction)
    {
        ...
        __FAIL_FAST_IMMEDIATE_ASSERT__((details::g_pfnOriginateCallback == nullptr) || (callbackFunction == nullptr) ||
                                       (details::g_pfnOriginateCallback == callbackFunction));
        details::g_pfnOriginateCallback = callbackFunction;
    }

    // [optionally] Plug in failfast callback
    // This provides the ability for a module to call RoFailFastWithErrorContext in the failfast handler -if- there is
    // stowed exception data available.  Normally, the callback is owned by including result_originate.h in the
    // including module. Alternatively a module could re-route to its own implementation.
    inline void SetFailfastWithContextCallback(
        _In_opt_ decltype(details::g_pfnFailfastWithContextCallback) callbackFunction)
    {
        // Only ONE function can own the failfast with context callback
        __FAIL_FAST_IMMEDIATE_ASSERT__((details::g_pfnFailfastWithContextCallback == nullptr) ||
    ...
    class StoredFailureInfo
    {
      public:
        ...
        void SetFailureInfo(FailureInfo const &failure) WI_NOEXCEPT
        {
            ...
            size_t const cbNeed =
                details::ResultStringSize(failure.pszMessage) + details::ResultStringSize(failure.pszCode) +
            ...
            {
                pBuffer =
                    details::WriteResultString(pBuffer, pBufferEnd, failure.pszMessage, &m_failureInfo.pszMessage);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.pszCode, &m_failureInfo.pszCode);
                pBuffer =
                    details::WriteResultString(pBuffer, pBufferEnd, failure.pszFunction, &m_failureInfo.pszFunction);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.pszFile, &m_failureInfo.pszFile);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.pszCallContext,
                                                     &m_failureInfo.pszCallContext);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.pszModule, &m_failureInfo.pszModule);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.callContextCurrent.contextName,
                                                     &m_failureInfo.callContextCurrent.contextName);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.callContextCurrent.contextMessage,
                                                     &m_failureInfo.callContextCurrent.contextMessage);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.callContextOriginating.contextName,
                                                     &m_failureInfo.callContextOriginating.contextName);
                pBuffer = details::WriteResultString(pBuffer, pBufferEnd, failure.callContextOriginating.contextMessage,
    ...
    //! This class stores all of the FailureInfo context that is available when the exception is thrown.  It's also
    //! caught by exception guards for automatic conversion to HRESULT.
    //!
    //! In c++/cx, Platform::Exception^ is used instead of this class (unless @ref wil::g_fResultThrowPlatformException
    //! has been changed).
    ...
    //! Identical to 'throw;', but can be called from error-code neutral code to rethrow in code that *may* be running
    //! under an exception context
    ...
    namespace details
    {
        ...
        {
            // This is an intentional fail-fast that was caught by an exception guard with WIL.  Look back up the
            // callstack to determine the source of the actual exception being thrown.  The exception guard used by the
            // calling code did not expect this exception type to be thrown or is specifically requesting fail-fast for
            // this class of exception.

            FailureInfo failure{};
            WilFailFast(failure);
        }

        inline void MaybeGetExceptionString(const ResultException &exception,
                                            _Out_writes_opt_(debugStringChars) PWSTR debugString,
        ...
        inline void MaybeGetExceptionString(const std::exception &exception,
                                            _Out_writes_opt_(debugStringChars) PWSTR debugString,
                                            _When_(debugString != nullptr, _Pre_satisfies_(debugStringChars > 0))
                                                size_t debugStringChars)
        {
        ...
        inline HRESULT ResultFromKnownException(const ResultException &exception, const DiagnosticsInfo &diagnostics,
                                                void *returnAddress)
        {
            wchar_t message[2048]{};
            MaybeGetExceptionString(exception, message, ARRAYSIZE(message));
            auto hr = exception.GetErrorCode();
            wil::details::ReportFailure_Base<FailureType::Log>(__R_DIAGNOSTICS_RA(diagnostics, returnAddress),
                                                               ResultStatus::FromResult(hr), message);
            return hr;
        }

        inline HRESULT ResultFromKnownException(const std::bad_alloc &exception, const DiagnosticsInfo &diagnostics,
                                                void *returnAddress)
        {
            wchar_t message[2048]{};
            MaybeGetExceptionString(exception, message, ARRAYSIZE(message));
        ...
        inline HRESULT ResultFromKnownException(const std::exception &exception, const DiagnosticsInfo &diagnostics,
                                                void *returnAddress)
        {
            wchar_t message[2048]{};
            MaybeGetExceptionString(exception, message, ARRAYSIZE(message));
            constexpr auto hr = __HRESULT_FROM_WIN32(ERROR_UNHANDLED_EXCEPTION);
            ReportFailure_Base<FailureType::Log>(__R_DIAGNOSTICS_RA(diagnostics, returnAddress),
                                                 ResultStatus::FromResult(hr), message);
            return hr;
        }
        ...
        inline HRESULT RecognizeCaughtExceptionFromCallback(_Inout_updates_opt_(debugStringChars) PWSTR debugString,
                                                                ...
                                                                size_t debugStringChars)
        {
            ...
            // If we still don't know the error -- or we would like to get the debug string for the error (if possible)
            // we rethrow and catch std::exception.
    ...
    WI_HEADER_INITIALIZATION_FUNCTION(InitializeWinRt, [] {
            ...
            _Out_ bool *isNormalized) WI_NOEXCEPT
        {
            ...
            *isNormalized = false;
            ...
            {
                const auto hr =
                    details::g_pfnResultFromCaughtException_CppWinRt(debugString, debugStringChars, isNormalized);
                if (FAILED(hr))
                {
            ...
            {
                const auto hr =
                    details::g_pfnResultFromCaughtException_WinRt(debugString, debugStringChars, isNormalized);
                return ResultStatus::FromResult(hr);
            }
        ...
        // Runs the given functor, converting any exceptions of the supported types that are known to HRESULTs and
        // returning that HRESULT.  Does NOT attempt to catch unknown exceptions (which propagate).  Primarily used by
        // SEH exception handling techniques to stop at the point the exception is thrown.
        inline HRESULT ResultFromKnownExceptions(const DiagnosticsInfo &diagnostics, void *returnAddress,
                                                 SupportedExceptions supported, IFunctor &functor)
        {
            if (supported == SupportedExceptions::Default)
            {
                supported =
                    g_fResultSupportStdException ? SupportedExceptions::Known : SupportedExceptions::ThrownOrAlloc;
            }
        ...
        inline HRESULT ResultFromExceptionSeh(const DiagnosticsInfo &diagnostics, void *returnAddress,
                                              SupportedExceptions supported, IFunctor &functor) WI_NOEXCEPT
        {
            __try
            {
        ...
        __declspec(noinline) inline HRESULT ResultFromException(const DiagnosticsInfo &diagnostics,
                                                                SupportedExceptions supported,
                                                                IFunctor &functor) WI_NOEXCEPT
        {
#ifdef RESULT_DEBUG
            ...
            {
                return wil::details::ReportFailure_CaughtException<FailureType::Log>(__R_DIAGNOSTICS(diagnostics),
                                                                                     _ReturnAddress(), supported);
            }
        }

        __declspec(noinline) inline HRESULT ResultFromExceptionDebug(const DiagnosticsInfo &diagnostics,
                                                                     SupportedExceptions supported,
                                                                     IFunctor &functor) WI_NOEXCEPT
        {
            return wil::details::ResultFromExceptionSeh(diagnostics, _ReturnAddress(), supported, functor);
        ...
        inline HRESULT __stdcall RunFunctorWithExceptionFilter(IFunctor &functor, IFunctorHost &host,
                                                               void *returnAddress)
        {
            try
            {
            ...
            {
                // Note that the host may choose to re-throw, throw a normalized exception, return S_OK and eat the
                // exception or return the remapped failure.
    ...
    //!
    //! Note that an overload exists that does not report failures to telemetry at all.  This version should be
    //! preferred to that version.  Also note that neither of these versions are preferred over using try catch blocks
    //! to accomplish the same thing as they will be more efficient.
    //! ~~~~
    ...
    __forceinline HRESULT ResultFromException(const DiagnosticsInfo &diagnostics, SupportedExceptions supported,
                                              Functor &&functor) WI_NOEXCEPT
    {
        static_assert(details::functor_tag<ErrorReturn::None, Functor>::value != details::tag_return_other::value,
                      "Functor must return void or HRESULT");
    ...
    //! This overload uses SupportedExceptions::Known by default.  See @ref ResultFromException for more detailed
    //! information.
    template <typename Functor>
    ...
    //! @param functor  A lambda that accepts no parameters; any return value is ignored
    //! @return         S_OK on success (no exception thrown) or an error based upon the exception thrown
    template <typename Functor> inline HRESULT ResultFromException(Functor &&functor) WI_NOEXCEPT
    try
    {
        static_assert(details::functor_tag<ErrorReturn::None, Functor>::value == details::tag_return_void::value,
                      "Functor must return void");
        typename details::functor_tag<ErrorReturn::None, Functor>::template functor_wrapper<Functor> functorObject(
            wistd::forward<Functor>(functor));
    ...
    //! A lambda-based exception guard that can identify the origin of unknown exceptions and can vary the supported
    //! exception types. Functionally this is nearly identical to the corresponding @ref ResultFromException function
    //! with the exception that it utilizes structured exception handling internally to be able to terminate at the
    //! point where a unknown exception is thrown, rather than after that unknown exception has been unwound.  Though
    //! less efficient, this leads to a better debugging experience when analyzing unknown exceptions.
    //!
    //! For example:
    //! ~~~~
    ...
    //! throwing a `long` as an exception object which is not what the caller intended.  The normal @ref
    //! ResultFromException would fail-fast when this is encountered, but it would do so AFTER FunctionWhichMayThrow()
    //! is already off of the stack and has been unwound.  Because SEH is used for ResultFromExceptionDebug, the
    //! fail-fast occurs with everything leading up to and including the `throw INVALIDARG;` still on the stack (and
    //! easily debuggable).
    //!
    //! The penalty paid for using this, however, is efficiency.  It's far less efficient as a general pattern than
    //! either using ResultFromException directly or especially using try with CATCH_ macros directly.  Still it's
    //! helpful to deploy selectively to isolate issues a component may be having with unknown/unhandled exceptions.
    //!
    ...
    //! exception, you can use this function to quickly add an exception guard that will fail-fast any exception at the
    //! point the exception occurs (the throw) in a codepath where the origination of unknown exceptions need to be
    //! tracked down.
    //!
    //! This will fail-fast and stop on std::exception based exceptions (but not Platform::Exception^ or
    //! wil::ResultException). Using this can help isolate where an unexpected exception is being generated from.
    //! @param diagnostics  Always pass WI_DIAGNOSTICS_INFO as the first parameter
    //! @param supported    What kind of exceptions you want to support
    //! @param functor      A lambda that accepts no parameters; any return value is ignored
    //! @return             S_OK on success (no exception thrown) or an error based upon the exception thrown
    template <typename Functor>
    __forceinline HRESULT ResultFromExceptionDebug(const DiagnosticsInfo &diagnostics, SupportedExceptions supported,
                                                   Functor &&functor) WI_NOEXCEPT
    {
        static_assert(details::functor_tag<ErrorReturn::None, Functor>::value == details::tag_return_void::value,
                      "Functor must return void");
    ...
    //! This overload uses SupportedExceptions::Known by default.  See @ref ResultFromExceptionDebug for more detailed
    //! information.
    template <typename Functor>
    ...
    //! Technically this is an overload of @ref ResultFromExceptionDebug that uses SupportedExceptions::None by default.
    //! Any uncaught exception that makes it back to this guard would result in a fail-fast at the point the exception
    //! is thrown.
    template <typename Functor>
    ...
    namespace details
    {
                               ...
                               FailureFlags flags, _Out_ FailureInfo *failure) WI_NOEXCEPT
        {
            ...
            case FailureType::Log:
                ...
                {
                    // If you hit this assert (or are reviewing this failure telemetry), then most likely you are trying
                    // to log success using one of the WIL macros.  Example:
                    //      LOG_HR(S_OK);
            ...
            // If the hook is enabled then it will be given the opportunity to call RoOriginateError to greatly improve
            // the diagnostic experience for uncaught exceptions.  In cases where we will be throwing a C++/CX
            // Platform::Exception we should avoid originating because the CX runtime will be doing that for us.
            // fWantDebugString is only set to true when the caller will be throwing a Platform::Exception.
            ...
            {
                // Call the logging callback (if present) to allow them to generate the debug string that will be pushed
                // to the console or the platform exception object if the caller desires it.
                ...
                // The callback only optionally needs to supply the debug string -- if the callback didn't populate it,
                // yet we still want it for OutputDebugString or exception message, then generate the default string.
            ...
            {
                // [deprecated behavior]
                // This callback was at one point *always* called for all failures, so we continue to call it for
                // failures even when we don't need to generate the debug string information (when the callback was
                // supplied directly).  We can avoid this if the caller used the explicit function (through
                // g_resultMessageCallbackSet)
                if ((g_pfnResultLoggingCallback != nullptr) && !g_resultMessageCallbackSet)
                {
            ...
            if ((WI_IsFlagSet(failure->flags, FailureFlags::RequestDebugBreak) || g_fBreakOnFailure) &&
                (g_pfnDebugBreak != nullptr))
            {
                g_pfnDebugBreak();
            }
        }
        ...
        inline RESULT_NORETURN void __stdcall WilFailFast(const wil::FailureInfo &failure)
        {
            ...
            er.ExceptionInformation[0] =
                FAST_FAIL_FATAL_APP_EXIT; // see winnt.h, generated from minkernel\published\base\ntrtl_x.w
            if (failure.returnAddress ==
                nullptr) // FailureInfo does not have _ReturnAddress, have RaiseFailFastException generate it
            {
            ...
            else // use FailureInfo caller address
            {
                ...
                // parameter 2 is the line number.  This is never used for bucketing (due to code churn causing
                // re-bucketing) but is available in the dump's exception record to aid in failure locality. Putting it
                // here prevents it from being poisoned in triage dumps.
        ...
        inline __declspec(noinline) void ReportFailure_Return(__R_FN_PARAMS_FULL, const ResultStatus &resultPair,
                                                              PCWSTR message, ReportFailureOptions options,
                                                              FailureFlags flags)
        {
            bool needPlatformException =
                ((T == FailureType::Exception) && WI_IsFlagClear(options, ReportFailureOptions::MayRethrow) &&
                 ...
                 (g_fResultThrowPlatformException ||
                  WI_IsFlagSet(options, ReportFailureOptions::ForcePlatformException)));

            FailureInfo failure;
            wchar_t debugString[2048];
            char callContextString[1024];

            LogFailure(__R_FN_CALL_FULL, T, resultPair, message, needPlatformException, debugString,
                       ARRAYSIZE(debugString), callContextString, ARRAYSIZE(callContextString), flags, &failure);
        ...
        inline __declspec(noinline) void ReportFailure_Base(__R_FN_PARAMS_FULL, const ResultStatus &resultPair,
                                                            PCWSTR message, ReportFailureOptions options,
                                                            FailureFlags flags)
        {
        ...
        template <FailureType T>
        inline __declspec(noinline) RESULT_NORETURN void ReportFailure_NoReturn(__R_FN_PARAMS_FULL,
                                                                                const ResultStatus &resultPair,
                                                                                PCWSTR message,
                                                                                ReportFailureOptions options)
        {
            bool needPlatformException =
                ((T == FailureType::Exception) && WI_IsFlagClear(options, ReportFailureOptions::MayRethrow) &&
                 (g_pfnThrowPlatformException != nullptr) &&
            ...
            LogFailure(__R_FN_CALL_FULL, T, resultPair, message, needPlatformException, debugString,
                       ARRAYSIZE(debugString), callContextString, ARRAYSIZE(callContextString), FailureFlags::None,
                       &failure);
            ...
            else
            {
                ...
                // Wil was instructed to throw, but doesn't have any capability to do so (global function pointers are
                // not setup)
                WilFailFast(const_cast<FailureInfo &>(failure));
            }
        ...
        __declspec(noinline) inline void ReportFailure(__R_FN_PARAMS_FULL, FailureType type,
                                                       const ResultStatus &resultPair, _In_opt_ PCWSTR message,
                                                       ReportFailureOptions options)
        {
            switch (type)
            {
        ...
        template <FailureType T>
        inline ResultStatus ReportFailure_CaughtExceptionCommon(__R_FN_PARAMS_FULL,
                                                                ...
                                                                SupportedExceptions supported)
        {
            ...
            if (details::g_pfnResultFromCaughtExceptionInternal)
            {
                resultPair = details::g_pfnResultFromCaughtExceptionInternal(debugString + length,
                                                                             debugStringChars - length, &isNormalized);
            }
            ...
            {
                // By default WIL will issue a fail fast for unrecognized exception types.  Wil recognizes any
                // std::exception or wil::ResultException based types and Platform::Exception^, so there aren't too many
                // valid exception types which could cause this.  Those that are valid, should be handled by remapping
                // the exception callback.  Those that are not valid should be found and fixed (meaningless accidents
                // like 'throw hr;'). The caller may also be requesting non-default behavior to fail-fast more
                // frequently (primarily for debugging unknown exceptions).
        ...
        inline ResultStatus RESULT_NORETURN ReportFailure_CaughtExceptionCommonNoReturnBase(
            __R_FN_PARAMS_FULL, _Inout_updates_(debugStringChars) PWSTR debugString,
            _Pre_satisfies_(debugStringChars > 0) size_t debugStringChars, SupportedExceptions supported)
        {
            bool isNormalized = false;
        ...
        template <FailureType T>
        inline void ReportFailure_Msg(__R_FN_PARAMS_FULL, const ResultStatus &resultPair,
                                      _Printf_format_string_ PCSTR formatString, va_list argList)
        {
            wchar_t message[2048];
        ...
        template <>
        inline RESULT_NORETURN void ReportFailure_Msg<FailureType::FailFast>(__R_FN_PARAMS_FULL,
                                                                             const ResultStatus &resultPair,
        ...
        template <>
        inline RESULT_NORETURN void ReportFailure_Msg<FailureType::Exception>(__R_FN_PARAMS_FULL,
                                                                              const ResultStatus &resultPair,
                                                                              _Printf_format_string_ PCSTR formatString,
                                                                              va_list argList)
        {
        ...
        {
            ReportFailure_Base<T>(__R_FN_CALL_FULL, ResultStatus::FromResult(hr), nullptr /*message*/,
                                  ReportFailureOptions::None /*options*/, flags);
        }

        template <>
        __declspec(noinline) inline RESULT_NORETURN void ReportFailure_Hr<FailureType::FailFast>(__R_FN_PARAMS_FULL,
                                                                                                 HRESULT hr,
                                                                                                 FailureFlags)
        {
        ...
        template <>
        __declspec(noinline) inline RESULT_NORETURN void ReportFailure_Hr<FailureType::Exception>(__R_FN_PARAMS_FULL,
                                                                                                  HRESULT hr,
                                                                                                  FailureFlags)
        {
        ...
        template <FailureType T>
        _Success_(true) _Translates_Win32_to_HRESULT_(err) __declspec(noinline) inline HRESULT
            ReportFailure_Win32(__R_FN_PARAMS_FULL, DWORD err)
        {
        ...
        template <>
        _Success_(true) _Translates_Win32_to_HRESULT_(err) __declspec(noinline) inline RESULT_NORETURN HRESULT
        ...
        template <FailureType T> __declspec(noinline) inline DWORD ReportFailure_GetLastError(__R_FN_PARAMS_FULL)
        {
        ...
        ReportFailure_GetLastError<FailureType::FailFast>(__R_FN_PARAMS_FULL)
        {
            const auto err = GetLastErrorFail(__R_FN_CALL_FULL);
            const auto hr = __HRESULT_FROM_WIN32(err);
            ReportFailure_Base<FailureType::FailFast>(__R_FN_CALL_FULL, ResultStatus::FromResult(hr));
        ...
        template <>
        __declspec(noinline) inline RESULT_NORETURN DWORD
        ReportFailure_GetLastError<FailureType::Exception>(__R_FN_PARAMS_FULL)
        {
            const auto err = GetLastErrorFail(__R_FN_CALL_FULL);
            const auto hr = __HRESULT_FROM_WIN32(err);
            ReportFailure_Base<FailureType::Exception>(__R_FN_CALL_FULL, ResultStatus::FromResult(hr));
        ...
        template <FailureType T>
        _Success_(true) _Translates_last_error_to_HRESULT_ __declspec(noinline) inline HRESULT
            ReportFailure_GetLastErrorHr(__R_FN_PARAMS_FULL)
        {
        ...
        template <>
        _Success_(true) _Translates_last_error_to_HRESULT_ __declspec(noinline) inline RESULT_NORETURN HRESULT
        ...
        template <FailureType T>
        _Success_(true) _Translates_NTSTATUS_to_HRESULT_(status) __declspec(noinline) inline HRESULT
        ...
        template <>
        _Success_(true) _Translates_NTSTATUS_to_HRESULT_(status) __declspec(noinline) inline RESULT_NORETURN HRESULT
        ...
        template <FailureType T>
        __declspec(noinline) inline HRESULT ReportFailure_CaughtException(__R_FN_PARAMS_FULL,
                                                                          SupportedExceptions supported)
        {
            wchar_t message[2048]{};
        ...
        template <FailureType T>
        __declspec(noinline) inline void ReportFailure_HrMsg(__R_FN_PARAMS_FULL, HRESULT hr,
                                                             _Printf_format_string_ PCSTR formatString, va_list argList)
        {
            ReportFailure_Msg<T>(__R_FN_CALL_FULL, ResultStatus::FromResult(hr), formatString, argList);
        }
        ...
        {
            ReportFailure_Msg<FailureType::FailFast>(__R_FN_CALL_FULL, ResultStatus::FromResult(hr), formatString,
                                                     argList);
        }
        ...
        __declspec(noinline) inline RESULT_NORETURN void ReportFailure_HrMsg<FailureType::Exception>(
            __R_FN_PARAMS_FULL, HRESULT hr, _Printf_format_string_ PCSTR formatString, va_list argList)
        {
            ReportFailure_Msg<FailureType::Exception>(__R_FN_CALL_FULL, ResultStatus::FromResult(hr), formatString,
                                                      argList);
        }

        template <FailureType T>
        _Success_(true) _Translates_Win32_to_HRESULT_(err) __declspec(noinline) inline HRESULT
            ReportFailure_Win32Msg(__R_FN_PARAMS_FULL, DWORD err, _Printf_format_string_ PCSTR formatString,
                                   va_list argList)
        {
        ...
        template <>
        _Success_(true) _Translates_Win32_to_HRESULT_(err)
        ...
        template <FailureType T>
        __declspec(noinline) inline DWORD ReportFailure_GetLastErrorMsg(__R_FN_PARAMS_FULL,
                                                                        _Printf_format_string_ PCSTR formatString,
                                                                        va_list argList)
        {
        ...
        template <FailureType T>
        _Success_(true) _Translates_last_error_to_HRESULT_ __declspec(noinline) inline HRESULT
            ReportFailure_GetLastErrorHrMsg(__R_FN_PARAMS_FULL, _Printf_format_string_ PCSTR formatString,
                                            va_list argList)
        {
        ...
        template <>
        _Success_(true) _Translates_last_error_to_HRESULT_
        ...
        template <FailureType T>
        _Success_(true) _Translates_NTSTATUS_to_HRESULT_(status) __declspec(noinline) inline HRESULT
            ReportFailure_NtStatusMsg(__R_FN_PARAMS_FULL, NTSTATUS status, _Printf_format_string_ PCSTR formatString,
                                      va_list argList)
        {
            const auto resultPair = ResultStatus::FromStatus(status);
        ...
        template <>
        _Success_(true) _Translates_NTSTATUS_to_HRESULT_(status)
        ...
        template <FailureType T>
        __declspec(noinline) inline HRESULT ReportFailure_CaughtExceptionMsg(__R_FN_PARAMS_FULL,
                                                                             _Printf_format_string_ PCSTR formatString,
                                                                             va_list argList)
        {
    ...
    inline void SetFailureInfo(_In_ FailureInfo const&, _Inout_ Platform::Exception^ exception) WI_NOEXCEPT
    {
        ...
        template <typename T>
        RESULT_NORETURN inline void ReportFailure_CustomExceptionHelper(_Inout_ T &exception, __R_FN_PARAMS_FULL,
                                                                        _In_opt_ PCWSTR message = nullptr)
        {
            ...
            LogFailure(__R_FN_CALL_FULL, FailureType::Exception, ResultStatus::FromResult(hr), message,
                       false, // false = does not need debug string
                       debugString, ARRAYSIZE(debugString), callContextString, ARRAYSIZE(callContextString),
                       FailureFlags::None, &failure);
        ...
        namespace __R_NS_NAME
        {
            ...
            _Success_(true) _Translates_Win32_to_HRESULT_(err)
                __R_DIRECT_METHOD(HRESULT, Return_Win32Msg)(__R_DIRECT_FN_PARAMS DWORD err,
                                                            _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                va_list argList;
                va_start(argList, formatString);
                __R_FN_LOCALS;
                return wil::details::ReportFailure_Win32Msg<FailureType::Return>(__R_DIRECT_FN_CALL err, formatString,
                                                                                 argList);
            }

            _Success_(true) _Translates_last_error_to_HRESULT_ __R_DIRECT_METHOD(HRESULT, Return_GetLastErrorMsg)(
                __R_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                va_list argList;
                va_start(argList, formatString);
                __R_FN_LOCALS;
                return wil::details::ReportFailure_GetLastErrorHrMsg<FailureType::Return>(
                    __R_DIRECT_FN_CALL formatString, argList);
            }

            _Success_(true) _Translates_NTSTATUS_to_HRESULT_(status)
                __R_DIRECT_METHOD(HRESULT, Return_NtStatusMsg)(__R_DIRECT_FN_PARAMS NTSTATUS status,
                                                               _Printf_format_string_ PCSTR formatString,
                                                               ...) WI_NOEXCEPT
            {
                va_list argList;
                va_start(argList, formatString);
                __R_FN_LOCALS;
                return wil::details::ReportFailure_NtStatusMsg<FailureType::Return>(__R_DIRECT_FN_CALL status,
            ...
            (__R_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                return wil::details::ReportFailure_CaughtExceptionMsg<FailureType::Return>(
                    __R_DIRECT_FN_CALL formatString, argList);
            }
#endif
            ...
            _Post_satisfies_(return == hr)
                __R_CONDITIONAL_NOINLINE_METHOD(HRESULT, Log_IfFailedWithExpected)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
            ...
            _Post_satisfies_(return == pointer) __R_CONDITIONAL_TEMPLATE_METHOD(PointerT, Log_IfNullAlloc)(
            ...
            __R_CONDITIONAL_TEMPLATE_METHOD(void, Log_IfNullAlloc)(__R_CONDITIONAL_FN_PARAMS const PointerT &pointer)
                WI_NOEXCEPT
            {
                if (pointer == nullptr)
                {
            ...
            _Post_satisfies_(return == pointer)
                __R_CONDITIONAL_TEMPLATE_METHOD(PointerT, Log_HrIfNull)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_CLASS(PointerT)>
            __R_CONDITIONAL_TEMPLATE_METHOD(void, Log_HrIfNull)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_NOT_CLASS(PointerT)>
            _Post_satisfies_(return == pointer) __R_CONDITIONAL_TEMPLATE_METHOD(PointerT, Log_GetLastErrorIfNull)(
                __R_CONDITIONAL_FN_PARAMS _Pre_maybenull_ PointerT pointer) WI_NOEXCEPT
            {
                if (pointer == nullptr)
                {
            ...
            (__R_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                return wil::details::ReportFailure_GetLastErrorMsg<FailureType::Log>(__R_DIRECT_FN_CALL formatString,
                                                                                     argList);
            }

            _Post_satisfies_(return == status)
                __R_DIRECT_METHOD(NTSTATUS, Log_NtStatusMsg)(__R_DIRECT_FN_PARAMS NTSTATUS status,
                                                             _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                va_list argList;
                va_start(argList, formatString);
                __R_FN_LOCALS;
                wil::details::ReportFailure_NtStatusMsg<FailureType::Log>(__R_DIRECT_FN_CALL status, formatString,
                                                                          argList);
                return status;
            }
            ...
            (__R_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                return wil::details::ReportFailure_CaughtExceptionMsg<FailureType::Log>(__R_DIRECT_FN_CALL formatString,
                                                                                        argList);
            }
#endif
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::Log>(__R_INTERNAL_NOINLINE_FN_CALL hr, formatString,
                                                                    argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_GetLastErrorMsg<FailureType::Log>(
                    __R_INTERNAL_NOINLINE_FN_CALL formatString, argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_Win32Msg<FailureType::Log>(__R_INTERNAL_NOINLINE_FN_CALL err, formatString,
                                                                       argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::Log>(__R_INTERNAL_NOINLINE_FN_CALL E_OUTOFMEMORY,
                                                                    formatString, argList);
            }

            __R_INTERNAL_NOINLINE_METHOD(_Log_NtStatusMsg)
            (__R_INTERNAL_NOINLINE_FN_PARAMS NTSTATUS status, _Printf_format_string_ PCSTR formatString,
             va_list argList) WI_NOEXCEPT
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_NtStatusMsg<FailureType::Log>(__R_INTERNAL_NOINLINE_FN_CALL status,
                                                                          formatString, argList);
            }

            _Post_satisfies_(return == hr)
                __R_CONDITIONAL_NOINLINE_METHOD(HRESULT, Log_IfFailedMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                          _Printf_format_string_ PCSTR formatString,
                                                                          ...) WI_NOEXCEPT
            {
                if (FAILED(hr))
                {
                    va_list argList;
                    va_start(argList, formatString);
                    __R_CALL_INTERNAL_NOINLINE_METHOD(_Log_HrMsg)(__R_CONDITIONAL_NOINLINE_FN_CALL hr, formatString,
                                                                  argList);
                }
                return hr;
            }
            ...
            _Post_satisfies_(return == err)
                __R_CONDITIONAL_NOINLINE_METHOD(DWORD, Log_IfWin32ErrorMsg)(__R_CONDITIONAL_FN_PARAMS DWORD err,
                                                                            _Printf_format_string_ PCSTR formatString,
                                                                            ...) WI_NOEXCEPT
            {
                if (FAILED_WIN32(err))
                {
                    va_list argList;
                    va_start(argList, formatString);
                    __R_CALL_INTERNAL_NOINLINE_METHOD(_Log_Win32Msg)(__R_CONDITIONAL_NOINLINE_FN_CALL err, formatString,
                                                                     argList);
                }
                return err;
            }
            ...
            _Post_satisfies_(return == handle)
                __R_CONDITIONAL_NOINLINE_METHOD(HANDLE, Log_IfHandleNullMsg)(__R_CONDITIONAL_FN_PARAMS HANDLE handle,
                                                                             _Printf_format_string_ PCSTR formatString,
                                                                             ...) WI_NOEXCEPT
            {
                if (handle == nullptr)
            ...
            _Post_satisfies_(return == condition)
                __R_CONDITIONAL_NOINLINE_METHOD(bool, Log_HrIfMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr, bool condition,
                                                                   _Printf_format_string_ PCSTR formatString,
                                                                   ...) WI_NOEXCEPT
            {
                if (condition)
            ...
            _Post_satisfies_(return == condition)
                __R_CONDITIONAL_NOINLINE_METHOD(bool, Log_HrIfFalseMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                        bool condition,
                                                                        _Printf_format_string_ PCSTR formatString,
                                                                        ...) WI_NOEXCEPT
            {
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_CLASS(PointerT)>
            __R_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(void, Log_HrIfNullMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
            ...
            _Post_satisfies_(return == condition)
                __R_CONDITIONAL_NOINLINE_METHOD(bool, Log_GetLastErrorIfMsg)(__R_CONDITIONAL_FN_PARAMS bool condition,
                                                                             _Printf_format_string_ PCSTR formatString,
                                                                             ...) WI_NOEXCEPT
            {
                if (condition)
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_NOT_CLASS(PointerT)>
            _Post_satisfies_(return == pointer) __R_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(
                PointerT, Log_GetLastErrorIfNullMsg)(__R_CONDITIONAL_FN_PARAMS _Pre_maybenull_ PointerT pointer,
                                                     _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                if (pointer == nullptr)
                {
        ...
        namespace __RFF_NS_NAME
        {
            ...
            _Post_satisfies_(return == ret) _When_(!ret, _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(BOOL,
                                         FailFast_IfWin32BoolFalse)(__RFF_CONDITIONAL_FN_PARAMS BOOL ret) WI_NOEXCEPT
            {
                if (!ret)
                {
            ...
            _Post_satisfies_(return == handle) _When_(handle == INVALID_HANDLE_VALUE, _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(HANDLE, FailFast_IfHandleInvalid)(__RFF_CONDITIONAL_FN_PARAMS HANDLE handle)
                    WI_NOEXCEPT
            {
                if (handle == INVALID_HANDLE_VALUE)
                {
            ...
            _Post_satisfies_(return == handle) _When_(handle == nullptr, _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(RESULT_NORETURN_NULL HANDLE,
                                         FailFast_IfHandleNull)(__RFF_CONDITIONAL_FN_PARAMS HANDLE handle) WI_NOEXCEPT
            {
                if (handle == nullptr)
                {
                    __RFF_CALL_INTERNAL_METHOD(_FailFast_GetLastError)(__RFF_CONDITIONAL_FN_CALL_ONLY);
            ...
            __RFF_CONDITIONAL_TEMPLATE_METHOD(void,
                                              FailFast_IfNullAlloc)(__RFF_CONDITIONAL_FN_PARAMS const PointerT &pointer)
            {
                if (pointer == nullptr)
                {
                    __RFF_CALL_INTERNAL_METHOD(_FailFast_NullAlloc)(__RFF_CONDITIONAL_FN_CALL_ONLY);
            ...
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(bool, FailFast_HrIfFalse)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                   bool condition) WI_NOEXCEPT
            {
                if (!condition)
                {
            ...
            template <__RFF_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_NOT_CLASS(PointerT)>
            _Post_satisfies_(return == pointer) _When_(pointer == nullptr, _Analysis_noreturn_)
                __RFF_CONDITIONAL_TEMPLATE_METHOD(RESULT_NORETURN_NULL PointerT,
                                                  FailFast_HrIfNull)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
            ...
            // Should be decorated WI_NOEXCEPT, but conflicts with forceinline.
            template <__RFF_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_CLASS(PointerT)>
            __RFF_CONDITIONAL_TEMPLATE_METHOD(void, FailFast_HrIfNull)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                       _In_opt_ const PointerT &pointer)
            {
                if (pointer == nullptr)
                {
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_) __RFF_CONDITIONAL_METHOD(
                bool, FailFast_GetLastErrorIf)(__RFF_CONDITIONAL_FN_PARAMS bool condition) WI_NOEXCEPT
            {
                if (condition)
                {
                    __RFF_CALL_INTERNAL_METHOD(_FailFast_GetLastError)(__RFF_CONDITIONAL_FN_CALL_ONLY);
            ...
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_) __RFF_CONDITIONAL_METHOD(
                bool, FailFast_GetLastErrorIfFalse)(__RFF_CONDITIONAL_FN_PARAMS bool condition) WI_NOEXCEPT
            {
                if (!condition)
                {
                    __RFF_CALL_INTERNAL_METHOD(_FailFast_GetLastError)(__RFF_CONDITIONAL_FN_CALL_ONLY);
            ...
            _Post_satisfies_(return == status) _When_(FAILED_NTSTATUS(status), _Analysis_noreturn_)
                __RFF_CONDITIONAL_METHOD(NTSTATUS, FailFast_IfNtStatusFailed)(
                    __RFF_CONDITIONAL_FN_PARAMS NTSTATUS status) WI_NOEXCEPT
            {
                if (FAILED_NTSTATUS(status))
                {
            ...
            (__RFF_DIRECT_FN_PARAMS HRESULT hr, _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::FailFast>(__RFF_DIRECT_FN_CALL hr, formatString,
                                                                         argList);
            }
            ...
            (__RFF_DIRECT_FN_PARAMS DWORD err, _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_Win32Msg<FailureType::FailFast>(__RFF_DIRECT_FN_CALL err, formatString,
                                                                            argList);
            }
            ...
            (__RFF_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_GetLastErrorMsg<FailureType::FailFast>(__RFF_DIRECT_FN_CALL formatString,
                                                                                   argList);
            }
            ...
            (__RFF_DIRECT_FN_PARAMS NTSTATUS status, _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_NtStatusMsg<FailureType::FailFast>(__RFF_DIRECT_FN_CALL status,
                                                                               formatString, argList);
            }
            ...
            (__RFF_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_CaughtExceptionMsg<FailureType::FailFast>(__RFF_DIRECT_FN_CALL formatString,
                                                                                      argList);
            }
#endif

            __RFF_INTERNAL_NOINLINE_NORET_METHOD(_FailFast_HrMsg)
            (__RFF_INTERNAL_NOINLINE_FN_PARAMS HRESULT hr, _Printf_format_string_ PCSTR formatString, va_list argList)
                WI_NOEXCEPT
            {
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::FailFast>(__RFF_INTERNAL_NOINLINE_FN_CALL hr,
                                                                         formatString, argList);
            }
            ...
            {
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_GetLastErrorMsg<FailureType::FailFast>(
                    __RFF_INTERNAL_NOINLINE_FN_CALL formatString, argList);
            }

            __RFF_INTERNAL_NOINLINE_NORET_METHOD(_FailFast_Win32Msg)
            (__RFF_INTERNAL_NOINLINE_FN_PARAMS DWORD err, _Printf_format_string_ PCSTR formatString, va_list argList)
                WI_NOEXCEPT
            {
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_Win32Msg<FailureType::FailFast>(__RFF_INTERNAL_NOINLINE_FN_CALL err,
                                                                            formatString, argList);
            }
            ...
            {
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::FailFast>(__RFF_INTERNAL_NOINLINE_FN_CALL E_OUTOFMEMORY,
                                                                         formatString, argList);
            }

            __RFF_INTERNAL_NOINLINE_NORET_METHOD(_FailFast_NtStatusMsg)
            (__RFF_INTERNAL_NOINLINE_FN_PARAMS NTSTATUS status, _Printf_format_string_ PCSTR formatString,
             va_list argList) WI_NOEXCEPT
            {
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_NtStatusMsg<FailureType::FailFast>(__RFF_INTERNAL_NOINLINE_FN_CALL status,
                                                                               formatString, argList);
            }

            _Post_satisfies_(return == hr) _When_(FAILED(hr), _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(HRESULT,
                                                  FailFast_IfFailedMsg)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                        _Printf_format_string_ PCSTR formatString,
                                                                        ...) WI_NOEXCEPT
            {
                if (FAILED(hr))
            ...
            _Post_satisfies_(return == err) _When_(FAILED_WIN32(err), _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(DWORD,
                                                  FailFast_IfWin32ErrorMsg)(__RFF_CONDITIONAL_FN_PARAMS DWORD err,
                                                                            _Printf_format_string_ PCSTR formatString,
                                                                            ...) WI_NOEXCEPT
            {
                if (FAILED_WIN32(err))
            ...
            _Post_satisfies_(return == handle) _When_(handle == INVALID_HANDLE_VALUE, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(HANDLE, FailFast_IfHandleInvalidMsg)(
                    __RFF_CONDITIONAL_FN_PARAMS HANDLE handle, _Printf_format_string_ PCSTR formatString,
                    ...) WI_NOEXCEPT
            {
                if (handle == INVALID_HANDLE_VALUE)
                {
            ...
            _Post_satisfies_(return == handle) _When_(handle == nullptr, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(RESULT_NORETURN_NULL HANDLE,
                                                  FailFast_IfHandleNullMsg)(__RFF_CONDITIONAL_FN_PARAMS HANDLE handle,
                                                                            _Printf_format_string_ PCSTR formatString,
                                                                            ...) WI_NOEXCEPT
            {
                if (handle == nullptr)
            ...
            __RFF_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(void, FailFast_IfNullAllocMsg)(
                __RFF_CONDITIONAL_FN_PARAMS const PointerT &pointer, _Printf_format_string_ PCSTR formatString,
                ...) WI_NOEXCEPT
            {
                if (pointer == nullptr)
                {
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(bool, FailFast_HrIfMsg)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                          bool condition,
                                                                          _Printf_format_string_ PCSTR formatString,
                                                                          ...) WI_NOEXCEPT
            {
            ...
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(bool,
                                                  FailFast_HrIfFalseMsg)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                         bool condition,
                                                                         _Printf_format_string_ PCSTR formatString,
                                                                         ...) WI_NOEXCEPT
            {
            ...
            template <__RFF_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_CLASS(PointerT)>
            __RFF_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(void,
                                                       FailFast_HrIfNullMsg)(__RFF_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                             _In_opt_ const PointerT &pointer,
                                                                             _Printf_format_string_ PCSTR formatString,
                                                                             ...) WI_NOEXCEPT
            {
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(bool, FailFast_GetLastErrorIfMsg)(
                    __RFF_CONDITIONAL_FN_PARAMS bool condition, _Printf_format_string_ PCSTR formatString,
                    ...) WI_NOEXCEPT
            {
                if (condition)
                {
            ...
            template <__RFF_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_NOT_CLASS(PointerT)>
            _Post_satisfies_(return == pointer) _When_(pointer == nullptr, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(RESULT_NORETURN_NULL PointerT,
                                                           FailFast_GetLastErrorIfNullMsg)(
                    __RFF_CONDITIONAL_FN_PARAMS _Pre_maybenull_ PointerT pointer,
                    _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                if (pointer == nullptr)
            ...
            __RFF_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(void, FailFast_GetLastErrorIfNullMsg)(
                __RFF_CONDITIONAL_FN_PARAMS _In_opt_ const PointerT &pointer, _Printf_format_string_ PCSTR formatString,
                ...) WI_NOEXCEPT
            {
                if (pointer == nullptr)
                {
            ...
            _Post_satisfies_(return == status) _When_(FAILED_NTSTATUS(status), _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(NTSTATUS, FailFast_IfNtStatusFailedMsg)(
                    __RFF_CONDITIONAL_FN_PARAMS NTSTATUS status, _Printf_format_string_ PCSTR formatString,
                    ...) WI_NOEXCEPT
            {
                if (FAILED_NTSTATUS(status))
                {
            ...
            // Should be decorated WI_NOEXCEPT, but conflicts with forceinline.
            template <__RFF_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_NOT_CLASS(PointerT)>
            __WI_SUPPRESS_NULLPTR_ANALYSIS _Post_satisfies_(return == pointer)
            ...
            (__RFF_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...) WI_NOEXCEPT
            {
                ...
                va_start(argList, formatString);
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::FailFast>(__RFF_DIRECT_FN_CALL E_UNEXPECTED,
                                                                         formatString, argList);
            }
            ...
            {
                __RFF_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::FailFast>(__RFF_INTERNAL_NOINLINE_FN_CALL E_UNEXPECTED,
                                                                         formatString, argList);
            }

            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(bool, FailFast_IfMsg)(__RFF_CONDITIONAL_FN_PARAMS bool condition,
                                                                        _Printf_format_string_ PCSTR formatString,
                                                                        ...) WI_NOEXCEPT
            {
                if (condition)
            ...
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_)
                __RFF_CONDITIONAL_NOINLINE_METHOD(bool, FailFast_IfFalseMsg)(__RFF_CONDITIONAL_FN_PARAMS bool condition,
                                                                             _Printf_format_string_ PCSTR formatString,
                                                                             ...) WI_NOEXCEPT
            {
                if (!condition)
            ...
            _Post_satisfies_(return == pointer) _When_(pointer == nullptr, _Analysis_noreturn_)
                __RFF_CONDITIONAL_TEMPLATE_METHOD(RESULT_NORETURN_NULL PointerT,
                                                  FailFastImmediate_IfNull)(_Pre_maybenull_ PointerT pointer)
            {
                if (pointer == nullptr)
                {
                    __RFF_CALL_INTERNAL_METHOD(_FailFastImmediate_Unexpected)();
        ...
        namespace __R_NS_NAME
        {
            ...
            _Post_satisfies_(return == handle) _When_(handle == nullptr, _Analysis_noreturn_)
                __R_CONDITIONAL_METHOD(RESULT_NORETURN_NULL HANDLE,
                                       Throw_IfHandleNull)(__R_CONDITIONAL_FN_PARAMS HANDLE handle)
            {
                if (handle == nullptr)
                {
                    __R_CALL_INTERNAL_METHOD(_Throw_GetLastError)(__R_CONDITIONAL_FN_CALL_ONLY);
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_NOT_CLASS(PointerT)>
            _Post_satisfies_(return == pointer) _When_(pointer == nullptr, _Analysis_noreturn_)
                __R_CONDITIONAL_TEMPLATE_METHOD(RESULT_NORETURN_NULL PointerT,
                                                Throw_HrIfNull)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                _Pre_maybenull_ PointerT pointer)
            {
                if (pointer == nullptr)
                {
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_CLASS(PointerT)>
            __R_CONDITIONAL_TEMPLATE_METHOD(void, Throw_HrIfNull)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                  _In_opt_ const PointerT &pointer)
            {
                if (pointer == nullptr)
                {
            ...
            (__R_DIRECT_FN_PARAMS DWORD err, _Printf_format_string_ PCSTR formatString, ...)
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                wil::details::ReportFailure_Win32Msg<FailureType::Exception>(__R_DIRECT_FN_CALL err, formatString,
                                                                             argList);
            }
                ...
                __R_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...)
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                wil::details::ReportFailure_GetLastErrorMsg<FailureType::Exception>(__R_DIRECT_FN_CALL formatString,
                                                                                    argList);
            }
            ...
            (__R_DIRECT_FN_PARAMS NTSTATUS status, _Printf_format_string_ PCSTR formatString, ...)
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                wil::details::ReportFailure_NtStatusMsg<FailureType::Exception>(__R_DIRECT_FN_CALL status, formatString,
                                                                                argList);
            }
            ...
            (__R_DIRECT_FN_PARAMS _Printf_format_string_ PCSTR formatString, ...)
            {
                ...
                va_start(argList, formatString);
                __R_FN_LOCALS;
                wil::details::ReportFailure_CaughtExceptionMsg<FailureType::Exception>(__R_DIRECT_FN_CALL formatString,
                                                                                       argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::Exception>(__R_INTERNAL_NOINLINE_FN_CALL hr,
                                                                          formatString, argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_GetLastErrorMsg<FailureType::Exception>(
                    __R_INTERNAL_NOINLINE_FN_CALL formatString, argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_Win32Msg<FailureType::Exception>(__R_INTERNAL_NOINLINE_FN_CALL err,
                                                                             formatString, argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_HrMsg<FailureType::Exception>(__R_INTERNAL_NOINLINE_FN_CALL E_OUTOFMEMORY,
                                                                          formatString, argList);
            }
            ...
            {
                __R_FN_LOCALS;
                wil::details::ReportFailure_NtStatusMsg<FailureType::Exception>(__R_INTERNAL_NOINLINE_FN_CALL status,
                                                                                formatString, argList);
            }

            _Post_satisfies_(return == hr) _When_(FAILED(hr), _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(HRESULT, Throw_IfFailedMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                            _Printf_format_string_ PCSTR formatString,
                                                                            ...)
            {
                if (FAILED(hr))
                {
                    va_list argList;
                    va_start(argList, formatString);
                    __R_CALL_INTERNAL_NOINLINE_METHOD(_Throw_HrMsg)(__R_CONDITIONAL_NOINLINE_FN_CALL hr, formatString,
                                                                    argList);
                }
                return hr;
            }
            ...
            _Post_satisfies_(return == ret) _When_(!ret, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(BOOL,
                                                Throw_IfWin32BoolFalseMsg)(__R_CONDITIONAL_FN_PARAMS BOOL ret,
                                                                           _Printf_format_string_ PCSTR formatString,
                                                                           ...)
            {
                if (!ret)
            ...
            _Post_satisfies_(return == err) _When_(FAILED_WIN32(err), _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(DWORD, Throw_IfWin32ErrorMsg)(__R_CONDITIONAL_FN_PARAMS DWORD err,
                                                                              _Printf_format_string_ PCSTR formatString,
                                                                              ...)
            {
                if (FAILED_WIN32(err))
                {
                    va_list argList;
                    va_start(argList, formatString);
                    __R_CALL_INTERNAL_NOINLINE_METHOD(_Throw_Win32Msg)(__R_CONDITIONAL_NOINLINE_FN_CALL err,
                                                                       formatString, argList);
                }
                return err;
            }
            ...
            _Post_satisfies_(return == handle) _When_(handle == INVALID_HANDLE_VALUE, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(HANDLE,
                                                Throw_IfHandleInvalidMsg)(__R_CONDITIONAL_FN_PARAMS HANDLE handle,
                                                                          _Printf_format_string_ PCSTR formatString,
                                                                          ...)
            {
                if (handle == INVALID_HANDLE_VALUE)
            ...
            _Post_satisfies_(return == handle) _When_(handle == 0, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(RESULT_NORETURN_NULL HANDLE,
                                                Throw_IfHandleNullMsg)(__R_CONDITIONAL_FN_PARAMS HANDLE handle,
                                                                       _Printf_format_string_ PCSTR formatString, ...)
            {
                if (handle == nullptr)
                {
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(bool, Throw_HrIfMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                     bool condition,
                                                                     _Printf_format_string_ PCSTR formatString, ...)
            {
                if (condition)
            ...
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(bool, Throw_HrIfFalseMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                          bool condition,
                                                                          _Printf_format_string_ PCSTR formatString,
                                                                          ...)
            {
            ...
            template <__R_CONDITIONAL_PARTIAL_TEMPLATE typename PointerT, __R_ENABLE_IF_IS_CLASS(PointerT)>
            __WI_SUPPRESS_NULLPTR_ANALYSIS _When_(pointer == nullptr, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_TEMPLATE_METHOD(void,
                                                         Throw_HrIfNullMsg)(__R_CONDITIONAL_FN_PARAMS HRESULT hr,
                                                                            _In_opt_ const PointerT &pointer,
                                                                            _Printf_format_string_ PCSTR formatString,
                                                                            ...)
            {
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(bool, Throw_Win32IfMsg)(__R_CONDITIONAL_FN_PARAMS DWORD err,
                                                                        bool condition,
                                                                        _Printf_format_string_ PCSTR formatString, ...)
            {
                if (condition)
            ...
            _Post_satisfies_(return == condition) _When_(condition, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(bool,
                                                Throw_GetLastErrorIfMsg)(__R_CONDITIONAL_FN_PARAMS bool condition,
                                                                         _Printf_format_string_ PCSTR formatString, ...)
            {
                if (condition)
                {
            ...
            _Post_satisfies_(return == condition) _When_(!condition, _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(bool,
                                                Throw_GetLastErrorIfFalseMsg)(__R_CONDITIONAL_FN_PARAMS bool condition,
                                                                              _Printf_format_string_ PCSTR formatString,
                                                                              ...)
            {
                if (!condition)
            ...
            _Post_satisfies_(return == status) _When_(FAILED_NTSTATUS(status), _Analysis_noreturn_)
                __R_CONDITIONAL_NOINLINE_METHOD(NTSTATUS,
                                                Throw_IfNtStatusFailedMsg)(__R_CONDITIONAL_FN_PARAMS NTSTATUS status,
                                                                           _Printf_format_string_ PCSTR formatString,
                                                                           ...)
            {
                if (FAILED_NTSTATUS(status))
